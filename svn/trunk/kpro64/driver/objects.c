	...
	case O_l:
		...
		if (strcmp(arg, "m") == 0 ||
			...
			if (xpg_flag && invoked_lang == L_f77) {
				...
				//char *f77math;
				//f77math = concat_strings(root, "/lib64/libf77math" TOOL_EXTENSION ".a");
				//add_object(O_object, f77math);
				//f77math = concat_strings(root, "/lib64/libaf77math" TOOL_EXTENSION ".a");
				//add_object(O_object, f77math);
				add_library(objects, "af77math");
