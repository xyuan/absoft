...
static char *rcs_id = "$Source$ $Revision: 2663 $";
...
/*            fprintf(stderr, "This software has expired.\n"
	...
	if (dump_version) {
        ...
        if (show_version) {
            fprintf(stderr, "Absoft C/C++ Compiler %s\n", 
			  compiler_version);
	    if (show_version > 1) {
		...
		fprintf(stderr, "Built by: %s@%s in %s\n", build_user,
            ...
            fprintf(stderr, "GNU gcc version " PSC_GCC_VERSION
                    " (Absoft " PSC_FULL_VERSION " driver)\n");
        }
	/*
	if (show_copyright) {
	    if (show_version)
	    ...
	    fprintf(stderr, "Copyright 2002, 2003, 2004, 2005 PathScale, Inc.  "
		    "All Rights Reserved.\n");
	    fprintf(stderr, "Copyright 2005 Absoft Corporation."
		    "All Rights Reserved.\n");

	    fprintf(stderr, "See complete copyright, patent and legal notices "
		    "in the\n");
	    fprintf(stderr, "%.*s/license.txt file\n",
...
static void set_executable_dir (void) {
  ...
  if (strcmp (ldir, "bin") == 0) {
    ...
    substitute_phase_dirs ("/usr/lib/" PSC_NAME_PREFIX "cc-lib",
			   basedir, "/lib");
    substitute_phase_dirs ("/usr/lib/" PSC_NAME_PREFIX "cc-lib/bin",
			   basedir, "/bin");
...
print_help_msg (void)
{
	...
	if (help_pattern == NULL && invoked_lang == L_cc) {
	  fprintf(stderr, "The environment variable ABSOFT_CC is also checked\n");
	}
	do_exit(RC_OKAY);
}
...
append_default_options (int *argc, char *(*argv[]))
{
  char *compiler_defaults_path =
	 string_copy(getenv("ABSOFT_COMPILER_DEFAULTS_PATH"));
  ...
  if (compiler_defaults_path == NULL) {
    compiler_defaults_path = strdup("/opt/absoft/etc");
  }
...
{
  char * default_opt = string_copy(getenv("ABSOFT_GENFLAGS"));
  char * p, * q;
  char ** new_argv;
  int new_argc, fin = 0;
...
#if defined(ABSOFT_EXTENSIONS)
	asprintf(&apath, ":%s/shlib:%s/bin", root_prefix, root_prefix);
	add_string(libdirs, apath);
	free(apath);
#endif
