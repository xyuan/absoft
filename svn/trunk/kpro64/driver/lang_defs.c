...
static lang_info_t language_info[] = {
	...
	{'c',	0x00000002,	{"cc", PSC_NAME_PREFIX "cc" DEBUG_EXTENSION, PSC_NAME_PREFIX "cc", PSC_LONG_NAME_PREFIX "cc" DEBUG_EXTENSION, PSC_LONG_NAME_PREFIX "cc", PSC_TARGET "-" PSC_NAME_PREFIX "cc","gcc"}},	/* cc */
	{'C',	0x00000004,	{"CC", PSC_LONG_NAME_PREFIX "++" DEBUG_EXTENSION, PSC_LONG_NAME_PREFIX "++", PSC_NAME_PREFIX "++" DEBUG_EXTENSION, PSC_NAME_PREFIX "++", "g++"}},	/* c++ */
...
#define PHASEBINPATH	"/usr/lib/" PSC_NAME_PREFIX "cc-lib/bin"
...
static phase_info_t phase_info[] = {
   ...
   {'p',  0x0000000000000010LL,	"cpp",	PHASEBINPATH,	FALSE},	/* cpp */
   ...
   {'p',  0x0000000000000080LL,	"fec",	 PHASEBINPATH,	FALSE},	/* c_cpp */
   {'p',  0x0000000000000100LL, "cpp",   PHASEBINPATH,     FALSE}, /* cplus_cpp */
   {'p',  0x0000000000000200LL,	"mfef77",PHASEBINPATH,	FALSE},	/* f_cpp */
   {'p',  0x0000000000000400LL,	"ftpp"   ,PHASEBINPATH,	FALSE},	/* f90_cpp */
   /* place-holder for generic cpp, whose mask unites all cpp's; */
   {'p',  0x0000000000000ff0LL,	"",	"",		FALSE},	/* any_cpp */

   {'K',  0x0000000000001000LL,	"pfa",	PHASEBINPATH,	FALSE},	/* pfa */
   {'K',  0x0000000000002000LL,	"pca",	PHASEBINPATH,	FALSE},	/* pca */
   {'M',  0x0000000000008000LL, "mpc",  PHASEBINPATH,      FALSE}, /* mpc */
   ...
   {'f',  0x0000000000010000LL,	"mfef77",PHASEBINPATH,	FALSE},	/* f_fe */
   {'f',  0x0000000000020000LL, "mfef77",PHASEBINPATH,     FALSE}, /* cppf_fe */
   {'f',  0x0000000000040000LL,	"mfef95",PHASEBINPATH,	FALSE},	/* f90_fe */
   {'f',  0x0000000000080000LL,	"mfef95",PHASEBINPATH,	FALSE},	/* cppf90_fe */
   {'f',  0x0000000000100000LL,	"gfec" TOOL_EXTENSION,PHASEBINPATH,	TRUE }, /* c_gfe */
   {'f',  0x0000000000200000LL,	"gfecc" TOOL_EXTENSION,PHASEBINPATH,	TRUE }, /* cplus_gfe */
   ...
   {'X',  0x0000000002000000LL, "ftnlx", PHASEBINPATH,	FALSE}, /* Lister */ 

   {'i',  0x0000000010000000LL,	"inline" TOOL_EXTENSION,PHASEBINPATH,	TRUE},	/* inline */
   {'i',  0x0000000020000000LL,	"ipl" TOOL_EXTENSION,	PHASEBINPATH,	TRUE},	/* ipl */
   {'i',  0x00000000f0000000LL,	"",	"",		TRUE},	/* ipl, inline*/

   {'b',  0x0000000100000000LL,	"be" TOOL_EXTENSION,	PHASEBINPATH,	TRUE},	/* be */
   /* We use 'B' for options to be passed to be via ipacom. */

   {'a',  0x0000001000000000LL,	"asm",	PHASEBINPATH,	FALSE},	/* as */
   {'a',  0x0000002000000000LL,	NAMEPREFIX "gcc", BINPATH, FALSE}, /* gcc */
   {'a',  0x0000003000000000LL,	"",	"",		FALSE},	/* any_as */

   {'d',  0x0000008000000000LL, "dsm_prelink", PHASEBINPATH,FALSE},/* dsm_prelink*/
   {'j',  0x0000010000000000LL,	"ipa_link" TOOL_EXTENSION, PHASEBINPATH,TRUE},	/* ipa_link */
   {'l',  0x0000020000000000LL,	"collect2" TOOL_EXTENSION, PHASEBINPATH,TRUE},	/* collect */
...
/* return LD_LIBRARY_PATH for absoft, if needed */
char *
get_absoft_ld_library_path (phases_t index)
{
    char *ldir, *basedir = NULL;

    if (!phase_info[index].set_ld_library_path)
	return 0;
    ldir = drop_path (phase_info[index].dir);
    if (strcmp (ldir, "lib") == 0 || strcmp (ldir, "bin") == 0) {
	basedir = directory_path(phase_info[index].dir);
	basedir = concat_strings(basedir, "/shlib");
    }
    return basedir;
}
