...
{
	if ((report_file = getenv("ABSOFT_CRASH_REPORT")) != NULL)
		goto bail;

	if (asprintf(&report_file, "%s/absoft_crash_XXXXXX", tmpdir) == -1) {
		report_file = NULL;
		goto bail;
	}
	...
	setenv("ABSOFT_CRASH_REPORT", report_file, 1);
bail:
	return;
}
...
save_cpp_output (char *path)
{
	...
	if ((save_dir = getenv("ABSOFT_PROBLEM_REPORT_DIR")) == NULL &&
	    (home = getenv("HOME")) != NULL) {
		asprintf(&save_dir, "%s/.absoft-bugs", home);
	}
	...
	now = time(NULL);
	fprintf(ofp, "/*\n\nAbsoft compiler problem report - %s",
		ctime(&now));
	fprintf(ofp, "Please report this problem to <support@absoft.com>.\n");
