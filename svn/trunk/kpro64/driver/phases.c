...
set_library_paths(string_list_t *args)
{
	...
	char *our_path1; //, *our_path2;
	
	if (abi == ABI_N32) {
		if(shared == RELOCATABLE || shared == DSO_SHARED)
			asprintf(&our_path1, "%s/shlib", root_prefix);
		else
			asprintf(&our_path1, "%s/lib", root_prefix);
	} else {
		if(shared == RELOCATABLE || shared == DSO_SHARED)
			asprintf(&our_path1, "%s/shlib64", root_prefix);
		else
			asprintf(&our_path1, "%s/lib64", root_prefix);
	}
	
	//add_string(args, concat_strings("-L", our_path2));
	add_string(args, concat_strings("-L", our_path1));
...
add_file_args_first (string_list_t *args, phases_t index)
{
  switch (index) {
	...
	add_string(args, "-D_ABSOFT=\"" PSC_FULL_VERSION "\"");
	add_string(args, "-D__ACC__=" PSC_MAJOR_VERSION);
	add_string(args, "-D__ACC_MINOR__=" PSC_MINOR_VERSION);
	add_string(args, "-D__ACC_PATCHLEVEL__=" PSC_PATCH_LEVEL);
      }
  }
}
#endif
...
add_final_ld_args (string_list_t *args)
{
	...
	add_library(args, "af77math");
	/* We don't add acml_mv by default in case ACML not installed
	 * This should be done by -fast-math (which is NOT done yet)
	 * bug ALC-59
         */
#else
#ifdef TARG_X8664 
        extern boolean link_with_mathlib;
#if 1 // Bug 4813 - acml_mv is not in yet.
        if ((link_with_mathlib) && 
...
check_existence_of_phases (void)
{
	...
	case P_ipl:
	    if (!file_exists (concat_strings (directory_path(get_phase_dir(phase_order[i])),
					      "/shlib/ipl.so")))
		give_warning = TRUE;
	...
	case P_be:
	    ...
	    if (give_warning)
		warning ("%s does not contain all of the compiler"
...
run_ld (void)
{
	...
	if (ipa == TRUE) {
	    ...
	    ldpath = concat_strings (directory_path(ldpath), "/shlib/ipa.so");
