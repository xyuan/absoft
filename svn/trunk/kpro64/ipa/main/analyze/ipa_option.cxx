 ...
 * Module: ipa_option.c
 * $Revision: 2472 $
 * $Date: 2006-10-05 11:33:20 -0400 (Thu, 05 Oct 2006) $
 * $Author: yin $
 * $Source$
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source$ $Revision: 2472 $";
#endif /* _KEEP_RCS_ID */
...
Process_IPA_Options ( INT argc, char **argv )
{
    ...
    int IPA_Small_Callee_Limit_Set = 0;
#endif

    for (i = 0; i < argc; i++) {
	if (argv[i] != NULL && *(argv[i]) == '-' ) {
#ifdef ABSOFT_EXTENSIONS
	    if( strncmp( argv[i], "-IPA:callee_limit=", 18 ) == 0 ){
 		IPA_Small_Callee_Limit_Set = 1;
	    }
#endif
...
#ifdef ABSOFT_EXTENSIONS
    if( Is_Target_32bit() ){
	if( !IPA_Small_Callee_Limit_Set ){
	     IPA_Small_Callee_Limit = 100;
	}
    }
#endif
