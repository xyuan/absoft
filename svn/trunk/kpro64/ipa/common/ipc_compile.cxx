...
#ifdef TARG_X64
char makefile_shortname[256];           // the only file name
#endif
...
#ifdef TARG_X64
  strcpy(makefile_shortname, name_buffer );
#endif
...
#include "pathscale_defs.h"
  ...
  static char* tmp_cc_name_base = PSC_INSTALL_PREFIX "/bin/" PSC_NAME_PREFIX "f90_x";
#else
  static char* tmp_cc_name_base = PSC_INSTALL_PREFIX "/bin/" PSC_NAME_PREFIX "f90";
#endif
#else
...
#if defined(ABSOFT_EXTENSIONS)
		|| looks_like(my_cc, PSC_NAME_PREFIX "++")
		|| looks_like(my_cc, PSC_NAME_PREFIX "f95")
		|| looks_like(my_cc, PSC_NAME_PREFIX "f77")
		|| looks_like(my_cc, PSC_NAME_PREFIX "f2k3")
#if defined(Is_True_On)
		|| looks_like(my_cc, PSC_NAME_PREFIX "cc_x")
		|| looks_like(my_cc, PSC_NAME_PREFIX "++_x")
		|| looks_like(my_cc, PSC_NAME_PREFIX "f90_x")
		|| looks_like(my_cc, PSC_NAME_PREFIX "f95_x")
		|| looks_like(my_cc, PSC_NAME_PREFIX "f77_x")
		|| looks_like(my_cc, PSC_NAME_PREFIX "f2k3_x")
#endif
#endif  
	  ) {
...
#if defined(ABSOFT_EXTENSIONS) && defined(Is_True_On)
		|| looks_like(my_cc, "ipa_link_x")
#endif
	  ) {
...
#if defined(ABSOFT_EXTENSIONS)
#if defined(Is_True_On)
	if (ld_ipa_opt[LD_IPA_F77].flag == TRUE)
		  strcpy(++s, "bin/" PSC_NAME_PREFIX "f77_x");
	else if (ld_ipa_opt[LD_IPA_F90].flag == TRUE)
		  strcpy(++s, "bin/" PSC_NAME_PREFIX "f90_x");
	else if (ld_ipa_opt[LD_IPA_F95].flag == TRUE)
		  strcpy(++s, "bin/" PSC_NAME_PREFIX "f95_x");
	else if (ld_ipa_opt[LD_IPA_F2K3].flag == TRUE)
		  strcpy(++s, "bin/" PSC_NAME_PREFIX "f2k3_x");
	else
		  strcpy(++s, "bin/" PSC_NAME_PREFIX "cc_x");
#else
	if (ld_ipa_opt[LD_IPA_F77].flag == TRUE)
		  strcpy(++s, "bin/" PSC_NAME_PREFIX "f77");
	else if (ld_ipa_opt[LD_IPA_F90].flag == TRUE)
		  strcpy(++s, "bin/" PSC_NAME_PREFIX "f90");
	else if (ld_ipa_opt[LD_IPA_F95].flag == TRUE)
		  strcpy(++s, "bin/" PSC_NAME_PREFIX "f95");
	else if (ld_ipa_opt[LD_IPA_F2K3].flag == TRUE)
		  strcpy(++s, "bin/" PSC_NAME_PREFIX "f2k3");
	else
		  strcpy(++s, "bin/" PSC_NAME_PREFIX "cc");
#endif
#else		  
		  strcpy(++s, "bin/" PSC_NAME_PREFIX "cc");
#endif // ABSOSFT_EXTENSIONS
		  ...
		  if (file_exists (my_cc)) {
	       ...
	       if (ld_ipa_opt[LD_IPA_F77].flag == TRUE)
		  strcpy(filename, "bin/" PSC_NAME_PREFIX "f77");
	       else if (ld_ipa_opt[LD_IPA_F90].flag == TRUE)
		  strcpy(filename, "bin/" PSC_NAME_PREFIX "f90");
	       else if (ld_ipa_opt[LD_IPA_F95].flag == TRUE)
		  strcpy(filename, "bin/" PSC_NAME_PREFIX "f95");
	       else if (ld_ipa_opt[LD_IPA_F2K3].flag == TRUE)
		  strcpy(filename, "bin/" PSC_NAME_PREFIX "f2k3");
	       else
	 	  strcpy(filename, "bin/" PSC_NAME_PREFIX "cc"); 
	 	  
#if defined(Is_True_On)	       
	       strcat( filename, "_x" );
#endif	
	       
	       // In case, readlink failed
	       char *absoft_env = getenv("ABSOFT");
	       char absoft[MAXPATHLEN];
	       absoft[0] = '\0';
	       strcpy( absoft, absoft_env );
	       if( absoft != NULL ){
		       for( int i = 0; i < strlen( absoft) ; i ++ ){
			    if( absoft[i] == '\\' ) absoft[i] = '/';
		       }
		       // use cygwin path
		       if( absoft[1] == ':' && absoft[2] == '/' ){
			       // like c:/absoft -> /cygdrive/c/absoft
			       absoft[1] = absoft[0];
			       absoft[0] = '/';
			       strcpy( my_cc, "/cygdrive" );
			       strcat( my_cc, absoft );
			       strcat( my_cc, "/" );
			       strcat( my_cc, filename );
	       ...
	       cc_name_base = my_cc;	
  	       ...
  	       // For MacOsx, when readlink failed
	       char filename[MAXPATHLEN];
	       my_cc[0] = '\0';
	       filename[0] = '\0';
...
#ifdef TARG_X64
  char* temp_cc_name = concat_names( "'", (char*) (*command_map)["cc"] );
  (*command_map)["cc"] = concat_names( temp_cc_name, "'" );
#endif
...
#ifdef TARG_X64
  sprintf(buf, "%s%s -c %s %s -o %s -no-asmdebug -TENV:emit_global_data=%s %s",
#else
                            ...
                            const PU_Info* pu, UINT32 ProMP_id)
{
...
#ifdef ABSOFT_EXTENSIONS
    if( strncmp( *i, "-L", 2 ) == 0 && strchr(*i, ' ') != NULL && strchr(*i, '\"') == NULL ){
	cmdline_length += strlen(*i) + 3;
    }else{
  ...
  for (i = argv.begin(); i != argv.end(); ++i) {
#ifdef ABSOFT_EXTENSIONS
    if( strncmp( *i, "-L", 2 ) == 0 && strchr(*i, ' ') != NULL && strchr(*i, '\"') == NULL ) strcat(cmdline, "\"");
#endif
    strcat(cmdline, *i);
#ifdef ABSOFT_EXTENSIONS
    if( strncmp( *i, "-L", 2 ) == 0 && strchr(*i, ' ') != NULL && strchr(*i, '\"') == NULL ) strcat(cmdline, "\"");
...
void ipacom_doit (const char* ipaa_filename)
{
...
#ifdef TARG_X64
  if( outfilename != NULL ){
      for( int i = 0; i < strlen( outfilename ); i ++ ){
	      if( outfilename[i] == '\\' ) outfilename[i] = '/';
      }
  }
  if( tmpdir != NULL ){
      for( int i = 0; i < strlen( tmpdir ); i ++ ){
	      if( tmpdir[i] == '\\' ) tmpdir[i] = '/';
      }
  }
  if( makefile_name != NULL ){
      for( int i = 0; i < strlen( makefile_name ); i ++ ){
	      if( makefile_name[i] == '\\' ) makefile_name[i] = '/';
      }	  
  }
#endif
  ...
  if (IPA_Enable_final_link) {
...
#ifdef TARG_X64
    char link_cmdfile_shortname[256];
    strcpy( link_cmdfile_shortname, cmdfile_buf );
#endif
...
#ifdef TARG_X64
	strcat( link_optline, " -Wl,-rpath" );
#endif
	++i;
      }
#if defined(ABSOFT_EXTENSIONS)
      if (strcmp(*i, "-melf_i386") == 0) {
	  fputs("-m32\n", cmdfile);
#ifdef TARG_X64
	  strcat( link_optline, " -m32" );
#endif
	  continue;
      }
#endif
    ...
    if (IPA_Enable_Cord) {
      ...
      fprintf(cmdfile, " %s\n", cord_output_file_name);
#ifdef TARG_X64
      strcat( link_optline, " -T " );
      strcat( link_optline, cord_output_file_name );
#endif
    }
#endif
...
#ifdef TARG_X64
      fprintf(makefile, "\techo -n %s ' ' \n\tcat %s\n",
              link_line->front(), link_cmdfile_shortname);
#else
...
#ifdef TARG_X64
    fprintf(makefile, "%s" TARGET_DELIMITER "\n", "dummy");
#else
...
#ifdef TARG_X64
            fprintf(makefile, "\t%s -Wb,-OPT:procedure_reorder=on -fb_create=%s %s -Wb,-CG:enable_feedback=off\n\n",
                symtab_command_line, Feedback_Filename, symtab_extra_args);
#else
#ifdef ABSOFT_EXTENSIONS
...
#ifdef TARG_X64
      fprintf (makefile, "\t%s -Wb,-OPT:procedure_reorder=on -fb_opt=%s %s -Wb,-CG:enable_feedback=on \n\n",
	       symtab_command_line, 
...
#ifdef TARG_X64
            fprintf(makefile, "\t%s -Wb,-OPT:procedure_reorder=on %s -Wb,-CG:enable_feedback=off\n\n",
...
#ifdef TARG_X64
    fprintf(makefile, "%s" TARGET_DELIMITER "%s %s\n\n",
            elf_symtab_name,
            input_symtab_name,
            "dummy");
...
#ifdef TARG_X64
    fprintf(makefile, "%s" TARGET_DELIMITER "%s %s %s\n",
            (*outfiles)[i],
            elf_symtab_name,
            whirl_symtab_name,
            (*infiles)[i]);
...
#ifdef TARG_X64
        fprintf(makefile, "\t%s -Wb,-OPT:procedure_reorder=on -fb_create=%s %s -Wb,-CG:enable_feedback=off\n",
                (*commands)[i], Feedback_Filename, extra_args);
#else
#ifdef ABSOFT_EXTENSIONS
        fprintf(makefile, "\tcd %s; %s -Wb,-OPT:procedure_reorder=on -fb_create=%s %s -Wb,-CG:enable_feedback=off\n",
#else
...
#ifdef TARG_X64
  // To support DOS command
  fprintf(sh_cmdfile, "cd \"%s\"\n", tmpdir );
  fprintf(sh_cmdfile, "%s -f %s ", smake_name, makefile_shortname);
#else
  ...
  if( strncmp(cur_dir, "/cygdrive/", 10 ) == 0 ){
      // Convert back to the register path
      if( cur_dir[11] == '/' ){
          int length  = strlen( cur_dir );
          for( int j = 9; j <= length; j ++ ){
               cur_dir[j-9] = cur_dir[j];
          } 	
          cur_dir[0] = cur_dir[1];
          cur_dir[1] = ':';
       }
       for( int j = 0; j < strlen( cur_dir ); j ++ ){
	       if( cur_dir[j] == '/' ) cur_dir[j] = '\\';
       }
  }
  fprintf(sh_cmdfile, "\ncd \"%s\"", cur_dir );
#endif
...
static const char* get_extra_symtab_args(const ARGV& argv)
{
  ...
  for (ARGV::const_iterator i = argv.begin(); i != argv.end(); ++i) {
	...
	/* TODO: add additional options for symtab.I here as necessary */
