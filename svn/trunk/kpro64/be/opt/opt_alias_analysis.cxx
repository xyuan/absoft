...
// $Revision: 3014 $
// $Date: 2009-05-06 08:48:03 -0400 (Wed, 06 May 2009) $
// $Author: yin $
// $Source$
//
// ====================================================================
				  ...
				  BOOL is_mp_barrier)
{
  ...
  if (WOPT_Enable_Strong_MP_Barrier && is_mp_barrier ){
...
#ifdef ABSOFT_EXTENSIONS
    if( IS_FORTRAN && (idx == Return_vsym() || idx == Default_vsym() ) ){
         if ( DCE_Casgn_Remove && 
...
#ifdef ABSOFT_EXTENSIONS
      st = psym->St();
      if( DCE_Casgn_Remove >= 2 && st != NULL ){
          TY_IDX ty = ST_type(st);
          const char *name = TY_name_idx (ty) == 0 ? "(anon)" : TY_name (ty);
          if( strcmp( name, ".ch_str." ) == 0 ){
	  }else{
              mu->New_mu_node(idx, mem_pool);
	  }
      }else{
...
#ifdef ABSOFT_EXTENSIONS
extern BOOL Pragma_need_chi(const WN *wn ); // in opt_wn.cxx
extern BOOL Pragma_need_mu(const WN *wn );

void 
OPT_STAB::Generate_pragma_mu_chi( WN* wn)
{
  WN_PRAGMA_ID prag_id;   
  BOOL mu;
  BOOL chi;

  mu = Pragma_need_mu( wn );
  chi = Pragma_need_chi( wn );
   
  if( mu == FALSE && chi == FALSE ) return;

  ST* st = WN_st( wn );
  if( st == NULL ) return;

  // Create OCC, must have 
  AUX_ID st_idx = -1;
  OCC_TAB_ENTRY *occ;
  ...
  if( occ == NULL ){
     // Need for an occ 
     occ = Enter_occ_tab( wn, 0 );
  }

  // Look for the right id
  // Question: Should we convert st* in pragma into aux form in Convert_ST_to_AUX?
  // Answer: I don't know so far -- Yin 
  AUX_ID idx;
  AUX_STAB_ITER aux_stab_iter(this);
  FOR_ALL_NODE(idx, aux_stab_iter, Init()) {
	AUX_STAB_ENTRY *psym = Aux_stab_entry(idx);
  ...
  if( st_idx == -1 ) return; // no use or something wrong 

  // mark it
  if( mu ) Set_wn_flags( wn, Wn_flags(wn) | WN_FLAG_PRAGMA_HAS_MU );
  if( chi ) Set_wn_flags( wn, Wn_flags(wn) | WN_FLAG_PRAGMA_HAS_CHI );  

  // Add mu chi 
  Add_to_mu_chi( occ, st_idx, mem_pool, FALSE , mu, chi );
