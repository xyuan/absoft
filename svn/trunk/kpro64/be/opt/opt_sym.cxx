...
// Module: opt_sym.cxx
// $Revision: 3026 $
// $Date: 2009-05-20 11:59:31 -0400 (Wed, 20 May 2009) $
// $Author: yin $
...
#ifndef ABSOFT_EXTENSIONS
  // Why enforce to keep all store? 
  // I believe that it is good to turn on WOPT_Enable_Strong_Barrier
#ifdef KEY // bug 5401 and 5267
...
void
OPT_STAB::Mark_ST_for_Parrallel_Region( WN* block, BOOL unmark )
{
   Is_True( WN_opcode( block) == OPC_BLOCK, ( "Wn is not a block" ) );

   WN* pragma;
   for( pragma = WN_first( block); pragma != NULL; pragma = WN_next( pragma) ){
     if( OPCODE_operator(WN_opcode(pragma)) == OPR_PRAGMA ){ //in case
	WN_PRAGMA_ID prag_id = (WN_PRAGMA_ID) WN_pragma(pragma);
	switch (prag_id ) {
...
//	   case WN_PRAGMA_THREADPRIVATE:
	   {
		ST* st = WN_st( pragma );
		// Should we ignore compiler generated temp? 
		if( st != NULL && ST_sclass(st) != SCLASS_REG ){
		  if( unmark == TRUE ) Clear_ST_is_been_localized(st);
		  if( unmark == FALSE ) Set_ST_is_been_localized(st);
		}
	   }
	   break;
	};
...
#ifdef TARG_X64
    PREG_NUM retreg[MAX_NUMBER_OF_REGISTERS_FOR_RETURN+1];
    TYPE_ID	ty[MAX_NUMBER_OF_REGISTERS_FOR_RETURN+1];
#else
...
#ifdef ABSOFT_EXTENSIONS
        // Mark private ST
	WN* pragma_block = WN_region_pragmas( wn );
	Mark_ST_for_Parrallel_Region( pragma_block, FALSE );
#endif
      }
...
AUX_STAB_ENTRY::Equivalent(AUX_STAB_ENTRY *sym)
{
  if (this->Base() == sym->Base() &&
	...
	if( WOPT_Enable_Loose_MP_Equivalent ) return TRUE;

	// need to see if it is both tread_localized 
  	// TODO: because Canonicalize process is out of Convert_ST_to_AUX process
	// been localized should be only applied into a region instead of whole process
	ST* sym_st = sym->St();
	if( st != NULL && sym_st != NULL && st != sym_st ){
	  if( ST_is_been_localized( st ) ) return FALSE;
	  if( ST_is_been_localized( sym_st ) ) return FALSE;
	  return TRUE;
	}else return TRUE;	
      }
#else
