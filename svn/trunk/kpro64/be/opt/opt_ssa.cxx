...
// Module: opt_ssa.cxx
// $Revision: 2912 $
// $Date: 2008-10-03 10:35:36 -0400 (Fri, 03 Oct 2008) $
// $Author: yin $
...
static char *rcs_id = 	opt_ssa_CXX"$Revision: 2912 $";
#endif /* _KEEP_RCS_ID */
...
SSA::Find_zero_versions(void)
{
  ...
  FOR_ALL_NODE(vid, ver_stab_iter, Init()) {
    ...
    if (Opt_stab()->Special_vsym(vse->Aux_id()))
      continue;
#ifdef KEY // because of bug 7847, do not do zero version for scalar variables
    if (Opt_stab()->Is_real_var(vse->Aux_id()))
      continue;
#endif
