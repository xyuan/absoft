...
// Module: opt_dce.cxx
// $Revision: 2924 $
// $Date: 2008-10-10 14:16:31 -0400 (Fri, 10 Oct 2008) $
// $Author: yin $
...
DCE::Required_call( const STMTREP *stmt, OPERATOR oper ) const
{
  ...
  if( oper == OPR_INTRINSIC_CALL ){
      ...
      if( DCE_Casgn_Remove && rhs->Intrinsic() == INTRN_CASSIGNSTMT ) return ( FALSE );
  }
#endif
...
COMP_UNIT::Find_uninit_locals_for_entry(BB_NODE *bb) 
{
  ...
  FOR_ALL_NODE(cnode, chi_iter, Init(bb->First_stmtrep()->Chi_list())) {
	...
	size_t length = 1023;
	int status;
	p = __cxa_demangle( output_pu_name, p, &length, &status );
#else	      
	...
	size_t length = 1023;
	int status;
	v = __cxa_demangle( output_pu_name, v, &length, &status );	
#else	      
