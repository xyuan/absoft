...
#ifdef ABSOFT_EXTENSIONS
	  // [YIN2005-10-21] [JIRA BE-64]
	    CODEREP *cr_vsym = cr->Get_ivar_vsym();
	    if( vsym != NULL && cr_vsym != NULL ){
		if( vsym->Aux_id() != cr_vsym->Aux_id() ){
		 	// Cannot be the same
			return FALSE;
		}	        
	    }
...
#ifdef ABSOFT_EXTENSIONS
            if (retv->Opr() == OPR_SHUFFLE) cr->Set_offset(retv->Offset());
	    if (retv->Opr() == OPR_DOT) cr->Set_offset(retv->Offset());
#endif
...
extern BOOL Pragma_need_chi(const WN *wn );
#endif
...
#ifdef ABSOFT_EXTENSIONS
  else if( _opr == OPR_PRAGMA ){
     return  Pragma_need_mu( Orig_wn() ); // no need to call Wn_flag
  }
#endif
...
STMTREP::Has_chi(void) const
{
  if (_opr == OPR_REGION) { // need to check what kind of region
	...
	return FALSE;
     ...
     return  Pragma_need_chi( Orig_wn() ); // no need to call Wn_flag
  }
#endif
  ...
  case OPR_ISTOREX:
	...
	&proped, copyprop);
      ...
      Set_lhs( htable->Add_idef(opc, opt_stab->Get_occ(Wn()), 
				...
				WN_field_id(Wn()),
				(mINT32)(INTPTR)index,
				(CODEREP *) ilod_base_ty, NULL, base) );
      if (Lhs()->Is_ivar_volatile())
	Set_volatile_stmt();
    }
    return;
