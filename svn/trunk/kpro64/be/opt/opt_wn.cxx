...
static char *rcs_id = 	opt_wn_CXX"$Revision: 3026 $";
#endif /* _KEEP_RCS_ID */
...
BOOL Pragma_need_chi(const WN *wn )
{
  WN_PRAGMA_ID prag_id;   

  if( WOPT_Enable_MPPragma_Need_CHI == FALSE ) return FALSE;

  switch (prag_id = (WN_PRAGMA_ID) WN_pragma(wn)) {
...
BOOL Pragma_need_mu(const WN *wn )
{
  WN_PRAGMA_ID prag_id;   

  if( WOPT_Enable_MPPragma_Need_MU == FALSE ) return FALSE;
...
#ifdef ABSOFT_EXTENSIONS
  else if( opc == OPC_PRAGMA ){
       if(Pragma_need_chi(wn)){
	   if( Wn_flags(wn) & WN_FLAG_PRAGMA_HAS_CHI ) return TRUE;
	   else return FALSE;
       }else return FALSE;
  }
#endif
...
BOOL WN_has_mu( const WN *wn, const REGION_LEVEL region_level )
{
  ...
  switch ( OPCODE_operator(opc) ) {
    ...
    case OPR_PRAGMA:
    {
       if(Pragma_need_mu(wn)){
	   if( Wn_flags(wn) & WN_FLAG_PRAGMA_HAS_MU ) return TRUE;
	   else return FALSE;
       }else return FALSE;
    }
#endif
