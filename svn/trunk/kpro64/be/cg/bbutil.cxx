...
void dump_bb_by_id(mBB_NUM id)
{
   BB *bp;	
   for (bp = REGION_First_BB; bp; bp = BB_next(bp)) {
     if( BB_id(bp) == id ){ 
	 printf( "BB[ %d ]Memory Address : %x\n", BB_id(bp), bp );
         Print_BB ( bp );
         fprintf ( TFile,"\n" );
         return;
     }
...
void dump_bb_by_idrange(mBB_NUM id1, mBB_NUM id2)
{
   mBB_NUM temp;
   if( id1 > id2 ){
	   temp = id2;
 ...
 * Set_Label_For_BB
 *
 * attach the new label to the BB, and return it.  
 * If there already is one, replace the old one
 *
 * ====================================================================
 */

void
Set_Label_For_BB(LABEL_IDX lab, BB *bb)
{
#define  EXTRA_NAME_LEN  32
