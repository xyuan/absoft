...
Generate_Entry (BB *bb, BOOL gra_run )
{
  ...
  if( Is_Target_32bit() && Gen_PIC_Shared ){
      if( bb == REGION_First_BB ){
          EETARG_Generate_MACOSX32_GOT_Entry_Code( bb, &ops );
      }else{
      	  EETARG_Generate_MACOSX32_GOT_Altentry_Code( bb, &ops );
      }
  }  
...
#ifdef TARG_X64
  if( Is_Target_32bit() &&
      call_st != NULL && // ICALL with call_st == NULL
      ST_is_STDCALL( call_st ) ){
    OPS ops = OPS_EMPTY;
    Exp_SUB( Pointer_Mtype, SP_TN, SP_TN, Gen_Literal_TN( Get_PU_arg_area_size( ST_pu_type(call_st) ),4), &ops );
...
#ifdef TARG_X64
    if( Is_Target_32bit() && ST_is_STDCALL(st)  ){
    	sp_adjust = Get_PU_arg_area_size( ST_pu_type (st) );
    }
#else
    if( Is_Target_32bit() ){
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
static void 
Init_MACOSX_Style_GOT_Entry_Data()
{
   // Generate Label for the PIC base
   char* buf;
   LABEL* label;
   buf = (char*) alloca( strlen(Cur_PU_Name) + 16 );
   sprintf( buf, ".LBB_%s_%s", Cur_PU_Name, "Pic_Base" );  
   label = &New_LABEL(CURRENT_SYMTAB, PU_PIC_Base_Label_Idx );
...
Generate_Entry_Exit_Code ( ST *pu, BOOL is_region )
{
  ...
  for ( elist = Entry_BB_Head; elist; elist = BB_LIST_rest(elist) ) {
    if( BB_LIST_first(elist) == REGION_First_BB )
...
Adjust_Entry(BB *bb)
{
  ...
  if (frame_len == 0) {
    ...
    if( EMIT_stack_probe ){
      if( frame_len > CG_memory_page_size ){
	  Exp_Chkstk( frame_len, NULL, &ops, TRUE );
      }
    } 
#endif 
