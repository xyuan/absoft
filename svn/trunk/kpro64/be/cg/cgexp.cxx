...
BOOL x87_Control_Fixed = FALSE;      /* the optimization level of expand.cxx and exp.cxx */
#endif
/* ====================================================================
 *
 * Expand_OP
...
Expand_OP (OPCODE opcode, TN *result, TN *op1, TN *op2, TN *op3, VARIANT variant, OPS *ops)
{
  ...
  case OPR_MOD:
#ifdef ABSOFT_EXTENSIONS
	if (Is_Target_32bit() && MTYPE_is_float(rtype) && (rtype == MTYPE_F4 || rtype == MTYPE_F8 ) ){
	   	Expand_Mod_Float( result, op1, op2, rtype, ops );
	}else
#endif
  ...
  case OPR_LT:
	if (MTYPE_is_float(desc))
		...
		Expand_Float_Vector_Compares (opcode, result, op1, op2, desc, ops );
	    }else
#endif
...
#ifdef ABSOFT_EXTENSIONS
	else if(Is_Target_SSE41() && MTYPE_is_vector(rtype) && MTYPE_is_vector(desc) ){
		Expand_Convert_Vector_Length(result, op1, rtype, desc, ops);
        }
#endif
#ifdef TARG_X8664
