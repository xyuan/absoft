...
static BOOL  CG_bss_section_emitted = TRUE;
static BOOL  CG_bss_section_gen_symindex = TRUE;
static INT64 CGEMIT_Get_Next_BSS_Section_Id(){
      CG_bss_section_id ++;
      CG_bss_section_gen_symindex = TRUE;
      return CG_bss_section_id;
}

INT64 CGEMIT_Get_This_BSS_Section_Id(){
      return CG_bss_section_id;
}

static void CGEMIT_Set_This_BSS_Symindex_Gen(){
	CG_bss_section_gen_symindex = FALSE;
}

static BOOL CGEMIT_Is_This_BSS_Symindex_Need(){
	return CG_bss_section_gen_symindex;
}
#endif
#ifdef TARG_IA64
 ...
 */
#ifdef ABSOFT_EXTENSIONS
UINT64 PC = 0;			/* PC for current text section */
#else
...
Init_Section (ST *st)
{
	...
	if( strcmp(ST_name(st), ".bss") == 0 && CG_bss_section_emitted == FALSE){
	    UINT32 tmp, power;
	    power = 0;
 ...
 * and if it overflows, the bundle address is adjusted as well.
 */
UINT64 PCs_Incr(UINT64 pc, UINT32 n)
{
  UINT32 i;
 ...
 */
UINT64 PC_Incr_N(UINT64 pc, UINT32 incr)
{
...
EMT_Write_Qualified_Name (FILE *f, ST *st)
{
  if (ST_name(st) && *(ST_name(st)) != '\0') {
	    ...
	    /* Add Leading Underscore, 32bit Windows ABI, STDCALL/CDECL*/ 
	    fputs( "_", f );    
	}
#endif  
#ifdef TARG_X64
...
Print_Common (FILE *pfile, ST *st)
{
  ...
  if(ST_is_Global(base_st))
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
  {
     if( Is_Target_32bit() ){
	    fprintf ( pfile, "\t.non_lazy_symbol_pointer\n" );
	    fprintf ( pfile, "L%s$non_lazy_ptr: \n", ST_name(st) );
	    fprintf ( pfile, "\t.indirect_symbol %s\n",ST_name(st) );
	    fprintf ( pfile, "\t.long 0\n" );
	    fprintf ( pfile, "\t.subsections_via_symbols\n" );
     }
#endif
    return;
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
...
#define ROUNDUP(val,align) 	( (-(INT64)align) & (INT64)(val+align-1) )
    if (ST_is_initialized_in_file(st)){
        
    }else{
          fprintf ( pfile, "\t%s\t", AS_COM);
          EMT_Write_Qualified_Name(pfile, st);
          fprintf ( pfile, ", %lld\n", ROUNDUP( TY_size(ST_type(st)), 16) );
          if( Is_Target_32bit() ){
	    fprintf ( pfile, "\t.non_lazy_symbol_pointer\n" );
	    fprintf ( pfile, "L" );
	    EMT_Write_Qualified_Name(pfile, st);
	    fprintf ( pfile, "$non_lazy_ptr: \n" );
	    fprintf ( pfile, "\t.indirect_symbol " );
	    EMT_Write_Qualified_Name(pfile, st);
	    fprintf ( pfile, "\n" );
	    fprintf ( pfile, "\t.long 0\n" );
	    fprintf ( pfile, "\t.subsections_via_symbols\n" );
...
#ifdef ABSOFT_EXTENSIONS
   if( LNO_Run_Simd && TY_align(ST_type(st)) < 16 ){
      // 32bit IPA with Vectorization may need this
      fprintf ( pfile, ", %lld, 16\n", TY_size(ST_type(st)));
   }else if( CG_force_common_16bit_aligned ){
      fprintf ( pfile, ", %lld, 16\n", TY_size(ST_type(st)));
   }else 
#endif
      fprintf ( pfile, ", %lld, %d\n",
    ...
    EMT_Write_Qualified_Name(pfile, st);
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
    // On Leopard Beta, This has to been an aligned value
    fputs (", 16\n", pfile);
#else
    fputs (", 1, 1\n", pfile);
#endif
...
/* ====================================================================
 *
 * EMT_Put_Elf_Symbol_BSS_Section
 *
 * Add a symbol to the BSS Section ELF symbol table
 * ====================================================================
 */
mINT32 EMT_Put_Elf_Symbol_BSS_Section (ST *sym )
{
  unsigned char symother;
  Elf64_Word symindex;
  char thisbss_name[256];
  
  if( CGEMIT_Is_This_BSS_Symindex_Need() == false ) return ST_elf_index(sym);
  ...
  symindex = Em_Add_New_Undef_Symbol (	thisbss_name, STB_LOCAL, STT_OBJECT, symother);
...
#ifdef TARG_X64
  ST *common_base_st;
  INT64 common_base_ofst = 0;
#endif
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
  ST *common_base_st;
  INT64 common_base_ofst = 0;
  
  if( strcmp( ST_name(sym), ".bss" ) == 0 ) return EMT_Put_Elf_Symbol_BSS_Section ( sym );
#endif
  ...
  switch (ST_class(sym)) {
	...
	case SCLASS_COMMON:
	  if (sym != base_st && ST_sclass(base_st) == SCLASS_COMMON) {
		// use base common
#ifdef TARG_X64
                Base_Symbol_And_Offset (base_st, &common_base_st, &common_base_ofst);
                if( base_st == common_base_st ) return ST_elf_index( base_st );
#endif
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
                Base_Symbol_And_Offset (base_st, &common_base_st, &common_base_ofst);
                if( base_st == common_base_st ) return ST_elf_index( base_st );
...
#endif
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
            if( PU_has_SpecialABI(Pu_Table[ST_pu(sym)]) ){
	        // for mcount
		fprintf(Asm_File, "\t%s\t%s\n", AS_GLOBAL, ST_name(sym));
	    }else{
	        fprintf(Asm_File, "\t%s\t_%s\n", AS_GLOBAL, ST_name(sym));
  ...
  vstring *buf)		/* A buffer to format it into */
{
  ...
  if ( TN_is_symbol(t) ) {
	...
	add_name = TRUE;
#ifdef ABSOFT_EXTENSIONS
        if( TN_relocs(t) == TN_RELOC_MACOSX32_GOT ){
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
        if( ST_sclass(st) == SCLASS_COMMON && !ST_is_initialized_in_file(st) && Is_Target_32bit() ){
	        *buf = vstr_concat( *buf, "L" );
	}
	if( strcmp( ST_name(st), ".text" ) == 0 ||
	    strcmp( ST_name(st), ".data" ) == 0 ||
	    strcmp( ST_name(st), ".rodata" ) == 0 ) {
	      *buf = vstr_concat (*buf, "L" );
	}
        if( ST_class(st) == CLASS_FUNC ){
	    /* Add Leading Underscore, MACOSX ABI, STDCALL/CDECL*/ 
	    if( PU_has_SpecialABI(Pu_Table[ST_pu(st)]) ){
	         // no leading underscore
	    }else{
	   	 *buf = vstr_concat( *buf, "_" );    
	    }
	}
#endif
		...
		if( Get_PU_arg_area_size( call_ty ) == 0 ){
			...
			if( Tylist_Table[idx] != 0 ){
			  ...
			  while (Tylist_Table[idx] != 0 ) {
				ploc = Get_Output_Parameter_Location( Tylist_Table[idx] ); 
				++idx;
			  }  
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
        if( ST_sclass(st) == SCLASS_COMMON && !ST_is_initialized_in_file(st) && Is_Target_32bit() ){
		*buf = vstr_concat( *buf, "$non_lazy_ptr");
	}
#endif
    }
  }
  ...
  BOOL is_result)
{
  ...
  if (ISA_OPERAND_VALTYP_Is_Register(vtype)) {
	...
	if (val == nextpc) {
#ifdef ABSOFT_EXTENSIONS
	  DevWarn("branch to next instruction at PC=0x%llx", PC);
#else
	...
	if (!okay) {
#ifdef ABSOFT_EXTENSIONS
	  DevWarn("branch to 0? (possible bug at PC=0x%llx, label %s)", 
		   PC, LABEL_name(lab));
#else
...
#ifdef ABSOFT_EXTENSIONS
    // [YIN2006-02-15] [JIRA BE-130]
    // For Expand_Ordered_Branch
    if( prev == NULL ){
	BB* cur_bb = OP_bb( op );
        BOOL bFound;
        if( cur_bb != NULL ){
           while( BB_preds_len( cur_bb ) == 1 ){
		BB* prev_bb = BB_First_Pred( cur_bb );               
		prev = BB_last_op( prev_bb );
                bFound = false;
		while( prev != NULL ){
		    if( TOP_is_change_rflags( OP_code(prev) ) ){
                        bFound = true;
...
Perform_Sanity_Checks_For_OP (OP *op, BOOL check_def)
{
  ...
  if (((tn = CGTARG_Copy_Operand_TN(op)) != NULL) &&
...
#ifdef ABSOFT_EXTENSIONS
      DevWarn("Redundant Copy instruction in %sBB:%d (PC=0x%llx)",
#else
    ...
    {
#ifdef ABSOFT_EXTENSIONS
      DevWarn ("Redundant XOR instruction in %sBB:%d (PC=0x%llx)",
#else
...
#ifdef ABSOFT_EXTENSIONS  
        DevWarn ("Redundant shift instruction in %sBB:%d (PC=0x%llx)",    
#else
...
#ifdef ABSOFT_EXTENSIONS  
    DevWarn("Noop not removed in BB:%d (PC=0x%llx)", BB_id(OP_bb(op)), PC);
#else
...
static INT32 num_pus = 100; /*pathscale initialized as 64*/
#else
//********************************************************
...
static char* int_reg_names[5][16] = {
  /* BYTE_REG: low 8-bit */
  { "%al", "%bl", "%bpl", "%spl", "%dil", "%sil", "%cl", "%dl",
    "%r8b",  "%r9b",  "%r10b", "%r11b", "%r12b", "%r13b", "%r14b", "%r15b" },
  /* BYTE_REG: high 8-bit */
  { "%ah", "%bh", "%bph", "%sph", "%dih", "%sih", "%ch", "%dh",
    /* The following is a filler. There is no "high"  for these registers */
    "%r8b",  "%r9b",  "%r10b", "%r11b", "%r12b", "%r13b", "%r14b", "%r15b" },
  /* WORD_REG: 16-bit */
  { "%ax", "%bx", "%bp", "%sp", "%di", "%si", "%cx", "%dx",
    "%r8w",  "%r9w",  "%r10w", "%r11w", "%r12w", "%r13w", "%r14w", "%r15w" },
  /* DWORD_REG: 32-bit */
  { "%eax", "%ebx", "%ebp", "%esp", "%edi", "%esi", "%ecx", "%edx",
    "%r8d",  "%r9d",  "%r10d", "%r11d", "%r12d", "%r13d", "%r14d", "%r15d" },
  /* QWORD_REG: 64-bit */
  { "%rax", "%rbx", "%rbp", "%rsp", "%rdi", "%rsi", "%rcx", "%rdx",
...
#endif
#ifdef ABSOFT_EXTENSIONS
	DevWarn ("label %s offset %lld doesn't match PC %lld",
#else
  ...
  Elf64_Xword scn_ofst)	/* Section offset to emit it at */
{
  ...
  if ( Trace_Init ) {
    ...
    if (Assembly) {
        ...
        if (name != NULL && *name != 0) {
#ifdef ABSOFT_EXTENSIONS
           if( CG_force_common_16bit_aligned && ST_sclass(sym) == SCLASS_DGLOBAL ){
#ifdef __APPLE__
               fprintf ( Asm_File, "\t%s\t4\n", AS_ALIGN  );
#else
               fprintf ( Asm_File, "\t%s\t16\n", AS_ALIGN  );
#endif
          }else{
               ...
               UINT32 tmp, power;
	       power = 0;
	       for (tmp = TY_align( ST_type ( sym ) ); tmp > 1; tmp >>= 1) power++;
	       fprintf ( Asm_File, "\t%s\t%d\n", AS_ALIGN, power );
#else
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
			if ( strcmp( ST_name(base), ".text" ) == 0 ) {
				/* __TEXT, __text */
				fprintf ( Asm_File, "\n\t%s %s\n", AS_SECTION, AS_TEXT_SECTION );
			} else if ( strcmp( ST_name(base), ".rodata" ) == 0 ) {
				fprintf ( Asm_File, "\n\t%s __TEXT, __const\n", AS_SECTION);
			} else if ( strcmp( ST_name(base), ".data") == 0) {
				fprintf ( Asm_File, "\n\t%s %s\n", AS_SECTION, AS_DATA_SECTION );
			} else if ( strcmp( ST_name(base), ".bss") == 0) {
			     if( Emit_Global_Data ){
			        // IPA
  			        UINT32 tmp, power;
	                        power = 0;
 	                        for (tmp = STB_align(base); tmp > 1; tmp >>= 1) power++;
			        fprintf ( Asm_File, "\n\t.lcomm\t.bss_symbol ,%lld, %d\n", ST_size(base), power ); 
			     }else{
				fprintf ( Asm_File, "\n\t%s %s __bss\n", AS_ZEROFILL, AS_DATA_SEG);
			     }
			} else if ( strncmp( ST_name(base), ".text.", 6 ) == 0){
				fprintf ( Asm_File, "\n\t%s %s\n", AS_SECTION, AS_TEXT_SECTION );
			} else {
				fprintf ( Asm_File, "\n\t%s %s%s\n", AS_SECTION, AS_TEXT_SEG,ST_name(base));
			}
#else
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
	    if (strcmp(ST_name(base), ".data") == 0)
		fprintf( Asm_File, "\n\t%s%s\n", AS_DATA_SECTION, AS_DATA_SECT);
	    else if (strcmp(ST_name(base), ".rodata") == 0)
		fprintf( Asm_File, "\n\t%s __TEXT, __const\n", AS_SECTION );
#else
	    fprintf ( Asm_File, "\n\t%s %s\n", 
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
	  if (!has_named_section && strcmp(ST_name(base), ".bss") != 0) {	
#else
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
	if (Assembly && strcmp(ST_name(base), ".bss") != 0 ) {
#else
	if (Assembly) {
#endif
...
Setup_Text_Section_For_BB (BB *bb)
{
  ...
  if (cur_section != PU_base) {
    if (Assembly) {
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
      if (strcmp(ST_name(PU_base), ".text"))
	fprintf (Asm_File, "\n\t%s %s\n", AS_SECTION, AS_TEXT_SECTION);
      else {
	DevWarn("Verify .section __TEXT,%s is valid for Setup_Text_Section_For_BB ", ST_name(PU_base));
      fprintf (Asm_File, "\n\t%s %s%s\n", AS_SECTION, AS_TEXT_SEG, ST_name(PU_base));
	}
#else
...
Setup_Text_Section_For_PU (ST *pu)
{
  ...
  if (Assembly){
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
   if (strcmp(ST_name(text_base),".text") == 0 ){
	fprintf(Asm_File, "\t%s %s\n", AS_SECTION, AS_TEXT_SECTION);
   }else if (strcmp(ST_name(text_base),".data") == 0){
	fprintf(Asm_File, "\t%s %s\n", AS_SECTION, AS_DATA_SECTION);
   }else if (strcmp(ST_name(text_base), ".rodata") == 0){
	fprintf(Asm_File, "\t%s __TEXT, __const\n", AS_SECTION );
   }else if (strcmp(ST_name(text_base), ".bss") == 0){
	fprintf(Asm_File, "\t%s %s__bss\n", AS_ZEROFILL, AS_DATA_SEG);
   }else if (strncmp(ST_name(text_base), ".text.", 6 ) == 0){
	fprintf(Asm_File, "\t%s %s\n", AS_SECTION, AS_TEXT_SECTION);
   }else{
   	fprintf(Asm_File, "\t%s __TEXT, %s\n", AS_SECTION, ST_name(text_base) );
   }
#else
#ifdef TARG_X64
   if (strncmp( ST_name(text_base), ".text.", 6 ) == 0 ){
...
#ifdef KEY
#if defined(TARG_X8664) && !defined(__APPLE__)
      fprintf (Asm_File, "\t%s\t%d\n", AS_ALIGN, 1 << power );
#else
      fprintf (Asm_File, "\t%s\t%d\n", AS_ALIGN, power);
#endif
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
	if (strcmp(ST_name(PU_base), ".text") == 0)
		fprintf(Asm_File, "\n\t%s %s\n", AS_SECTION, AS_TEXT_SECTION);
	else {
		DevWarn("Verify .section __TEXT,%s is valid for PU_base EMT_Emit_PU", ST_name(PU_base));
    		fprintf (Asm_File, "\n\t%s %s\n", AS_TEXT_SECTION, ST_name(PU_base));
	}
#else
    	fprintf (Asm_File, "\n\t%s %s\n", AS_SECTION, ST_name(PU_base));
#endif
...
#endif
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
      if (Assembly) fprintf (Asm_File, "\t%s\t _%s\n", AS_GLOBAL, ST_name(sym));
#else
...
// it at the start of the 1st BB in the PU.
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
  Elf64_Word scn_index = 0;
  if (generate_dwarf) {
      scn_index =  Em_Create_Section_Symbol(PU_section);
      Cg_Dwarf_Set_Section_Index( scn_index );
  }
#endif
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
  if (generate_dwarf) {
      Em_Dwarf_Last_Label_Line( Cg_Dwarf_Symtab_Entry(CGD_LABIDX, Last_Label, scn_index ) );
  }  
#endif
  ...
  if( Is_Target_32bit() && ST_class(pu) == CLASS_FUNC ){
      if( ST_is_STDCALL(pu) ){
          ...
          fprintf( Asm_File, "\t.size _%s, %s-_%s\n", 
	   ST_name(pu), LABEL_name(Last_Label), ST_name(pu));	      
      }
  }else 
#endif
#if defined(__APPLE__) && defined(ABSOFT_EXTENSIONS)
...
EMT_End_File( void )
{
	...
	FOREACH_SYMBOL (GLOBAL_SYMTAB, sym, i) {
		...
		if (Assembly) {
...
#endif
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
			if( strcmp( newname, ".rodata_symbol" ) == 0 || 
			    strcmp( newname, ".data_symbol" ) == 0   ||
			    strcmp( newname, ".bss_symbol" ) == 0 ||
			    strcmp( newname, ".text_symbol" ) == 0 ){
			fprintf (Asm_File, "\t%s\t%s\n", AS_GLOBAL, newname);
			}else{
			    fprintf (Asm_File, "\t%s\t_%s\n", AS_GLOBAL, newname);
			}
			...
			if( strcmp( newname, ".rodata_symbol" ) == 0 || 
			    ...
			    strcmp( newname, ".text_symbol" ) == 0 ){
			    fprintf (Asm_File, "%s:\n", newname);
			}else if( strcmp( newname, ".bss_symbol" ) == 0 ){
			    // Do nothing
			}else{
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
	        fprintf (Asm_File, "\t%s\t _%s\n", AS_GLOBAL, ST_name(sym));
		/* Only use .globl is not enough for linking an initialized data in a library for 64bit OSX 
                   output a explicit use is required */
                fprintf (Asm_File, "\t%s\t __DATA, __MODDATALINK\n", AS_SECTION );
		if( Is_Target_32bit() ){
		    fprintf (Asm_File, "\t%s\t _%s\n", AS_WORD, ST_name(sym));
		}else{
                    fprintf (Asm_File, "\t%s\t _%s@GOTPCREL\n", AS_WORD, ST_name(sym));        
		}
#else
		fprintf (Asm_File, "\t%s\t %s\n", AS_GLOBAL, ST_name(sym));
#endif
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
			if (strcmp( ST_name(sym), ".text" ) == 0 ) {
				/* __TEXT, __text */
				fprintf ( Asm_File, "\n\t%s %s\n", AS_SECTION, AS_TEXT_SECTION ); 
                        } else if ( strcmp( ST_name(sym), ".rodata" ) == 0) {
				fprintf ( Asm_File, "\n\t%s __TEXT, __const\n", AS_SECTION );
			} else if ( strcmp( ST_name(sym), ".data" ) == 0) {
				fprintf ( Asm_File, "\n\t%s %s\n", AS_SECTION, AS_DATA_SECTION );
			} else if ( strcmp( ST_name(sym), ".bss" ) == 0) {
				fprintf ( Asm_File, "\n\t%s %s__bss\n", AS_ZEROFILL, AS_DATA_SEG);
			} else if ( strncmp(ST_name(sym), ".text.", 6 ) == 0){
			        fprintf ( Asm_File, "\n\t%s %s\n", AS_SECTION, AS_TEXT_SECTION ); 
			} else{
				fprintf ( Asm_File, "\n\t%s %s%s\n", AS_SECTION, AS_TEXT_SEG, ST_name(sym));
			}
#else
#ifdef TARG_X64
        if (strncmp( ST_name(sym), ".text.", 6 ) == 0 ){
            continue;
        }else
#endif
	...
	FOREACH_SYMBOL (GLOBAL_SYMTAB, sym, i) {
 	  if (ST_class(sym) == CLASS_FUNC && ST_sclass(sym) != SCLASS_EXTERN ){
	      ...
	      if( Is_Target_32bit() ){
		      ...
		      /* cdel */
		      fprintf( Asm_File, "\t.ascii \"-export:_%s \"\n", ST_name(sym));
		  }
	      }else{
	          fprintf( Asm_File, "\t.ascii \"-export:%s \"\n", ST_name(sym));  			
              }
	  ...
	  }else if (ST_sclass(sym) == SCLASS_DGLOBAL && ST_class(sym) == CLASS_VAR ){
	      // .common. type
	      TY_IDX ty_idx = ST_type( sym );
	      const char* ty_name = TY_name( ty_idx );
	      
	      if( ty_name != NULL ){
		  if( strcmp( ty_name, ".common." ) == 0 ){
		      fprintf ( Asm_File, "\t.ascii \"-export:%s \"\n", ST_name(sym));  
		  }
	      } 
	  }
  ...
  if( Gen_PIC_Shared && Is_Target_32bit()){
      // Output MACOSX Style Function Stub
      fprintf ( Asm_File, "\n# Print Function Stubs\n");
      FOREACH_SYMBOL (GLOBAL_SYMTAB, sym, i) {
        if( ST_class(sym) == CLASS_FUNC && !ST_is_export_local(sym) && !ST_is_not_used(sym)  ){

	    if( strncmp( ST_name(sym), "LBB", 3 ) == 0 ){
	    	// If it is a PIC Label
	     	char* endstr = ST_name(sym);
		endstr += strlen(endstr) - 8;
		if( strcmp( endstr, "Pic_Base" ) == 0 ){
		    // Skip it
		    continue;
		}else{
	            char* reststr = ST_name(sym) + 3;
		    char* underscore = strstr( reststr, "_" );
		    if( underscore != NULL ){
			char num[256];
			char* err;
			num[0] = '\0';
			strncpy( num, reststr, underscore-reststr );
			strtol( num, &err, 10 );
			if( err != NULL && err[0] == '\0' ) continue;
		    }		
		}
	    }
	    ...
	    FOREACH_SYMBOL (GLOBAL_SYMTAB, st, j) {
	    	if( j >= i ) break;
		if( ST_class(st) == CLASS_FUNC && !ST_is_export_local(st) && !ST_is_not_used(st)  ){
		    if( strcmp( ST_name(st), ST_name(sym) ) == 0 ){
		        duplicated = TRUE;
			break;
		    }
		}
	    ...
	    if( duplicated == FALSE ){
	         fprintf( Asm_File, "\t.section __IMPORT,__jump_table,symbol_stubs,self_modifying_code,5\n" );
		 if( PU_has_SpecialABI(Pu_Table[ST_pu(sym)]) ){
	             fprintf( Asm_File, "%s$stub:\n", ST_name(sym)); 
	             fprintf( Asm_File, "\t.indirect_symbol %s\n", ST_name(sym));		     
		 }else{
	             fprintf( Asm_File, "_%s$stub:\n", ST_name(sym)); 
	             fprintf( Asm_File, "\t.indirect_symbol _%s\n", ST_name(sym));
		 }
	         fprintf( Asm_File, "\t.byte   0xf4,0xf4,0xf4,0xf4,0xf4\n");
	    }
	}
      }
  }
  
  if( Gen_PIC_Shared && Is_Target_32bit() && (Run_autopar && LNO_Run_AP > 0) ){
  
      BOOL sug_numthread_seen = FALSE;
      ST* st;
      int i;
      FOREACH_SYMBOL (GLOBAL_SYMTAB, st, i) {
	    	
	if( ST_class(st) != CLASS_FUNC && ST_sclass(st) == SCLASS_EXTERN && !ST_is_export_local(st) ){
	    if( strcmp( ST_name(st), "__ompc_sug_numthreads" ) == 0 ){
		   sug_numthread_seen = TRUE;
	    }
	}
      }      
      
      if( sug_numthread_seen ){
      fprintf ( Asm_File, "\t.non_lazy_symbol_pointer\n" );
      fprintf ( Asm_File, "L%s$non_lazy_ptr: \n", "__ompc_sug_numthreads" );
      fprintf ( Asm_File, "\t.indirect_symbol %s\n", "__ompc_sug_numthreads" );
      fprintf ( Asm_File, "\t.long 0\n" );
      fprintf ( Asm_File, "\t.subsections_via_symbols\n" );       
      }
  }
  
  if( Gen_PIC_Shared && Is_Target_32bit() && Read_Global_Data){
  
      BOOL data_seen = FALSE;
      BOOL rodata_seen = FALSE;
      BOOL bss_seen = FALSE;
	...
	if( ST_class(st) != CLASS_FUNC && ST_sclass(st) == SCLASS_EXTERN && !ST_is_export_local(st) ){
	    Base_Symbol_And_Offset( st, &base_st, &base_ofst );
	    if( base_st ){
	       if( strcmp( ST_name(base_st), ".data_symbol" ) == 0 ){
		   data_seen = TRUE;
	       }else if( strcmp( ST_name(base_st), ".rodata_symbol" ) == 0 ){
		   rodata_seen = TRUE;
	       }else if( strcmp( ST_name(base_st), ".bss_symbol" ) == 0 ){
		   bss_seen = TRUE;
	       }
 	    }
	}
      ...
      // Output MACOSX Style Global Extern Symbol
      fprintf ( Asm_File, "\n# Print Extern Global Symbol Pointers For IPA \n");
...
#if defined(__APPLE__) && defined(ABSOFT_EXTENSIONS) 
    fprintf ( Asm_File, "\t%s %s.note.GNU-stack\n", AS_SECTION, AS_DATA_SEG);
#else
#if defined(TARG_X64)
// Disable it because new assembler doesn't like it
