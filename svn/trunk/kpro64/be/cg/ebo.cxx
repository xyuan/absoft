 ...
 *  Module: ebo.cxx
 *  $Revision: 3010 $
 *  $Date: 2009-05-04 10:29:48 -0400 (Mon, 04 May 2009) $
 *  $Author: yin $
 *  $Source$
 *
 *  Revision comments:
...
#include "lra.h" 	// Single_Register_Subclass
#endif
                      ...
                      EBO_TN_INFO **actual_tninfo)
{
  ...
  if ((index_tninfo == NULL) ||
      (index_op == NULL) ||
#ifdef ABSOFT_EXTENSIONS
      (OP_bb(index_op) == NULL) || //already deleted
#endif
...
#ifdef ABSOFT_EXTENSIONS
  // [YIN2006-02-22] [JIRA BE-132]
  if (actual_tninfo[index_opnd] != NULL && additive_index_tn != NULL){
      if( TN_spill( actual_tninfo[index_opnd]->local_tn ) == TN_spill( additive_index_tn ) ){
          // Used a previous value, spill location becomes the actual memory location.
          Set_TN_spill( additive_index_tn, NULL );
      }
  }
#endif
...
#endif
#ifdef ABSOFT_EXTENSIONS
	 // See if a register can be allocated after replacement 
	 // This is a bug of LRA, because LRA assumes if a GTN opnd(with a register) is not preallocated,
  	 // the register will be marked as use 
 	 // In this situation, 
	 // [0x80c1ce0] [   3] [ 152] :- store32_abs TN483(%rax) (sym:.bss+340) ; WN id:243 IDAVAL+0x0
	 // [0x80e6360] [   3] [   0] TN223(%rsi) :- mov64 GTN1(%rax) ; copy
	 // GTN1(%rax) will mark %rax used
	 // TN483(%rax) is preallocated, but cannot get %rax because it is used 
	 // so that LRA fails 
	 // The following code just prevent replacement to temporarily workaround, it is not a good solution
	 // TODO: give a better solution
	 if( mcmodel >= MEDIUM  // target on store_abs so far
	     && TN_is_global_reg(pred_tn)    // GTN 
	     && TN_is_dedicated(pred_tn)   // has a register 
	     && !TN_is_preallocated(pred_tn)) { // Not preallocated 

      	     ISA_REGISTER_CLASS cl = TN_register_class(pred_tn);
      	     REGISTER reg = TN_register(pred_tn);		
	     
	     // Check all ops before this op until the defined op 
	     // or a op has re-defined this register( not implemented)
	     BOOL not_replace = FALSE;
	     OP* cur_op = OP_prev(op);
	     while( cur_op != NULL && cur_op != pred_op ){
		ASM_OP_ANNOT* asm_info = (OP_code(cur_op) == TOP_asm) ?
      			(ASM_OP_ANNOT*) OP_MAP_Get(OP_Asm_Map, cur_op) : NULL;
		for( INT i = 0; i < OP_opnds(cur_op); i ++ ){
		  ISA_REGISTER_SUBCLASS subclass = asm_info ?
			ASM_OP_opnd_subclass(asm_info)[i] : OP_opnd_reg_subclass(cur_op, i);
		  REGISTER reg = Single_Register_Subclass(subclass);
		  if (reg != REGISTER_UNDEFINED && !TN_is_preallocated( OP_opnd(cur_op,i) ) ) {
		     if( cl == TN_register_class( OP_opnd(cur_op,i) ) 
			&& reg == reg ){
			// match 
			not_replace = TRUE;
			break;
 ...
 */
{
  ...
  if(  OP_reads_rflags( op ) ){
    for( OP* prev_op = OP_prev(op); prev_op != NULL; prev_op = OP_prev( prev_op ) ){

      if( TOP_is_change_rflags( OP_code(prev_op) ) )
	return FALSE;;

      if( OP_reads_rflags( prev_op ) )
	break;
    }
  }
#endif
...
#ifdef ABSOFT_EXTENSIONS
      // Lea process
      if( OP_code(op) == OP_code(pred_op) && hash_op_matches == FALSE ){
	if( OP_code(op) == TOP_leax64 || OP_code(op) == TOP_leax32 ){
		// Those should be same 
	 	// TN179 :- leax64 GTN138 TN298 (0x1) (0x0) ;
		// TN193 :- leax64 TN298 GTN138 (0x1) (0x0) ;
		TN* op_scale_tn = OP_opnd( op, 2 );
		TN* pr_scale_tn = OP_opnd( pred_op, 2 );
		
		TN* op_offset_tn = OP_opnd( op, 3 );
		TN* pr_offset_tn = OP_opnd( pred_op, 3 );

		// Should be all constant 
		if( TN_has_value( op_scale_tn ) &&
		    TN_has_value( pr_scale_tn ) &&
		    TN_has_value( op_offset_tn ) &&
		    TN_has_value( pr_offset_tn ) ){
		    // offset should be the same 
		    // scale should be one
		    if( TN_value( op_offset_tn ) == TN_value( pr_offset_tn ) &&
			TN_value( op_scale_tn ) == TN_value( pr_scale_tn ) &&
			TN_value( op_scale_tn ) == 1 ){

			TN* op_tn1 = OP_opnd( op, 0 );
			TN* pr_tn1 = OP_opnd( pred_op, 0 );

			TN* op_tn2 = OP_opnd( op, 1 );
			TN* pr_tn2 = OP_opnd( pred_op, 1 );

			if( op_tn1 == pr_tn2 && op_tn2 == pr_tn1 ){
				// recover the status
				hash_op_matches = TRUE;
			}
		    }
...
#endif
#ifdef ABSOFT_EXTENSIONS
            && OP_code(op) != TOP_fprem // Top_fprem must has two different opnds.
#endif
	   ) {
