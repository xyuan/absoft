...
static INT32 mispredict, absoftfixed, taken;
static double times;
...
// length each. Hence we do a (3/4) - as the more conservative of the
// two divisions. We retain the (- 60).
// Pathscale Compiler uses 1/2 for its 2.4 version

#define Large_BB(bb, loop) \
        (LOOP_DESCR_nestlevel((loop)) == 0 && \
        BB_length((bb)) >= ((INT)(Split_BB_Length * .75) - 60))
...
Similar_Ptr_Addrs_Match (OP *pred_op, OP *succ_op)
{
  ...
  if( pred_base_num >= 0 )  pred_base_tn = OP_opnd(pred_op, pred_base_num);
  TN *succ_base_tn = NULL;
  if( succ_base_num >= 0 )  succ_base_tn = OP_opnd(succ_op, succ_base_num);
  if( ! (succ_base_tn != NULL && pred_base_tn != NULL) ) return FALSE;
...
OP_Has_Restrictions(OP *op, BB *source_bb, BB *target_bb, mINT32 motion_type)
{
  ...
  for (i = 0; i < OP_results(op); ++i) {
    ...
    // %ebx is reserved
    if( Is_Target_32bit() && Gen_PIC_Shared && PU_References_GOT && result_tn == Ebx_TN() ) return TRUE;
#endif
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
    // %ebx is reserved
    if( Is_Target_32bit() && Gen_PIC_Shared && PU_References_GOT && opnd_tn == Ebx_TN() ) return TRUE;
#endif
		    ...
		    mINT32 motion_type)
{
  ...
  if( mcmodel >= MEDIUM &&
	...
	return FALSE;
#ifdef ABSOFT_EXTENSIONS
      if( top == TOP_store8_abs || top == TOP_store16_abs ||
          top == TOP_store32_abs || top == TOP_store64_abs )
        return FALSE;
#endif
    }
  }
...
#ifdef KEY
#ifdef TARG_X8664
   op_access_mem |= OP_load_exe(cur_op);
#endif
...
void GCM_Schedule_Region (HBS_TYPE hbs_type)
{
  ...
  CGTARG_Compute_Branch_Parameters(&mispredict, &absoftfixed, &taken, &times);
