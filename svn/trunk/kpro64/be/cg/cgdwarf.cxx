...
Cg_Dwarf_Name_From_Handle(Dwarf_Unsigned idx)
{
  ...
  if (CGD_Symtab[idx].type == CGD_ELFSYM) {
    ...
    st_type = Em_Get_Symbol_Type( CGD_Symtab[idx].index );
    char* st_name = Em_Get_Symbol_Name(CGD_Symtab[idx].index);
    
    if( st_type == STT_FUNC ){
        // Add Name ABI
	return Index_To_Str(Save_Str2( "_", st_name ));
    }
    return st_name;
  }
  else {
    ...
    char* label = Index_To_Str(sidx);
    if( label != NULL ){
        if( strncmp( label, ".LBB", 4 ) == 0 ) return (label+1);
	if( strncmp( label, ".LDW", 4 ) == 0 ) return (label+1);
	if( strncmp( label, ".LEH", 4 ) == 0 ) return (label+1);
...
#else
#ifdef TARG_X64
  // [JIRA BE-189 YIN 2007-02-21]
  if (CGD_Symtab[idx].type == CGD_ELFSYM) {
    unsigned char st_type;
    st_type = Em_Get_Symbol_Type( CGD_Symtab[idx].index );
    char* st_name = Em_Get_Symbol_Name(CGD_Symtab[idx].index);
    
    if( Is_Target_32bit() && st_type == STT_FUNC ){
        // Add Name ABI
	return Index_To_Str(Save_Str2( "_", st_name ));
    }
    return st_name;
...
put_variable(DST_flag flag, DST_VARIABLE *attr, Dwarf_P_Die die)
{
  if (DST_IS_const(flag))  /* Not yet supported */ {
...
#ifdef ABSOFT_EXTENSIONS
    DST_STR_IDX str_idx = DST_VARIABLE_def_name(attr);
    if( !DST_IS_NULL(str_idx) ){
        if (DST_IS_artificial(flag)) return;
    } 
#endif
    ...
    DST_STR_IDX str_idx = DST_VARIABLE_def_name(attr);
    if( !DST_IS_NULL(str_idx) ){
        char* name = DST_STR_IDX_TO_PTR (str_idx);
	if( name != NULL && name[0] == '.' ) no_name = true;
        if (DST_IS_artificial(flag)) no_name = true;
    }
#endif
...
#ifdef ABSOFT_EXTENSIONS
void Cg_Dwarf_Set_Section_Index( Elf64_Word scn_index )
{
...
#ifdef ABSOFT_EXTENSIONS
        // [YIN2005-11-23] [JIRA-99]
	if( strchr(text, '\n' ) == NULL ){
          fprintf (Asm_File, "%s%4d  %s\n", ASM_CMNT_LINE, i+1, text);
	}else{
...
Cg_Dwarf_Add_Line_Entry (INT code_address, SRCPOS srcpos)
{
  ...
  if (Assembly) {
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
    CGEMIT_Prn_Line_Dir_In_Asm(usrcpos, cur_text_index);
#else
				      ...
				      BOOL                  is_64bit)
{
  ...
  while (k <= reloc_count) {
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
        if( strncmp( Cg_Dwarf_Name_From_Handle(reloc_buffer[k].drd_symbol_index), ".debug", 6 ) == 0 ){
	  fprintf(asm_file, "\t%s\t%s", reloc_name, "0x0" );
	}else if( strcmp( Cg_Dwarf_Name_From_Handle(reloc_buffer[k].drd_symbol_index), ".data" ) == 0 || 
	     strcmp( Cg_Dwarf_Name_From_Handle(reloc_buffer[k].drd_symbol_index), ".text" ) == 0 || 
   	     strcmp( Cg_Dwarf_Name_From_Handle(reloc_buffer[k].drd_symbol_index), ".rodata" ) == 0 ){
	  fprintf(asm_file, "\t%s\tL%s", reloc_name, Cg_Dwarf_Name_From_Handle(reloc_buffer[k].drd_symbol_index) );
	}else if( strcmp( Cg_Dwarf_Name_From_Handle(reloc_buffer[k].drd_symbol_index), ".bss" ) == 0  ){
	  fprintf(asm_file, "\t%s\t%s$1", reloc_name, Cg_Dwarf_Name_From_Handle(reloc_buffer[k].drd_symbol_index) );
	}else{
	  fprintf(asm_file, "\t%s\t%s", reloc_name, Cg_Dwarf_Name_From_Handle(reloc_buffer[k].drd_symbol_index) );
	}
#else
...
#ifdef KEY
	if (gen_pic)
#if defined(ABSOFT_EXTENSIONS) && defined(TARG_X64)
        fputs (" ", asm_file);
#else
	    fputs ("-.", asm_file);
#endif
	...
	// bug 2729
	fprintf(asm_file, "\t%s\t.FDE%d - .LEHCIE", reloc_name, this_count);
	++k;
	}
	break;
#endif	// KEY
	...
	// need unaligned AS_ADDRESS for dwarf, so add .ua
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
	fprintf(asm_file, "\t.set\tRELOC_Offset_%llu, ", k );
	fprintf(asm_file, "%s - %s \n", 
	        Cg_Dwarf_Name_From_Handle(reloc_buffer[k + 1].drd_symbol_index) ,
		Cg_Dwarf_Name_From_Handle(reloc_buffer[k].drd_symbol_index));
	fprintf(asm_file, "\t%s\tRELOC_Offset_%llu", reloc_name, k );	
#else
