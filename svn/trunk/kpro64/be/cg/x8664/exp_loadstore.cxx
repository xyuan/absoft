		       ...
		       ISA_REGISTER_CLASS rclass)
{
  ...
  case MTYPE_FQ:
    ...
    // [YIN2005-11-8] [JIRA BE-75]
    if( rclass == ISA_REGISTER_CLASS_float ){
      return base != NULL ? TOP_ldsd : TOP_ldsd_n32;
    }else if( rclass == ISA_REGISTER_CLASS_x87 ){
...
void 
Expand_Indexed_Load (TYPE_ID mtype, TN *result, TN *base, TN *disp, TN* index_tn, OPS *ops)
{
  TN* res;

  if ( mtype == MTYPE_V16I1 || mtype == MTYPE_V8I1 ) {
    if( Is_Target_SSE41() ){
	if (base != NULL){
	   Build_OP (TOP_pinsrb, result, base, disp, Gen_Literal_TN( 0, 4 ), ops);    // 0  

     	   TN* base_tn_1i = Build_TN_Like( base ); // base + index*1
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_1i, base, index_tn, Gen_Literal_TN( 1, 4 ), Gen_Literal_TN( 0, 4 ), ops );	   

     	   TN* base_tn_5i = Build_TN_Like( base_tn_1i ); // base + index*1 + index*4
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_5i, base_tn_1i, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );	

     	   TN* base_tn_6i = Build_TN_Like( base_tn_1i ); // base + index*1 + index*5
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_6i, base_tn_5i, index_tn, Gen_Literal_TN( 1, 4 ), Gen_Literal_TN( 0, 4 ), ops );

     	   TN* base_tn_7i = Build_TN_Like( base_tn_1i ); // base + index*2 + index*5
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_7i, base_tn_5i, index_tn, Gen_Literal_TN( 2, 4 ), Gen_Literal_TN( 0, 4 ), ops );

     	   TN* base_tn_8i = Build_TN_Like( base_tn_1i ); // base + index*8 
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_8i, base, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 0, 4 ), ops );

      	   Build_OP (TOP_pinsrbx, result, base, disp, index_tn, Gen_Literal_TN( 1, 4 ),  Gen_Literal_TN( 1, 4 ), ops); // 1
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pinsrbx, result, base, disp, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 2, 4 ), ops); // 2
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF); 

      	   Build_OP (TOP_pinsrbx, result, base_tn_1i, disp, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 3, 4 ), ops); // 3
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pinsrbx, result, base, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 4, 4 ), ops); // 4
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      	   Build_OP (TOP_pinsrbx, result, base_tn_1i, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 5, 4 ), ops); // 5
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pinsrbx, result, base_tn_5i, disp, index_tn, Gen_Literal_TN( 1, 4 ),  Gen_Literal_TN( 6, 4 ), ops); // 6
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pinsrbx, result, base_tn_5i, disp, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 7, 4 ), ops); // 7
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

	   if( mtype == MTYPE_V8I1 ) return;

      	   Build_OP (TOP_pinsrbx, result, base, disp, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 8, 4 ), ops); // 8
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      	   Build_OP (TOP_pinsrbx, result, base_tn_1i, disp, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 9, 4 ), ops); // 9
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pinsrbx, result, base_tn_6i, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 10, 4 ), ops); // 10
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pinsrbx, result, base_tn_7i, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 11, 4 ), ops); // 11
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pinsrbx, result, base_tn_8i, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 12, 4 ), ops); // 15
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pinsrbx, result, base_tn_5i, disp, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 13, 4 ), ops); // 13
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pinsrbx, result, base_tn_6i, disp, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 14, 4 ), ops); // 14
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pinsrbx, result, base_tn_7i, disp, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 15, 4 ), ops); // 15
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

	}else{
	   Build_OP (TOP_pinsrb_n32, result, disp, Gen_Literal_TN( 0, 4 ), ops);    // 0  

     	   TN* base_tn_1i = Build_TN_Like( index_tn ); // base + index*1
     	   Build_OP( Is_Target_32bit() ? TOP_leaxx32 : TOP_leaxx64, base_tn_1i, index_tn, Gen_Literal_TN( 1, 4 ), Gen_Literal_TN( 0, 4 ), ops );	   
     	   ...
     	   TN* base_tn_8i = Build_TN_Like( base_tn_1i ); // base + index*8 
     	   Build_OP( Is_Target_32bit() ? TOP_leaxx32 : TOP_leaxx64, base_tn_8i, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 0, 4 ), ops );

      	   Build_OP (TOP_pinsrbxx, result, disp, index_tn, Gen_Literal_TN( 1, 4 ),  Gen_Literal_TN( 1, 4 ), ops); // 1
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pinsrbxx, result, disp, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 2, 4 ), ops); // 2
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF); 

      	   Build_OP (TOP_pinsrbxx, result, base_tn_1i, disp, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 3, 4 ), ops); // 3
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pinsrbxx, result, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 4, 4 ), ops); // 4
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  
	   ...
	   if( mtype == MTYPE_V8I1 ) return;
      	   ...
      	   Build_OP (TOP_pinsrbxx, result, disp, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 8, 4 ), ops); // 8
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  
       ...
       FmtAssert( FALSE, ( "Expand_Indexed_Load: not implemented" ) );
    }
   
  }
  else if ( mtype == MTYPE_V16I2 || mtype == MTYPE_V8I2 ) {
    if( Is_Target_SSE41() ){
	if (base != NULL){
	   Build_OP (TOP_pinsrw2, result, base, disp, Gen_Literal_TN( 0, 4 ), ops);    // 0  
     	   ...
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_5i, base_tn_1i, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );	
      	   ...
      	   Build_OP (TOP_pinsrwx, result, base, disp, index_tn, Gen_Literal_TN( 1, 4 ),  Gen_Literal_TN( 1, 4 ), ops); // 1
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pinsrwx, result, base, disp, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 2, 4 ), ops); // 2
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF); 

      	   Build_OP (TOP_pinsrwx, result, base_tn_1i, disp, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 3, 4 ), ops); // 3
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

	   if( mtype == MTYPE_V8I2 ) return;

      	   Build_OP (TOP_pinsrwx, result, base, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 4, 4 ), ops); // 4
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      	   Build_OP (TOP_pinsrwx, result, base_tn_1i, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 5, 4 ), ops); // 5
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pinsrwx, result, base_tn_5i, disp, index_tn, Gen_Literal_TN( 1, 4 ),  Gen_Literal_TN( 6, 4 ), ops); // 6
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pinsrwx, result, base_tn_5i, disp, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 7, 4 ), ops); // 7
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
  
	}else{

	   Build_OP (TOP_pinsrw_n32, result, disp, Gen_Literal_TN( 0, 4 ), ops);    // 0  
     	   ...
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_5i, base_tn_1i, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );	
      	   ...
      	   Build_OP (TOP_pinsrwxx, result, disp, index_tn, Gen_Literal_TN( 1, 4 ),  Gen_Literal_TN( 1, 4 ), ops); // 1
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pinsrwxx, result, disp, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 2, 4 ), ops); // 2
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF); 
           ...
           if( mtype == MTYPE_V8I2 ) return;
      	   ...
      	   Build_OP (TOP_pinsrwxx, result, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 4, 4 ), ops); // 4
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  
  ...
  else if ( mtype == MTYPE_V16I4 || mtype == MTYPE_V8I4 ) {

   if( Is_Target_SSE41() ){
    if (base != NULL) {
      Build_OP (TOP_pinsrd, result, base, disp, Gen_Literal_TN( 0, 4 ), ops);    // 1

      Build_OP (TOP_pinsrdx, result, base, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 1, 4 ), ops); // 2
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      if( mtype == MTYPE_V8I4 ) return;

      Build_OP (TOP_pinsrdx, result, base, disp, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 2, 4 ), ops);  // 3
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      TN* new_base_tn = Build_TN_Like( base );
      Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, new_base_tn, base, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );

      Build_OP (TOP_pinsrdx, result, new_base_tn, disp, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 3, 4 ), ops);  // 4
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF); 
    }
    else {
      Build_OP (TOP_pinsrd_n32, result, disp, Gen_Literal_TN( 0, 4 ), ops);    // 1
 
      Build_OP (TOP_pinsrdxx, result, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 1, 4 ), ops); // 2
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      if( mtype == MTYPE_V8I4 ) return;

      Build_OP (TOP_pinsrdxx, result, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 2, 4 ), ops);  // 3
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      TN* new_base_tn = Build_TN_Like( base );
      Build_OP( Is_Target_32bit() ? TOP_leaxx32 : TOP_leaxx64, new_base_tn, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );
    ...
    if (base != NULL) {
	 ...
	 result = Build_TN_Like( result );
      ...
      Build_OP (TOP_ldss, result, base, disp, ops);    // 1
      Build_OP (TOP_ldhpsx, result, base, disp, index_tn, Gen_Literal_TN( 4, 4 ), ops);      // 2
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      if( mtype == MTYPE_V8I4 ){
  	 // 11101000
	 Build_OP(TOP_pshufd, res, result, Gen_Literal_TN(0xE8, 4), ops);
	 return;
      }

      TN* result_tmp = Build_TN_Like( result );
      Build_OP (TOP_ldssx, result_tmp, base,  disp, index_tn, Gen_Literal_TN( 8, 4 ), ops);    // 3

      TN* new_base_tn = Build_TN_Like( base );
      Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, new_base_tn, base, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );

      Build_OP (TOP_ldhpsx, result_tmp, new_base_tn, disp, index_tn, Gen_Literal_TN( 8, 4 ), ops);   // 4
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF); 

      // Shufps
      // 10001000b = 136
      Build_OP(TOP_shufps, result, result, result_tmp, Gen_Literal_TN(136, 4), ops);
    }
    else {
	 ...
	 result = Build_TN_Like( result );
      ...
      Build_OP (TOP_ldss_n32, result, disp, ops);    // 1
      Build_OP (TOP_ldhpsxx, result, index_tn, Gen_Literal_TN( 4, 4 ), disp, ops);      // 2
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
	 ...
	 return;
      ...
      TN* result_tmp = Build_TN_Like( result );
      Build_OP (TOP_ldssxx, result_tmp, index_tn, Gen_Literal_TN( 8, 4 ), disp, ops);    // 3
  ...
  }else if ( mtype == MTYPE_V16I8) {

   if( Is_Target_SSE41() ){
    if (base != NULL) {
      Build_OP (TOP_pinsrq, result, base, disp, Gen_Literal_TN( 0, 4 ), ops);  
      Build_OP (TOP_pinsrqx, result, base, disp, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 1, 4 ), ops); // 2  
    }
    else {
      Build_OP (TOP_pinsrq_n32, result, disp, Gen_Literal_TN( 0, 4 ), ops);    
      Build_OP (TOP_pinsrqxx, result, disp, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 1, 4 ), ops); // 2      
    }
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
    ...
    if (base != NULL) {
      Build_OP (TOP_ldlpd, result, base, disp, ops);    
      Build_OP (TOP_ldhpdx, result, base, disp, index_tn, Gen_Literal_TN( 8, 4 ), ops);    
    }
    else {
      Build_OP (TOP_ldlpd_n32, result, disp, ops);    
      Build_OP (TOP_ldhpdxx, result, index_tn, Gen_Literal_TN( 8, 4 ), disp, ops);    
    }
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
   }
  ...
  else if (mtype == MTYPE_V16F4 || mtype == MTYPE_V8F4 || mtype == MTYPE_V16C4 ) {

   if( Is_Target_SSE41() ){
    if (base != NULL) {
      Build_OP (TOP_insertps, result, base, disp, Gen_Literal_TN( 0, 4 ), ops);    // 1

      Build_OP (TOP_insertpsx, result, base, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 1<<4, 4 ), ops); // 2
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      if( mtype == MTYPE_V8F4 ) return;

      Build_OP (TOP_insertpsx, result, base, disp, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 2<<4, 4 ), ops);  // 3
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      TN* new_base_tn = Build_TN_Like( base );
      Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, new_base_tn, base, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );

      Build_OP (TOP_insertpsx, result, new_base_tn, disp, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 3<<4, 4 ), ops);  // 4
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF); 
    }
    else {
      Build_OP (TOP_insertps_n32, result, disp, Gen_Literal_TN( 0, 4 ), ops);    // 1

      Build_OP (TOP_insertpsxx, result, disp, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 1<<4, 4 ), ops); // 2
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  
  
      if( mtype == MTYPE_V8F4 ) return;

      Build_OP (TOP_insertpsxx, result, base, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 2<<4, 4 ), ops);  // 3
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      TN* new_base_tn = Build_TN_Like( base );
      Build_OP( Is_Target_32bit() ? TOP_leaxx32 : TOP_leaxx64, new_base_tn, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );

      Build_OP (TOP_insertpsx, result, new_base_tn, disp, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 3<<4, 4 ), ops);  // 3
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF); 
    }
   }else{
    if (base != NULL) {
    ...
    FmtAssert( FALSE, ( "Expand_Indexed_Load: Unsupported mtype" ) );
  }
}

void
Expand_Indexed_Store (TYPE_ID mtype, TN *obj_tn, TN *base_tn, TN* disp_tn, TN* index_tn, OPS *ops )
{
  ...
  if (mtype == MTYPE_V16I1 ) {
    if( Is_Target_SSE41() ){
      if (base_tn != NULL){
	   ...
	   Build_OP (TOP_pextrb, obj_tn, base_tn, disp_tn, Gen_Literal_TN( 0, 4 ), ops);    // 0  

     	   TN* base_tn_1i = Build_TN_Like( base_tn ); // base_tn + index*1
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_1i, base_tn, index_tn, Gen_Literal_TN( 1, 4 ), Gen_Literal_TN( 0, 4 ), ops );	   

     	   TN* base_tn_5i = Build_TN_Like( base_tn_1i ); // base_tn + index*1 + index*4
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_5i, base_tn_1i, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );	

     	   TN* base_tn_6i = Build_TN_Like( base_tn_1i ); // base_tn + index*1 + index*5
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_6i, base_tn_5i, index_tn, Gen_Literal_TN( 1, 4 ), Gen_Literal_TN( 0, 4 ), ops );

     	   TN* base_tn_7i = Build_TN_Like( base_tn_1i ); // base_tn + index*2 + index*5
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_7i, base_tn_5i, index_tn, Gen_Literal_TN( 2, 4 ), Gen_Literal_TN( 0, 4 ), ops );

     	   TN* base_tn_8i = Build_TN_Like( base_tn_1i ); // base_tn + index*8 
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_8i, base_tn, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 0, 4 ), ops );

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 1, 4 ),  Gen_Literal_TN( 1, 4 ), ops); // 1
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 2, 4 ), ops); // 2
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF); 

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn_1i, disp_tn, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 3, 4 ), ops); // 3
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 4, 4 ), ops); // 4
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn_1i, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 5, 4 ), ops); // 5
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn_5i, disp_tn, index_tn, Gen_Literal_TN( 1, 4 ),  Gen_Literal_TN( 6, 4 ), ops); // 6
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn_5i, disp_tn, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 7, 4 ), ops); // 7
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 8, 4 ), ops); // 8
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn_1i, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 9, 4 ), ops); // 9
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn_6i, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 10, 4 ), ops); // 10
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn_7i, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 11, 4 ), ops); // 11
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn_8i, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 12, 4 ), ops); // 15
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn_5i, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 13, 4 ), ops); // 13
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn_6i, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 14, 4 ), ops); // 14
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn_7i, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 15, 4 ), ops); // 15
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      }else{

	   Build_OP (TOP_pextrb_n32, obj_tn, disp_tn, Gen_Literal_TN( 0, 4 ), ops);    // 0  

     	   TN* base_tn_1i = Build_TN_Like( index_tn ); // base_tn + index*1
     	   Build_OP( Is_Target_32bit() ? TOP_leaxx32 : TOP_leaxx64, base_tn_1i, index_tn, Gen_Literal_TN( 1, 4 ), Gen_Literal_TN( 0, 4 ), ops );	   
     	   ...
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_8i, base_tn, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 0, 4 ), ops );
      	   ...
      	   Build_OP (TOP_pextrbxx, obj_tn, disp_tn, index_tn, Gen_Literal_TN( 1, 4 ),  Gen_Literal_TN( 1, 4 ), ops); // 1
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pextrbxx, obj_tn, disp_tn, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 2, 4 ), ops); // 2
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF); 

      	   Build_OP (TOP_pextrbx, obj_tn, base_tn_1i, disp_tn, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 3, 4 ), ops); // 3
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pextrbxx, obj_tn, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 4, 4 ), ops); // 4
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  
      	   ...
      	   Build_OP (TOP_pextrbxx, obj_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ),  Gen_Literal_TN( 8, 4 ), ops); // 8
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  
      ...
      FmtAssert( FALSE, ( "Expand_Indexed_Store: not implemented" ) );
    }
  }else if (mtype == MTYPE_V16I2) {
    if( Is_Target_SSE41() ){
      if (base_tn != NULL){
	   Build_OP (TOP_pextrw, obj_tn, base_tn, disp_tn, Gen_Literal_TN( 0, 4 ), ops);    // 0  
     	   ...
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_5i, base_tn_1i, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );	
      	   ...
      	   Build_OP (TOP_pextrwx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 1, 4 ),  Gen_Literal_TN( 1, 4 ), ops); // 1
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pextrwx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 2, 4 ), ops); // 2
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF); 

      	   Build_OP (TOP_pextrwx, obj_tn, base_tn_1i, disp_tn, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 3, 4 ), ops); // 3
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pextrwx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 4, 4 ), ops); // 4
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  

      	   Build_OP (TOP_pextrwx, obj_tn, base_tn_1i, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 5, 4 ), ops); // 5
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pextrwx, obj_tn, base_tn_5i, disp_tn, index_tn, Gen_Literal_TN( 1, 4 ),  Gen_Literal_TN( 6, 4 ), ops); // 6
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      	   Build_OP (TOP_pextrwx, obj_tn, base_tn_5i, disp_tn, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 7, 4 ), ops); // 7
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);

      }else{

	   Build_OP (TOP_pextrw_n32, obj_tn, disp_tn, Gen_Literal_TN( 0, 4 ), ops);    // 0  
     	   ...
     	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, base_tn_5i, base_tn_1i, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );	
      	   ...
      	   Build_OP (TOP_pextrwxx, obj_tn, disp_tn, index_tn, Gen_Literal_TN( 1, 4 ),  Gen_Literal_TN( 1, 4 ), ops); // 1
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pextrwxx, obj_tn, disp_tn, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 2, 4 ), ops); // 2
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF); 

      	   Build_OP (TOP_pextrwx, obj_tn, base_tn_1i, disp_tn, index_tn, Gen_Literal_TN( 2, 4 ),  Gen_Literal_TN( 3, 4 ), ops); // 3
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);   

      	   Build_OP (TOP_pextrwxx, obj_tn, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ),  Gen_Literal_TN( 4, 4 ), ops); // 4
      	   Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);  
  ...
  else if (mtype == MTYPE_V16I4 ) {

    if( Is_Target_SSE41() ){
	if (base_tn != NULL) {
	   // Need to store one by one
           Build_OP (TOP_pextrd, obj_tn, base_tn, disp_tn, Gen_Literal_TN( 0, 4 ), ops);    // 1

           Build_OP (TOP_pextrdx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 1, 4 ), ops);  // 2
           Build_OP (TOP_pextrdx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 2, 4 ), ops);  // 3

           TN* new_base_tn = Build_TN_Like( base_tn );
	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, new_base_tn, base_tn, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );
           Build_OP (TOP_pextrdx, obj_tn, new_base_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 3, 4 ), ops);  // 4
	}
	else {
	   // Need to store one by one
           Build_OP (TOP_pextrd_n32, obj_tn, disp_tn, Gen_Literal_TN( 0, 4 ), ops);    // 1

           Build_OP (TOP_pextrdxx, obj_tn, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 1, 4 ), ops);  // 2
           Build_OP (TOP_pextrdxx, obj_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 2, 4 ), ops);  // 3
	...
	if (base_tn != NULL) {
	   // Need to store one by one
           Build_OP (TOP_stss, obj_tn, base_tn, disp_tn, ops);    

           // pshufd 11100101 = 0xE5
	   TN* obj_tn2 = Build_TN_Like( obj_tn );
	   Build_OP(TOP_pshufd, obj_tn2, obj_tn, Gen_Literal_TN(0xE5, 1), ops);
           Build_OP (TOP_stssx, obj_tn2, base_tn, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ), ops);    

           // pshufd 11100110 = 0xE6
	   TN* obj_tn3 = Build_TN_Like( obj_tn );
	   Build_OP(TOP_pshufd, obj_tn3, obj_tn, Gen_Literal_TN(0xE6, 1), ops);
           Build_OP (TOP_stssx, obj_tn3, base_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ), ops);    

           // pshufd 11100111 = 0xE7
	   TN* obj_tn4 = Build_TN_Like( obj_tn );
	   Build_OP(TOP_pshufd, obj_tn4, obj_tn, Gen_Literal_TN(0xE7, 1), ops);
	
           TN* new_base_tn = Build_TN_Like( base_tn );
	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, new_base_tn, base_tn, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );
           Build_OP (TOP_stssx, obj_tn4, new_base_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ), ops);    

	}
	else {
	   // Need to store one by one
           Build_OP (TOP_stss_n32, obj_tn, disp_tn, ops);    

           // pshufd 11100101 = 0xE5
	   TN* obj_tn2 = Build_TN_Like( obj_tn );
	   Build_OP(TOP_pshufd, obj_tn2, obj_tn, Gen_Literal_TN(0xE5, 1), ops);
           Build_OP (TOP_stssxx, obj_tn2, index_tn, Gen_Literal_TN( 4, 4 ), disp_tn, ops);    

           // pshufd 11100110 = 0xE6
	   TN* obj_tn3 = Build_TN_Like( obj_tn );
	   Build_OP(TOP_pshufd, obj_tn3, obj_tn, Gen_Literal_TN(0xE6, 1), ops);
           Build_OP (TOP_stssxx, obj_tn3, index_tn, Gen_Literal_TN( 8, 4 ), disp_tn, ops);    
   ...
   if( Is_Target_SSE41() ){
    if (base_tn != NULL) {
      Build_OP (TOP_pextrq, obj_tn, base_tn, disp_tn, Gen_Literal_TN( 0, 4 ), ops);    
      Build_OP (TOP_pextrqx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 1, 4 ), ops);    
    }
    else {
      Build_OP (TOP_pextrq_n32, obj_tn, disp_tn, Gen_Literal_TN( 0, 4 ), ops);    
      Build_OP (TOP_pextrqx, obj_tn, index_tn, Gen_Literal_TN( 8, 4 ), disp_tn, Gen_Literal_TN( 1, 4 ), ops);    
    }
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
   }else{
    if (base_tn != NULL) {
      Build_OP (TOP_stlpd, obj_tn, base_tn, disp_tn, ops);    
      Build_OP (TOP_sthpdxx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ), ops);    
    }
    else {
      Build_OP (TOP_stlpd_n32, obj_tn, disp_tn, ops);    
      Build_OP (TOP_sthpdxx, obj_tn, index_tn, Gen_Literal_TN( 8, 4 ), disp_tn, ops);    
    }
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
   }
  }
	...
	if (base_tn != NULL) {
	   // Need to store one by one
           Build_OP (TOP_extractps, obj_tn, base_tn, disp_tn, Gen_Literal_TN( 0, 4 ), ops);    // 1

           Build_OP (TOP_extractpsx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 1, 4 ), ops);  // 2
           Build_OP (TOP_extractpsx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 2, 4 ), ops);  // 3

           TN* new_base_tn = Build_TN_Like( base_tn );
	   Build_OP( Is_Target_32bit() ? TOP_leax32 : TOP_leax64, new_base_tn, base_tn, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 0, 4 ), ops );
           Build_OP (TOP_extractpsx, obj_tn, new_base_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 3, 4 ), ops);  // 4
	}
	else {
	   // Need to store one by one
           Build_OP (TOP_extractps_n32, obj_tn, disp_tn, Gen_Literal_TN( 0, 4 ), ops);    // 1

           Build_OP (TOP_extractpsxx, obj_tn, disp_tn, index_tn, Gen_Literal_TN( 4, 4 ), Gen_Literal_TN( 1, 4 ), ops);  // 2
           Build_OP (TOP_extractpsxx, obj_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ), Gen_Literal_TN( 2, 4 ), ops);  // 3
    ...
    if (base_tn != NULL) {
      Build_OP (TOP_stlpd, obj_tn, base_tn, disp_tn, ops);    
      Build_OP (TOP_sthpdx, obj_tn, base_tn, disp_tn, index_tn, Gen_Literal_TN( 8, 4 ), ops);    
    }
    else {
      Build_OP (TOP_stlpd_n32, obj_tn, disp_tn, ops);    
      Build_OP (TOP_sthpdxx, obj_tn, index_tn, Gen_Literal_TN( 8, 4 ), disp_tn, ops);    
    ...
    FmtAssert( FALSE, ( "Expand_Indexed_Store: Unsupported mtype" ) );
  }
}
#endif
  ...
  VARIANT variant)
{
  ...
  if( is_lda ){
...
#ifdef ABSOFT_EXTENSIONS
	  if( ST_is_Global( base_sym ) ){
	      tmp_tn = tn;
	      TN *new_base = Build_TN_Of_Mtype(Pointer_Mtype); 
	      Build_OP( TOP_lea64, new_base, Rip_TN(), Gen_Symbol_TN(base_sym, 0, TN_RELOC_NONE), &newops );
	      Set_OP_no_alias(OPS_last(&newops));
	      tn = Build_TN_Like( tn );
	      Build_OP( TOP_ld64, tn, new_base, Gen_Literal_TN( 0, 4 ), &newops );
	      Set_OP_no_alias(OPS_last(&newops));
          }else
#endif
	...
	if( (ST_sclass(base_sym) == SCLASS_COMMON  && !ST_is_initialized_in_file(base_sym)) ||
	    (Gen_PIC_Shared && ST_sclass(base_sym) == SCLASS_EXTERN  && ST_class(base_sym) != CLASS_FUNC && !ST_is_export_local(base_sym))  ){
		// .comm via non_lazy_symbol_pointer
   	        TN* new_base = Build_TN_Of_Mtype(Pointer_Mtype);
		if( Gen_PIC_Shared ){
		    Build_OP( TOP_leas32, new_base,  Gen_Symbol_TN( base_sym, 0, TN_RELOC_MACOSX32_GOT ),    &newops );
		    PU_References_GOT = TRUE;		
		}else{
		    Build_OP( TOP_ldc32, new_base,  Gen_Symbol_TN( base_sym, 0, TN_RELOC_NONE ),    &newops );
		}
		Set_OP_no_alias(OPS_last(&newops));
		TN* addr = Build_TN_Of_Mtype(Pointer_Mtype);
		Build_OP( TOP_ld32, addr, new_base, Gen_Literal_TN( 0, 4 ), &newops );
   	        Build_OP( TOP_addi32, tn, addr, Gen_Literal_TN( base_ofst, 4 ), &newops );
        }else if( Gen_PIC_Shared && !ST_is_thread_local(base_sym)){
	    Build_OP( TOP_leas32, tn, Gen_Symbol_TN( base_sym, base_ofst, TN_RELOC_MACOSX32_GOT ), &newops );
	    Set_OP_no_alias(OPS_last(&newops));
	    PU_References_GOT = TRUE;
	} 
#else
	if( Gen_PIC_Shared && (!ST_is_export_local(base_sym) ||
	  ...
	  if( ST_is_Global( base_sym ) ){
	      tmp_tn = tn;
	      TN *new_base = Build_TN_Of_Mtype(Pointer_Mtype); 
              Build_OP( TOP_leas32, new_base, Gen_Symbol_TN(base_sym, 0, TN_RELOC_NONE), &newops );
	      Set_OP_no_alias(OPS_last(&newops));
	      tn = Build_TN_Like( tn );
	      Build_OP( TOP_ld32, tn, new_base, Gen_Literal_TN( 0, 4 ), &newops );
	      Set_OP_no_alias(OPS_last(&newops));
          }else
#endif
...
#ifdef ABSOFT_EXTENSIONS
	  // It generates R_X86_64_32S relocation with movq
	  // However for common block, the address may be moved above 2GB boundary
	  if( ST_sclass(sym) != SCLASS_COMMON && ISA_LC_Value_In_Class(base_ofst, LC_simm32) ){
#else
	  if( ISA_LC_Value_In_Class(base_ofst, LC_simm32) ){
#endif
...
// Bug 4461
	  ...
	  */
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
      if( ST_is_Global( base_sym ) ){
          // [YIN20060601] [JIRA BE-177]   
          TN *new_base = Build_TN_Of_Mtype(Pointer_Mtype);  
	  if( Gen_PIC_Shared ){
            if( Is_Target_64bit() ){
		Build_OP( TOP_lea64, new_base, Rip_TN(), Gen_Symbol_TN(base_sym, 0, TN_RELOC_X8664_GOTPCREL), &newops );
	    }else{
        	Build_OP( TOP_leas32, new_base, Gen_Symbol_TN(base_sym, 0, TN_RELOC_MACOSX32_GOT), &newops );
		PU_References_GOT = TRUE;
	    }	  
		...
		Build_OP( TOP_lea64, new_base, Rip_TN(), Gen_Symbol_TN(base_sym, 0, TN_RELOC_NONE), &newops );
          ...
          base_tn = Build_TN_Of_Mtype(Pointer_Mtype);
	  if( Is_Target_64bit() ){
		  Build_OP( TOP_ld64, base_tn, new_base, Gen_Literal_TN( 0, 4 ), &newops );
	  }else{
	  	  Build_OP( TOP_ld32, base_tn, new_base, Gen_Literal_TN( 0, 4 ), &newops );
	  }
	  tmp_tn = tn;
	  tn = Build_TN_Like( tn );
          ofst_tn = Gen_Literal_TN( 0, 4 );
      }else if( (Is_Target_32bit() && ST_sclass(base_sym) == SCLASS_COMMON && !ST_is_initialized_in_file(base_sym) ) ||
          (Is_Target_32bit() && Gen_PIC_Shared && ST_sclass(base_sym) == SCLASS_EXTERN  && ST_class(base_sym) != CLASS_FUNC  && !ST_is_export_local(base_sym) ) ){
      	  // .comm via non_lazy_symbol_pointer
	  TN *new_base = Build_TN_Of_Mtype(Pointer_Mtype); 
	  if( Gen_PIC_Shared ){
	      Build_OP( TOP_leas32, new_base, Gen_Symbol_TN(base_sym, 0, TN_RELOC_MACOSX32_GOT), &newops );
      ...
      }else if( Gen_PIC_Shared && Is_Target_32bit() ){
	  // for -m32 here
	  TN *new_base = Build_TN_Of_Mtype(Pointer_Mtype);
	  Build_OP( TOP_leas32, new_base, Gen_Symbol_TN( base_sym, 0, TN_RELOC_MACOSX32_GOT ), &newops );
	  // got address should not alias
	  ...
	  base_tn = new_base;
	  ofst_tn = Gen_Literal_TN( base_ofst, 4 );
      }else if( Gen_PIC_Shared && Is_Target_64bit() && !ST_is_export_local(base_sym) ){
	  TN *new_base = Build_TN_Of_Mtype(Pointer_Mtype);
...
#ifdef ABSOFT_EXTENSIONS
  // [YIN20060601] [JIRA BE-177] 
  if( base_sym != NULL && ST_is_Global( base_sym ) ){
	ofst_tn = Gen_Literal_TN( base_ofst, 4 );
	base_tn = tn;
	tn = tmp_tn;
	// Add the offset
        if( Is_Target_64bit() ){	
	   Build_OP( TOP_addi64, tn, base_tn, ofst_tn, &newops );       
	}else{
 	   Build_OP( TOP_addi32, tn, base_tn, ofst_tn, &newops );
	}
  }
#endif
