...
const char * ip_calc_funcname = "getip.absoft";
#else
...
static TN* PIC_base_Label_TN = NULL;

void EETARG_Generate_MACOSX32_GOT_Entry_Code( BB* bb, OPS* ops )
{
  ...
  LABEL_IDX bb_label = PU_PIC_Base_Label_Idx;
  
  Set_Label_For_BB( bb_label, pop_bb );
...
void EETARG_Generate_MACOSX32_GOT_Altentry_Code( BB* bb, OPS* ops )
{
  FmtAssert( BB_entry(bb), ("BB is not entry") );
  FmtAssert( Is_Target_32bit(), ("PIC entry for -m32 only") );
  ...
  Build_OP( TOP_addlofst32, ebx_tn, PIC_base_Label_TN, label_tn, ebx_tn, ops );
  Set_OP_computes_got( OPS_last(ops) );
