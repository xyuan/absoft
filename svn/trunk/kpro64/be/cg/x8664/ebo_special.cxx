...
BOOL EBO_Can_Merge_Into_Offset (OP *op)
{
  ...
  if( top == TOP_leapic32 || top == TOP_leapic64 ||
      top == TOP_addlofst32 )
    return FALSE;
#endif
...
BOOL EBO_Load_Execution( OP* alu_op, TN** opnd_tn, EBO_TN_INFO** actual_tninfo )
{
  ...
  */
  ...
  if( Is_Target_64bit() && mode == BASE_MODE && opnd1 != NULL &&
      TN_has_value(opnd1) && base == Rip_TN() ){
      // At 64bit mode OSX only
      // cmpl $1, L.data(%rip) fails to generate the right relocation
      // cmpl $1, L.data+2(%rip) does not fail
      if( offset != NULL && TN_is_symbol( offset ) ){
      	  ST* var = TN_var( offset );
	  if( strcmp( ST_name( var ), ".data" ) == 0 && 
	      TN_offset(offset) == 0 ){
	      return FALSE;
	  }
      }
...
EBO_Lea_Insertion( OP* op, TN** opnd_tn, EBO_TN_INFO** actual_tninfo )
{
  ...
  case TOP_add64:
	...
	if (src0_info && src1_info &&
...
#ifdef ABSOFT_EXTENSIONS
      /*  
       *   A <= leaxx  C n  k    ( A =     C*n + k )
       *   D <= add  A C         ( D = A + C )
       *   becomes
       *   D <= leaxx  C n+1 k   ( D =     C*(n+1) + k )
       */
      if ( alu_op && alu_op->bb == op->bb && 
	  ((OP_code(alu_op) == TOP_leaxx32 && code == TOP_add32) ||
	   (OP_code(alu_op) == TOP_leaxx64 && code == TOP_add64)) && 
	  ...
	  TN_is_register(OP_opnd(alu_op, 0)) &&
	  TN_has_value(OP_opnd(alu_op, 1)) &&
	  TN_has_value(OP_opnd(alu_op, 2)) &&
	  TN_is_register(OP_opnd(op, 1)) &&  
          ( ( OP_opnd(alu_op, 0) == OP_opnd(op, 1) && OP_result(alu_op, 0) == OP_opnd(op, 0) ) ||
            ( OP_opnd(alu_op, 0) == OP_opnd(op, 0) && OP_result(alu_op, 0) == OP_opnd(op, 1) ) ) &&
          !TN_live_out_of(OP_result(alu_op, 0), OP_bb(alu_op)) &&
	  ((op->next != NULL &&
	    !is_live_tn(op->next, OP_result(alu_op, 0))) ||
	   (op->next == NULL && 
	  ...
	  Check_No_Use_Between(alu_op, op, OP_result(alu_op, 0))) {
  	     ...
  	     if ( (TN_value(OP_opnd(alu_op, 1))  == 0 || TN_value(OP_opnd(alu_op, 1)) == 1 || 
                  TN_value(OP_opnd(alu_op, 1))  == 3 || TN_value(OP_opnd(alu_op, 1)) == 7 ) ) {// has to be 1 2 4 8 after adding 1
	        
		new_op = Mk_OP ((code == TOP_add32)?TOP_leax32:TOP_leax64, 
				OP_result(op, 0), OP_opnd(alu_op, 0), 
				Gen_Literal_TN( TN_value(OP_opnd(alu_op, 1)) + 1 , 4), OP_opnd(alu_op, 2));	  
	  ...
	  Check_No_Redef_Between( alu_op, op, OP_opnd(alu_op, 0)) ) {
        	...
        	if( OP_opnd(alu_op, 0) != OP_opnd(op, 1) && OP_result(alu_op, 0) == OP_opnd(op, 0) ){
			new_op = Mk_OP ((code == TOP_add32)?TOP_leax32:TOP_leax64, 
				OP_result(op, 0), OP_opnd(op, 1), OP_opnd(alu_op, 0), 
				OP_opnd(alu_op, 1), OP_opnd(alu_op, 2));	  
		}else{
			new_op = Mk_OP ((code == TOP_add32)?TOP_leax32:TOP_leax64, 
				OP_result(op, 0), OP_opnd(op, 0), OP_opnd(alu_op, 0), 
				OP_opnd(alu_op, 1), OP_opnd(alu_op, 2));	
		}
      ...
      /*  
       *   A <= leax B C n  k    ( A = B + C*n + k )
       *   D <= add  A C         ( D = A + C )
       *   becomes
       *   D <= leax B C n+1 k   ( D = B + C*(n+1) + k )
       */
      if ( alu_op && alu_op->bb == op->bb && 
	  ((OP_code(alu_op) == TOP_leax32 && code == TOP_add32) ||
	   (OP_code(alu_op) == TOP_leax64 && code == TOP_add64)) && 
	  ...
	  Check_No_Use_Between(alu_op, op, OP_result(alu_op, 0))) {
	     ...
	     src1_info = get_tn_info( OP_opnd(alu_op, 0));
  	     ...
  	     if (src0_info && src1_info &&
	         src0_info->sequence_num > src1_info->sequence_num &&
                 (TN_value(OP_opnd(alu_op, 2))  == 0 || TN_value(OP_opnd(alu_op, 2)) == 1 || 
                  TN_value(OP_opnd(alu_op, 2))  == 3 || TN_value(OP_opnd(alu_op, 2)) == 7 ) // has to be 1 2 4 8 after adding 1
                 ) {
	  
                new_op = Mk_OP ((code == TOP_add32)?TOP_leax32:TOP_leax64, 
			  OP_result(op, 0), OP_opnd(alu_op, 0), 
			  OP_opnd(alu_op, 1), 
			  Gen_Literal_TN( TN_value(OP_opnd(alu_op, 2)) + 1 , 4), OP_opnd(alu_op, 3));	  
       ...
       *   D <= add  A C         ( D = A + C )
       *   becomes
       *   D <= leax B C 1 n     ( D = B + C + n ) if C is GTN but B is not
       *   D <= leax C B 1 n     ( D = B + C + n ) if B is GTN but C is not
       *   D <= leax C B 1 n     ( D = B + C + n ) if C&B are the same type, C has the smallest sequence_num
       *   D <= leax C B 1 n     ( D = B + C + n ) if C&B are the same type, B has the smallest sequence_num 
       */
       if ( alu_op && alu_op->bb == op->bb && 
	  ((OP_code(alu_op) == TOP_lea32 && code == TOP_add32) ||
	   (OP_code(alu_op) == TOP_lea64 && code == TOP_add64)) && 
	  TN_is_register(OP_opnd(alu_op, 0)) &&
	  TN_has_value(OP_opnd(alu_op, 1)) &&
	  TN_is_register(OP_opnd(op, 1)) &&  
          OP_opnd(alu_op, 1) == OP_opnd(op, 1) && 
	  ...
	  Check_No_Redef_Between( alu_op, op, OP_opnd(alu_op, 0)) ) {
             ...
             TN* tn_C = OP_opnd(op, 1);
	     TN* tn_B = OP_opnd(alu_op, 0);
	     EBO_TN_INFO *srcB_info, *srcC_info;
	     srcC_info = get_tn_info( tn_C ); // C
	     srcB_info = get_tn_info( tn_B ); // B
	  
	        if( TN_is_global_reg( tn_B ) && !TN_is_global_reg( tn_C )){  
			new_op = Mk_OP ((code == TOP_add32)?TOP_leax32:TOP_leax64, 
				OP_result(op, 0), tn_C, tn_B, 
				Gen_Literal_TN( 1 , 4), OP_opnd(alu_op, 1));	  
		}else if( TN_is_global_reg( tn_C ) && !TN_is_global_reg( tn_B )){
			new_op = Mk_OP ((code == TOP_add32)?TOP_leax32:TOP_leax64, 
				OP_result(op, 0), tn_B, tn_C, 
				Gen_Literal_TN( 1 , 4), OP_opnd(alu_op, 1));	
		}else if( srcB_info && srcC_info &&
			srcC_info->sequence_num > srcB_info->sequence_num) {
			new_op = Mk_OP ((code == TOP_add32)?TOP_leax32:TOP_leax64, 
				OP_result(op, 0), tn_C, tn_B, 
				Gen_Literal_TN( 1 , 4), OP_opnd(alu_op, 1));	
		}else{
  ...
  case TOP_addi64:
	...
	break;
       ...
       */  
       if ( alu_op && alu_op->bb == op->bb && 
	  ((OP_code(alu_op) == TOP_lea32 && code == TOP_addi32) ||
	   (OP_code(alu_op) == TOP_lea64 && code == TOP_addi64)) && 
	  TN_is_register(OP_opnd(alu_op, 0)) &&
	  TN_is_register(OP_opnd(op, 0)) &&
	  TN_has_value(OP_opnd(alu_op, 1)) &&
	  TN_has_value(OP_opnd(op, 1)) &&  
          TNs_Are_Equivalent(OP_opnd(op, 0),OP_result(alu_op, 0)) &&
          !TN_live_out_of(OP_result(alu_op, 0), OP_bb(alu_op)) &&
	  ((op->next != NULL &&
	    !is_live_tn(op->next, OP_result(alu_op, 0))) ||
	   (op->next == NULL && 
	  ...
	  Check_No_Use_Between(alu_op, op, OP_result(alu_op, 0)) &&
		...
		new_op = Mk_OP ((code == TOP_addi32)?TOP_lea32:TOP_lea64, 
				OP_result(op, 0), OP_opnd(alu_op, 0), 
				Gen_Literal_TN( TN_value(OP_opnd(op, 1)) + TN_value(OP_opnd(alu_op, 1)) , 4));	  
       ...
       *   D <= addi A h         ( D = A + h )
       *   becomes
       *   D <= leax B C n  k+h  ( D = B + C*n + (k+h) )
       */     
      if ( alu_op && alu_op->bb == op->bb && 
	  ((OP_code(alu_op) == TOP_leax32 && code == TOP_addi32) ||
	   (OP_code(alu_op) == TOP_leax64 && code == TOP_addi64)) && 
	  !TN_is_symbol(OP_opnd(alu_op, 3)) &&   
	  TN_is_register(OP_opnd(alu_op, 0)) &&
	  TN_is_register(OP_opnd(alu_op, 1)) &&
	  TN_is_register(OP_opnd(op, 0)) &&
	  TN_has_value(OP_opnd(alu_op, 2)) &&
	  TN_has_value(OP_opnd(alu_op, 3)) &&
	  TN_has_value(OP_opnd(op, 1)) && 
          TNs_Are_Equivalent(OP_opnd(op, 0),OP_result(alu_op, 0)) &&
          !TN_live_out_of(OP_result(alu_op, 0), OP_bb(alu_op)) &&
	  ((op->next != NULL &&
	  ...
	  Check_No_Use_Between(alu_op, op, OP_result(alu_op, 0)) &&
          Check_No_Redef_Between( alu_op, op, OP_opnd(alu_op, 0)) && 
	  Check_No_Redef_Between( alu_op, op, OP_opnd(alu_op, 1)) ) {        

                new_op = Mk_OP ((code == TOP_addi32)?TOP_leax32:TOP_leax64, 
			  OP_result(op, 0), OP_opnd(alu_op, 0), 
			  OP_opnd(alu_op, 1), 
			  OP_opnd(alu_op, 2), Gen_Literal_TN( TN_value(OP_opnd(op, 1)) + TN_value(OP_opnd(alu_op, 3)) , 4));	  
       ...
       *   D <= addi A h         ( D = A + h )
       *   becomes
       *   D <= leaxx  C n  k+h  ( D =     C*n + (k+h) )
       */ 
      if ( alu_op && alu_op->bb == op->bb && 
	  ((OP_code(alu_op) == TOP_leaxx32 && code == TOP_addi32) ||
	   (OP_code(alu_op) == TOP_leaxx64 && code == TOP_addi64)) && 
	  !TN_is_symbol(OP_opnd(alu_op, 2)) &&   
	  TN_is_register(OP_opnd(alu_op, 0)) &&
	  TN_has_value(OP_opnd(alu_op, 1)) &&
	  TN_has_value(OP_opnd(alu_op, 2)) &&
	  ...
	  Check_No_Use_Between(alu_op, op, OP_result(alu_op, 0)) &&
          Check_No_Redef_Between( alu_op, op, OP_opnd(alu_op, 0)) ) {        
                ...
                new_op = Mk_OP ((code == TOP_addi32)?TOP_leaxx32:TOP_leaxx64, 
			  OP_result(op, 0), OP_opnd(alu_op, 0), 
			  OP_opnd(alu_op, 1), 
			  Gen_Literal_TN( TN_value(OP_opnd(op, 1)) + TN_value(OP_opnd(alu_op, 2)) , 4));	  
