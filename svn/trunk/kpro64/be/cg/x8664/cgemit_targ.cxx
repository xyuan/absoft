...
#include "em_dwarf.h"   	// Em_Dwarf_Start_Label_Line
#include "cgdwarf.h"	        // Cg_Dwarf_Symtab_Entry
#endif
...
extern void
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
CGEMIT_Prn_Line_Dir_In_Asm (USRCPOS usrcpos, Elf64_Word scn_index )
#else
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
  // Create a Label
  static UINT64 label_sn = 0;
  char loc_name[256];
  loc_name[0] = '\0';
  sprintf( loc_name, "Loc_$_%llx", label_sn++ );
  
  LABEL_IDX lab;
  LABEL *label;  
  label = &New_LABEL(CURRENT_SYMTAB, lab);
  LABEL_Init (*label, Save_Str(loc_name), LKIND_DEFAULT);

  Em_Dwarf_Start_Label_Line( Cg_Dwarf_Symtab_Entry(CGD_LABIDX, lab, scn_index ) , usrcpos );
  
  fprintf( Asm_File, "%s:\t\t\t\t", loc_name );
		       ...
		       )
{
...
#endif
  {
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
    static bool have_text_label = false;
    if ( strcmp(scn_name, ".text") == 0 ) {
	/* __TEXT, __text */
	fprintf (asm_file, "\n\t%s %s", AS_SECTION, AS_TEXT_SECTION);
	if( !have_text_label ){
	    fprintf(asm_file, "\n%s:\n", "L.text");
	...
	/* __DATA, __data */
	static bool have_data_label = false;
	fprintf (asm_file, "\n\t%s %s", AS_SECTION, AS_DATA_SECTION);
	if( !have_data_label ){
	    fprintf(asm_file, "\n%s:\n", "L.data");
	...
	static bool have_rodata_label = false;
    	fprintf (asm_file, "\n\t%s %s", AS_SECTION, "__TEXT, __const");
	if (!have_rodata_label) {
	fprintf (asm_file, "\n%s:\n", "L.rodata");
	have_rodata_label = true;
	}
    } else if ( strcmp(scn_name, ".bss") == 0) {
    } else if ( strncmp(scn_name, ".debug_", 7 ) == 0 ){
        /* __DWARF, scn_name */
	if( strlen(scn_name) < 15 )
	    fprintf (asm_file, "\n\t%s __DWARF, __%s, regular, debug", AS_SECTION, scn_name+1);
	else
  	    fprintf (asm_file, "\n\t%s __DWARF, %s, regular, debug", AS_SECTION, scn_name+1);
    	//fprintf (asm_file, "\n%s:\n", scn_name+1);
    } else if ( strncmp(scn_name, ".text.", 6 ) == 0){
        /* IPA, .text name*/
	fprintf (asm_file, "\n\t%s %s", AS_SECTION, AS_TEXT_SECTION);
	if( !have_text_label ){
	    fprintf(asm_file, "\n%s:\n", "L.text");
	...
	/* __DATA, scn_name */
	if( scn_name[0] == '.' ){
    	    fprintf (asm_file, "\n\t%s __DATA, %s", AS_SECTION, scn_name+1);
	}else if( strlen(scn_name) <= 14 ){
	    fprintf (asm_file, "\n\t%s __DATA, __%s", AS_SECTION, scn_name);
	}else{
	    fprintf (asm_file, "\n\t%s __DATA, %s", AS_SECTION, scn_name);
	}
	fprintf (asm_file, "\n%s:\n", scn_name);
    }
#else
    ...
    if (! CG_emit_non_gas_syntax) {
#if (defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)) || defined( TARG_X64 )
#else
...
#ifdef ABSOFT_EXTENSIONS
 	case TN_RELOC_MACOSX32_GOT:
	  {
	    char* str = NULL;
	    if( ST_sclass(st) == SCLASS_COMMON && !ST_is_initialized_in_file(st) ){
                asprintf( &str, "\"L%s$non_lazy_ptr\"-\"%s\"%+lld(%%ebx) ", ST_name(st), LABEL_name(PU_PIC_Base_Label_Idx) ,*val);	    
	    }else if( Is_Target_32bit() && ST_sclass(st) == SCLASS_EXTERN && ST_class(st) != CLASS_FUNC && !ST_is_export_local(st)){
	        // Use Lazy Pointer
	        asprintf( &str, "\"L%s$non_lazy_ptr\"-\"%s\"%+lld(%%ebx) ", ST_name(st), LABEL_name(PU_PIC_Base_Label_Idx) ,*val);
	    }else if( Is_Target_32bit() && ST_class(st) == CLASS_FUNC ){
	    	if( !ST_is_export_local(st) ){
                     asprintf( &str, "\"_%s$stub\"-\"%s\"%+lld(%%ebx) ", ST_name(st), LABEL_name(PU_PIC_Base_Label_Idx) ,*val);
		}else{
		     asprintf( &str, "_%s", ST_name(st) );    
		}
	    }else if( !Is_Target_32bit() && ST_class(st) == CLASS_FUNC && !ST_is_export_local(st) ){
	        asprintf( &str, "\"%s$stub\"-\"%s\"%+lld(%%ebx) ", ST_name(st), LABEL_name(PU_PIC_Base_Label_Idx) ,*val);
	    }else{

	        if( strcmp( ST_name(st), ".text" ) == 0 || strcmp( ST_name(st), ".data" ) == 0 ||
...
static void Init_OP_Name()
{
...
#ifdef ABSOFT_EXTENSIONS
  OP_Name[TOP_leas32]   = "leal";
  OP_Name[TOP_leas64]   = "leaq";
  OP_Name[TOP_addlofst32] = "addl";
  OP_Name[TOP_leapic32] = "leal";
  OP_Name[TOP_leapic64] = "leaq";
  // SSE4.1 Instructions
  OP_Name[TOP_movntdqax] = "movntdqa";
  OP_Name[TOP_movntdqaxx] = "movntdqa";
  OP_Name[TOP_movntdqa_n64] = "movntdqa";
  ...
  OP_Name[TOP_pinsrwx] = "pinsrw";
  OP_Name[TOP_pinsrdx] = "pinsrd";
  OP_Name[TOP_pinsrqx] = "pinsrq";
  OP_Name[TOP_pinsrbxx] = "pinsrb";
  OP_Name[TOP_pinsrwxx] = "pinsrw";
  OP_Name[TOP_pinsrdxx] = "pinsrd";
  OP_Name[TOP_pinsrqxx] = "pinsrq";
  OP_Name[TOP_pinsrw2] = "pinsrw";
  OP_Name[TOP_pextrw2] = "pextrw";
  OP_Name[TOP_extractps_n32] = "extractps";	
  OP_Name[TOP_insertps_n32] = "insertps";
  OP_Name[TOP_pinsrb_n32] = "pinsrb";
  OP_Name[TOP_pinsrw_n32] = "pinsrw";
  OP_Name[TOP_pinsrd_n32] = "pinsrd";
  OP_Name[TOP_pinsrq_n32] = "pinsrq";
  OP_Name[TOP_pextrb_n32] = "pextrb";
  OP_Name[TOP_pextrw_n32] = "pextrw";
  OP_Name[TOP_pextrd_n32] = "pextrd";
  OP_Name[TOP_pextrq_n32] = "pextrq";
#endif
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
          if( TN_relocs( OP_opnd(op,opnd) ) != TN_RELOC_MACOSX32_GOT )
#endif
      Str_Prepend(name, '$');
    }
...
void CGEMIT_Setup_Ctrl_Register( FILE* f )
{
  ...
  if( FPU_Trap_Set ){
    x86_cntrl_mask &= ~FPU_Trap;	  
  }
  
  if( Is_Target_32bit() ){ 
      // Match SIMD_?Mask 
    if ( !SIMD_IMask || DEBUG_Trap_Uv)		// trap invalid operands
      x86_cntrl_mask &= 0xfffe ;
    if ( !SIMD_DMask )
      x86_cntrl_mask &= 0xfffd ;
    if ( !SIMD_ZMask )
      x86_cntrl_mask &= 0xfffb ;
    if ( !SIMD_OMask )
      x86_cntrl_mask &= 0xfff7 ;
    if ( !SIMD_UMask )
      x86_cntrl_mask &= 0xffef ;
    if ( !SIMD_PMask )
      x86_cntrl_mask &= 0xffdf ;      
  }
#endif
...
#ifdef ABSOFT_EXTENSIONS
   PC = old_PC; // Undo PC sync, those instructions should be inserted into blocks.
                // There is another UNDO down below
#endif 
