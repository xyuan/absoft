				   ...
				   TOP jmp_opcode, TN* targ, OPS* ops )
{
  ...
  if( jmp_opcode == TOP_jb || jmp_opcode == TOP_jbe || jmp_opcode == TOP_jle ){
      
    BB* bb_entry = Cur_BB;
    BB* bb_control = Gen_And_Append_BB( bb_entry );
    BB* bb_exit = Gen_And_Append_BB( bb_control );
      
    const LABEL_IDX bb_control_label = Gen_Label_For_BB( bb_control );
    const LABEL_IDX bb_exit_label = Gen_Label_For_BB( bb_exit );      
    const LABEL_IDX bb_targ_label = TN_label( targ );
    ...
    if( !false_br ){
        ...
        BB_branch_wn(bb_entry) = WN_Create(OPC_TRUEBR,1);
        WN_kid0(BB_branch_wn(bb_entry)) = NULL;
        WN_label_number(BB_branch_wn(bb_entry)) = bb_targ_label;
    }
    
    BB_branch_wn(bb_control) = WN_Create(OPC_TRUEBR,1);
    WN_kid0(BB_branch_wn(bb_control)) = NULL;
    WN_label_number(BB_branch_wn(bb_control)) = bb_targ_label;    
    
    // Build BB_entry
    Build_OP( cmp_opcode, rflags, src1, src2, ops );
    if( false_br ){
        Build_OP( TOP_jp, rflags, targ, ops );
    }else{
        Build_OP( TOP_jp, rflags, Gen_Label_TN( bb_exit_label, 0 ), ops );
    }
    if( &New_OPs != ops )
      OPS_Append_Ops( &New_OPs, ops );    
    Process_New_OPs();
    ...
    // Build BB_control
    bb_control_ops = &New_OPs;
    Build_OP( jmp_opcode, rflags, targ, bb_control_ops );
    total_bb_insts = 0;
    Last_Processed_OP = NULL;
    Process_New_OPs();
    BB_Append_Ops( bb_control, bb_control_ops );
    OPS_Init( bb_control_ops );
  ...
  default:
	...
	*/
	...
	src1 = tmp;
        ...
        // [YIN2005-12-07] [JIRA BE-109]
        if( TN_size( src1 ) == 4 && is_64bit && is_integer_signed && Is_Target_64bit() ){
           tmp = Gen_Register_TN( ISA_REGISTER_CLASS_integer, 8 );
           Build_OP( TOP_cltq, tmp, src1, ops );
           src1 = tmp;
	}
      }
#endif
...
#ifdef ABSOFT_EXTENSIONS
        // [YIN2005-12-07] [JIRA BE-109]
        if( TN_size( src2 ) == 4 && is_64bit && is_integer_signed && Is_Target_64bit() ){
           tmp = Gen_Register_TN( ISA_REGISTER_CLASS_integer, 8 );
           Build_OP( TOP_cltq, tmp, src2, ops );
           src2 = tmp;
	}
#endif
	cmp_opcode = is_64bit ? TOP_cmp64 : TOP_cmp32;
...
#ifdef ABSOFT_EXTENSIONS
          Expand_Ordered_Branch( cmp_opcode, src1, src2, variant, jmp_opcode, targ, ops);
#else
