...
static TN* Gen_Const_VectorF4_Symbol_TN( float val1, float val2, float val3, float val4,
				TN_RELOCS relocs = TN_RELOC_NONE )
{
  QUAD_TYPE v;
  VECTORF4 low;
  ...
  const TCON tcon = Host_To_Targ_Quad( v );
...
static TN* Gen_Const_Quad_Symbol_TN( double low, double hi,
				TN_RELOCS relocs = TN_RELOC_NONE )
{
  QUAD_TYPE v;
...
/* ====================================================================
 *
 * Expand_Convert_Vector_Length 
 *
 * Using SSE4.1 integer format conversion instructions
 * ====================================================================
 */
void Expand_Convert_Vector_Length ( TN *dest, TN *src, TYPE_ID rtype, TYPE_ID desc, OPS *ops )
{
  FmtAssert( MTYPE_is_vector( rtype ),  ("Expand_Convert_Vector_Length: data type must be vector\n"));
  FmtAssert( MTYPE_is_vector( desc ),  ("Expand_Convert_Vector_Length: data type must be vector\n"));
  FmtAssert( Is_Target_SSE41(),  ("Expand_Convert_Vector_Length: need sse4.1 instruction\n"));

  TOP top = TOP_UNDEFINED;
  if( rtype == MTYPE_V16I8 && desc == MTYPE_V8I4 ) top = TOP_pmovsxdq;
  if( rtype == MTYPE_V16I8 && desc == MTYPE_V8I2 ) top = TOP_pmovsxwq;
  if( rtype == MTYPE_V16I8 && desc == MTYPE_V8I1 ) top = TOP_pmovsxbq;
  if( rtype == MTYPE_V16I4 && desc == MTYPE_V8I2 ) top = TOP_pmovsxwd;
  if( rtype == MTYPE_V16I4 && desc == MTYPE_V8I1 ) top = TOP_pmovsxbd; 
  
  FmtAssert( top != TOP_UNDEFINED, ("Expand_Convert_Vector_Length: Undefined TOP code\n"));
...
#ifdef ABSOFT_EXTENSIONS
  } else if( TN_register_class( result ) == ISA_REGISTER_CLASS_x87 && Is_Target_32bit() ){
    Build_OP( TOP_fchs, result, src, ops );
#endif
  } else {
#if 1
...
#ifdef ABSOFT_EXTENSIONS
  if ( Is_Target_SSE41() && mtype == MTYPE_V16I4 ) {
    Expand_Flop(OPC_V16I4MPY, result, src1, src2, NULL, ops);
    return;
  }
#endif
...
static void Expand_Long_Double_To_Int( TN* dest, TN* src, TYPE_ID imtype, OPS* ops )
{
  ...
  if( x87_Control_Fixed ){
	...
	case 80:{
	   x87_cw = 0xfef; 
           x87_old_cw = 0x37f;
           break;	// double-extended precision
        }
	default:  FmtAssert(FALSE,("Expand_Long_Double_To_Int: invalid x87 precision"));
      }
      
      if( DEBUG_Trap_Uv ){
          x87_cw = x87_cw & 0xfffe;
          x87_old_cw = x87_old_cw & 0xfffe;
      }

      TN* con = Gen_Const_Symbol_TN( x87_cw, 0.0, MTYPE_U2 );
      Build_OP( TOP_fldcw_n32, X87_cw_TN(), con, ops );
...
#ifdef ABSOFT_EXTENSIONS
  if( x87_Control_Fixed == TRUE ){
      TN* con = Gen_Const_Symbol_TN( x87_old_cw, 0.0, MTYPE_U2 );
      Build_OP( TOP_fldcw_n32, X87_cw_TN(), con, ops );
      Set_OP_volatile( OPS_last(ops) );      
  }else{
  ...
  if( imtype == MTYPE_U8 ){
	...
	QUAD_TYPE qzero;
	qzero = Zero_Quad(qzero);
    TCON tcon = Host_To_Targ_Quad(qzero);
#else
    ...
    {
#ifdef ABSOFT_EXTENSIONS
      // Force_IEEE_Comparisons need to be disabled because
      // it will generate another setting of blocks
      BOOL prev_setting = Force_IEEE_Comparisons;
      Force_IEEE_Comparisons = FALSE;
#endif
      Exp_OP3v( OPC_TRUEBR,
		NULL,
		Gen_Label_TN( bb_exit_label, 0 ),
		...
		ops );
#ifdef ABSOFT_EXTENSIONS
      Force_IEEE_Comparisons = prev_setting;
#endif
...
#ifdef ABSOFT_EXTENSIONS
  /* [YIN2005-10-23] [JIRA BE-62] */
  ISA_REGISTER_CLASS rc = ISA_REGISTER_CLASS_x87;
  TN* x87_src = Gen_Register_TN (rc, MTYPE_RegisterSize(MTYPE_FQ) );
#else
...
Expand_Float_To_Float (TN *dest, TN *src, TYPE_ID rtype, TYPE_ID desc, OPS *ops)
{
  ...
  if( !MTYPE_is_quad(rtype) && 
       TN_register_class(dest) == ISA_REGISTER_CLASS_x87 && TN_register_class(src)  == ISA_REGISTER_CLASS_float ){
       ty = MTYPE_To_TY( desc );
  }else{
       ty = MTYPE_To_TY( !MTYPE_is_quad(rtype) ? rtype : desc );
  }
#else
  ...
  if( MTYPE_is_quad( desc ) ){
    ...
    CGTARG_Load_From_Memory( dest, st, ops );
#ifdef ABSOFT_EXTENSIONS
  } else if( TN_register_class(dest) == ISA_REGISTER_CLASS_x87 && TN_register_class(src)  == ISA_REGISTER_CLASS_float ){
    // sse2 -> x87 is decided by sse2 type
    CGTARG_Store_To_Memory( src, st, ops );
    CGTARG_Load_From_Memory( dest, st, ops );
  } else if( TN_register_class(dest) == ISA_REGISTER_CLASS_float && TN_register_class(src)  == ISA_REGISTER_CLASS_x87 ){
    // x87 -> sse2 type is decided by sse2 type
    CGTARG_Store_To_Memory( src, st, ops );
    CGTARG_Load_From_Memory( dest, st, ops );
#endif
  } else {
				      ...
				      OPS* ops )
{
  ...
  */
  ...
  /* [YIN2005-10-23] [JIRA BE-62] */
  ISA_REGISTER_CLASS rc = ISA_REGISTER_CLASS_x87;
  TN* x87_dest = Gen_Register_TN (rc, MTYPE_RegisterSize(MTYPE_FQ) );
#else
...
Expand_Min (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if( OP_NEED_PAIR(mtype) ){
    ...
    if( Is_Target_SSE41() ) { 
	switch(mtype){
	  case MTYPE_V16I1: top = TOP_pminsb;	 break;
	  case MTYPE_V16I2: top = TOP_pminsw;	 break;
	  case MTYPE_V16I4: top = TOP_pminsd;	 break;
/*	  case MTYPE_V16U1: top = TOP_min128v8;	 break;
	  case MTYPE_V16U2: top = TOP_pminuw; 	 break;
	  case MTYPE_V16U4: top = TOP_pminud;	 break; */
	}
    }
...
{     
  ...
  if( OP_NEED_PAIR(mtype) ){
    ...
    if( Is_Target_SSE41() ) { 
	switch(mtype){
	  case MTYPE_V16I1: top = TOP_pmaxsb;	 break;
	  case MTYPE_V16I2: top = TOP_pmaxsw;	 break;
	  case MTYPE_V16I4: top = TOP_pmaxsd;	 break;
/*	  case MTYPE_V16U1: top = TOP_max128v8;	 break;
	  case MTYPE_V16U2: top = TOP_pmaxuw; 	 break;
	  case MTYPE_V16U4: top = TOP_pmaxud;	 break; */
	}
    }
...
#ifdef ABSOFT_EXTENSIONS
extern void 
Exp_Select_Float_Vector_Noncondition ( 
        OPCODE select, TN *result, TN* cond, TN *true_tn, TN *false_tn, OPS *ops )
{

  const TYPE_ID select_type = OPCODE_rtype(select);
  BOOL is_rsize_double = (select_type == MTYPE_V16F8) ? TRUE : FALSE;
  
  FmtAssert( MTYPE_is_vector( select_type ), ( "Exp_Select_Float_Vector_Noncondition: Non-vector Select?") );
  FmtAssert( MTYPE_is_float( select_type ), ( "Exp_Select_Float_Vector_Noncondition: Only float type is valid" ) ); 
  ...
  if (result == cond) {
    TN* tmp = Build_TN_Like( cond );
    Expand_Copy( tmp, cond, select_type, ops );
    cond = tmp;
  }

  /* For case where <result>, <true_tn> and <false_tn> are fp vector type.
  ...
  */
  ...
  Build_OP( is_rsize_double ? TOP_andpd : TOP_andps, tmp2, true_tn, tmp1, ops );

  Build_OP( is_rsize_double ? TOP_andnpd : TOP_andnps, tmp3, tmp1, false_tn, ops );

  Build_OP( is_rsize_double ? TOP_orpd : TOP_orps, result, tmp3, tmp2, ops );
  ...
  if( MTYPE_is_float(select_type) &&
    ...
    */
	...
	cmp_kid2 = tmp;
      ...
      cmp_kid2 = cmp2;
#ifdef ABSOFT_EXTENSIONS
      // [YIN2005-11-27] [JIRA BE-90]
      is_ssize_double = is_rsize_double;
#endif
    }
...
#ifdef ABSOFT_EXTENSIONS
      {
        // [YIN2005-10-06][JIRA BE-38]
	Build_OP( ( is_ssize_double )?TOP_cmpsd:TOP_cmpss,
		  tmp1, cmp_kid1, cmp_kid2, ctrl, &new_ops );   
	// [YIN2005-11-26][JIRA BE-100]
	if( is_ssize_double == false && is_rsize_double == true ){
	   // float --> double conversion
	   TN* cvt = Build_TN_Like( result );
	   Build_OP(TOP_pshufd, cvt, tmp1, Gen_Literal_TN(0x50, 1), &new_ops);
	   tmp1 = cvt;
	}else if( is_ssize_double == true && is_rsize_double == false ){
	   // double --> float conversion
	   TN* cvt = Build_TN_Like( result );
	   Build_OP(TOP_pshufd, cvt, tmp1, Gen_Literal_TN(0xD8, 1), &new_ops);	   
	   tmp1 = cvt;
	}  
      }
#else
...
#ifdef ABSOFT_EXTENSIONS
void
Expand_Float_Vector_Compares( OPCODE compare, 
                              TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  FmtAssert( MTYPE_is_vector( mtype ), ( "Expand_Vector_Float_Compares: non vector oprand found" ) );
  FmtAssert( Is_Target_SSE2(), ( "Vector Float need the support of SSE2" ) );
  
  // For the vectorized comparision, the default type is V16I8 and V16I4
  // Yin feels still need a warning
  if( TN_register_class(dest) != ISA_REGISTER_CLASS_float ){
	Set_TN_register_class( dest, ISA_REGISTER_CLASS_float );
	DevWarn( "Expand_Vector_Float_Compare: NON_Float dest oprand" );
  }
  ...
  TOP top;
  if( mtype == MTYPE_V16F8 ){
      top = TOP_cmppd; 
  }else if( mtype == MTYPE_V16F4 ){
      top = TOP_cmpps;
  }else{
      FmtAssert( FALSE, ("Vectorized Comparision only supports float/double now!!!") );
  }
     
  Build_OP( top, dest, src1, src2, ctrl, ops );
}
#endif
...
static void Expand_Recip( TN* result, TN* src2, TYPE_ID mtype, OPS* ops )
{
  ...
  if (mtype == MTYPE_V16F4)
    ...
    tcon = MTYPE_is_quad(mtype)
      ? Host_To_Targ_Quad(q10) : Host_To_Targ_Float(mtype, 1.0);
#else
    tcon = MTYPE_is_quad(mtype)
...
Expand_Reduce_Add (OPCODE op, TN *result, TN *op1, OPS *ops)
{
  ...
  case OPC_F4V16F4REDUCE_ADD: 
  {
    ...
    if( Is_Target_SSE41() ){
	// use dpps*1.0  
        TN* vector_fone = Gen_Const_VectorF4_Symbol_TN( 1.0, 1.0, 1.0, 1.0 );
 	TN* tmp = Build_TN_Like(op1);
	Build_OP( TOP_movntdqa_n64, tmp, vector_fone, ops );
	Build_OP( TOP_dpps, result, op1, tmp, Gen_Literal_TN(0xf1,1), ops );
	break;
    }
#endif
    TN* tmp = Build_TN_Like(op1);
...
Expand_Reduce_Mpy (OPCODE op, TN *result, TN *op1, OPS *ops)
{
  ...
  case OPC_I4V16I4REDUCE_MPY:
  {
    ...
    Build_OP(TOP_movdq, tmp, op1, ops);
    Build_OP(TOP_psrldq, tmp_a, tmp, Gen_Literal_TN(8, 1), ops); 
    Build_OP(TOP_pmulld, tmp_b, tmp, tmp_a, ops);
    Build_OP(TOP_movdq, tmp_c, tmp_b, ops);
    Build_OP(TOP_psrldq, tmp_d, tmp_c, Gen_Literal_TN(4, 1), ops);
    Build_OP(TOP_pmulld, tmp_e, tmp_c, tmp_d, ops);
    Build_OP(TOP_movx2g, result, tmp_e, ops);
    break;
  }
...
#ifdef ABSOFT_EXTENSIONS
    if( variant == 2 ){
        Build_OP(TOP_pshufd, result, op1, Gen_Literal_TN(0xB1, 1), ops);
    }else if( variant == 4 ){
        Build_OP(TOP_pshufd, result, op1, Gen_Literal_TN(0x1B, 1), ops);
    }else{
        FmtAssert(FALSE, ("Expand_Shuffle V16F4/I4 unsupported vecterization step"));
    }
#else
    Build_OP(TOP_pshufd, result, op1, Gen_Literal_TN(0x1B, 1), ops);
#endif
  ...
  case OPC_V16I2V16I2SHUFFLE:
    {
      ...
      Set_OP_cond_def_kind( OPS_last(ops), OP_ALWAYS_COND_DEF );
#ifdef ABSOFT_EXTENSIONS
      Build_OP(TOP_pshuflw, tmp2, tmp1, Gen_Literal_TN(0xB1, 1), ops);
      Build_OP(TOP_pshufhw, result, tmp2, Gen_Literal_TN(0xB1, 1), ops);
...
#ifdef ABSOFT_EXTENSIONS
void Expand_Dot (OPCODE opcode, TN *result, TN *src1, TN *src2, VARIANT variant, OPS *ops)
{
  TOP top = TOP_UNDEFINED;

  if( opcode == OPC_F4V16F4DOT ){
	top = TOP_dpps;
  }else if( opcode == OPC_F8V16F8DOT ){
	top = TOP_dppd;
  }else{
	FmtAssert(FALSE, ("Expand_Dot:  unexpected opcode %s", OPCODE_name(opcode)));
  }

  TN* imm_tn = NULL;
  switch(variant){
	case 1: imm_tn = Gen_Literal_TN( 0x11, 4 ); break;
	case 2: imm_tn = Gen_Literal_TN( 0x31, 4 ); break;
	case 3: imm_tn = Gen_Literal_TN( 0x71, 4 ); break;
	case 4: imm_tn = Gen_Literal_TN( 0xf1, 4 ); break;
	default:
		FmtAssert(FALSE, ("Expand_Dot:  unexpected variant %d",variant));
  }

  Build_OP( top, result, src1, src2, imm_tn, ops );
  Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
}

void Expand_Round (OPCODE opcode, TN *result, TN *src1, VARIANT variant, OPS *ops)
{
  INT32 roundmode = variant;
  TOP top = TOP_UNDEFINED;

  if( opcode == OPC_F4F4ROUND ){
	top = TOP_roundss;
  }else if( opcode == OPC_F8F8ROUND ){
	top = TOP_roundsd;
  }else if( opcode == OPC_V16F4V16F4ROUND ){
	top = TOP_roundps;
  }else if( opcode == OPC_V16F8V16F8ROUND ){
	top = TOP_roundpd;
  }else{
	FmtAssert(FALSE, ("Expand_Round:  unexpected opcode %s", OPCODE_name(opcode)));
  }

  Build_OP( top, result, src1, Gen_Literal_TN( roundmode, 4 ), ops );
}
#endif
