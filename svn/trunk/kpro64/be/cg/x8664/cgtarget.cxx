  ...
  TN **tn2)
{
  ...
  if( cmp_op == NULL ){
      ...
      if( cur_bb != NULL ){
		...
		cmp_op = BB_last_op( prev_bb );
                bFound = false;
		while( cmp_op != NULL ){
		    if( TOP_is_change_rflags( OP_code(cmp_op) ) ){
                        bFound = true;
                        break;
                    }
		    cmp_op = OP_prev( cmp_op );        
		}          
                if( bFound ) break;
                else{
                    cur_bb = prev_bb;
...
#ifdef ABSOFT_EXTENSIONS
  else if( OP_code(cmp_op) == TOP_sahf ){
	*tn1 = OP_opnd( cmp_op, 0 );
	*tn2 = NULL;
  }
#endif
...
#ifdef ABSOFT_EXTENSIONS
  if( br_special ) return V_BR_NONE;
#endif  
...
#ifdef ABSOFT_EXTENSIONS
    case TOP_lddqa:	 return TOP_movntdqa; break;
    case TOP_lddqax:	 return TOP_movntdqax; break;
    case TOP_lddqaxx:	 return TOP_movntdqaxx; break;
    case TOP_ldapd:	 return TOP_movntdqa; break;
    case TOP_ldapdx:	 return TOP_movntdqax; break;
    case TOP_ldapdxx:	 return TOP_movntdqaxx; break;
    case TOP_ldaps:	 return TOP_movntdqa; break;
    case TOP_ldapsx:	 return TOP_movntdqax; break;
    case TOP_ldapsxx:	 return TOP_movntdqaxx; break;
#endif
    }
...
void CGTARG_LOOP_Optimize( LOOP_DESCR* loop )
{
  ...
  */
...
#ifdef ABSOFT_EXTENSIONS
      if( OP_store( op ) ){ 
      	store_size += CGTARG_Mem_Ref_Bytes(op);
      }else if(OP_load( op )){ 
      	vector_load_size += CGTARG_Mem_Ref_Bytes(op);
      }
#endif
    }
  }
  ...
  size *= trip_count;
#ifdef ABSOFT_EXTENSIONS
  store_size *= trip_count;
  vector_load_size *= trip_count;
#endif

  const INT64 cache_size = CG_movnti * 1024;

#ifdef ABSOFT_EXTENSIONS
  /* replace it by nt read */
  if( vector_load_size > cache_size || size > cache_size ){
