...
/*	Expand mod(x,y) with x87 instructions
Label:                                                          
        fprem                                                  
        fnstsw    %ax                                          
        sahf                                                   
...
*/  
void 
Expand_Mod_Float (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
   FmtAssert( Is_Target_32bit(), ("x87 instructions are required for fast floating point mod") );

   BB* bb_entry = Cur_BB;
   BB* bb_body  = Gen_And_Append_BB( bb_entry );
   const LABEL_IDX bb_body_label = Gen_Label_For_BB( bb_body );
   BB* bb_exit = Gen_And_Append_BB( bb_body );
   const LABEL_IDX bb_exit_label = Gen_Label_For_BB( bb_exit );

   BB_branch_wn(bb_entry) = WN_Create(OPC_TRUEBR,1);
   WN_kid0(BB_branch_wn(bb_entry)) = NULL;
   WN_label_number(BB_branch_wn(bb_entry)) = bb_body_label;

   BB_branch_wn(bb_body) = WN_Create(OPC_TRUEBR,1);
   WN_kid0(BB_branch_wn(bb_body)) = NULL;
   WN_label_number(BB_branch_wn(bb_body)) = bb_body_label;   

   // Convert to x87 register
   TN* x87_src1 = src1;
   TN* x87_src2 = src2;

   // src1 has to be copied in any case because src1 will be self-modified later 
//   if( TN_register_class(src1) != ISA_REGISTER_CLASS_x87 ){
	x87_src1 = Build_RCLASS_TN( ISA_REGISTER_CLASS_x87 );
	Exp_COPY( x87_src1 , src1, ops );
//   }
   if( TN_register_class(src2) != ISA_REGISTER_CLASS_x87 ){
	x87_src2 = Build_RCLASS_TN( ISA_REGISTER_CLASS_x87 );
	Exp_COPY( x87_src2 , src2, ops );
   }
   ...
   // Build bb_body
   {
	OPS* bb_body_ops = &New_OPs;
	Build_OP( TOP_fprem, x87_src1, X87_cw_TN(), x87_src1, x87_src2, bb_body_ops );
	Reset_TN_is_rematerializable(x87_src1);
	Set_TN_spill(x87_src1, NULL);
	Set_OP_cond_def_kind(OPS_last(bb_body_ops), OP_ALWAYS_COND_DEF);
	TN* ax_tn = Gen_Register_TN (ISA_REGISTER_CLASS_integer, 4);
	TN* rflag_tn = Rflags_TN();
	Build_OP( TOP_fnstsw, ax_tn, X87_cw_TN(), bb_body_ops );
	Build_OP( TOP_sahf, rflag_tn, ax_tn, bb_body_ops );
	Build_OP( TOP_jp, rflag_tn, Gen_Label_TN( bb_body_label, 0 ), bb_body_ops );

	Process_New_OPs();
	BB_Append_Ops( bb_body, bb_body_ops );
	OPS_Init( bb_body_ops );
   }

   // Build bb_exit
   {
	OPS* bb_exit_ops = &New_OPs;
	if( TN_register_class(result) != ISA_REGISTER_CLASS_x87 ){
	    Exp_COPY( result , x87_src1, bb_exit_ops );
	}else if( result != x87_src1 ){
	    Exp_COPY( result , x87_src1, bb_exit_ops );
	} 
   }
