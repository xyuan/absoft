...
void Set_OP_stack_reg( OP* op, int opnd, int idx )
{
  OP_stinfo* stinfo = Get_OP_stinfo( op );
  if( stinfo == NULL ){
    FmtAssert( FALSE, ("Invalid stack information.") );
    return;
  }

  if( opnd < 0 )
    stinfo->result_st[-opnd-1] = idx + 1;
  else
    stinfo->opnd_st[opnd] = idx + 1;
}
#endif
...
static BOOL Fxch_Quick_Remover_Process( OP* op, INT32* stack, OP** loadstack, INT32 cur_top, INT32& max_top, INT32 fxch_pos )
{
  if( op == NULL ) return FALSE;
  if( !OP_refs_x87(op) ) return Fxch_Quick_Remover_Process( OP_prev(op), stack, loadstack, cur_top, max_top, fxch_pos );
  ...
  if( op_type == 1 ){
    ...
    max_top ++;
    if( max_top >= MAX_SEARCH_DEPTH ) return FALSE;

  }else if( op_type == 3 ){     
    /* decrease stack & record ld */
    loadstack[ max_top - cur_top ] = op;
    cur_top --;
  }

  /* Check if the replacement can be applied */
  BOOL probe = TRUE;
  if( cur_top < 0 ){
	
     /* Test whether reordering load can remove fxch */
     BOOL ok = TRUE;
     ...
     /* Check whether any instruction in the middle prevents reordering due to dependence */
     /* from the loadstack[fxch_pos] to loadstack[0] */
     for( int i = fxch_pos ; i > 0; i -- ){
	OP* load = loadstack[i];
	OP* term = OP_next( loadstack[0] );
	for( OP* next = OP_next( load ); next != NULL && next != term; next = OP_next(next) ){
	    /* There should be no store to the same address */
	    if( OP_store( next ) && Similar_Ptr_Addrs_Match( next, load ) ) return FALSE;
	}
	for( int j = 0; j < OP_opnds(load); j ++ ){
	    /* There should be no register used in addressing redefined in the middle */
	    TN* opnd = OP_opnd(load,j);
	    if( TN_is_register( opnd ) ){
	        for( OP* next = OP_next( load ); next != NULL && next != term; next = OP_next(next) ){
	            for( int k = 0; k < OP_results( next ); k ++ ){
			TN* ropnd = OP_result( next, k );
			if( TN_is_register( ropnd ) && 
			    TN_register_class( ropnd ) == TN_register_class( opnd ) &&
			    TN_register( ropnd ) == TN_register( opnd ) ){
			    return FALSE;
			}
	    	    }
	        }
  ...
  /* Do further probe */
  if( probe == TRUE ){
    BOOL res = Fxch_Quick_Remover_Process( OP_prev(op), stack, loadstack, cur_top, max_top, fxch_pos );  
  ...
  if( op_type == 1 || op_type == 2 ){
        /* exchange the stack pos */
        INT32 exch_pos = cur_top - fxch_pos;
	INT32 cur_pos = cur_top;
	for( int i = 0; i < OP_opnds(op); i ++ ){
		TN* opnd = OP_opnd(op,i);
		if( TN_is_register( opnd ) && TN_register_class( opnd ) == ISA_REGISTER_CLASS_x87 ){
		  st_idx = Get_OP_stack_reg( op, i );
		  /* Need Exchange */
		  if( st_idx == cur_pos ){
			Set_OP_stack_reg( op, i, exch_pos );
		  }else if( st_idx == fxch_pos ){
			Set_OP_stack_reg( op, i, cur_top );
		  }	
		}
	}
	for( int i = 0; i < OP_results(op); i ++ ){
		...
		if( TN_is_register( opnd ) && TN_register_class( opnd ) == ISA_REGISTER_CLASS_x87 ){
		  st_idx = Get_OP_stack_reg( op, -(i+1) );
		  /* Need Exchange */
		  if( st_idx == cur_pos ){
			Set_OP_stack_reg( op, -(i+1), exch_pos );
		  }else if( st_idx == fxch_pos ){
			Set_OP_stack_reg( op, -(i+1), cur_top );
		  }	
		}
	}	
  }else if( op_type == 3 ){
     int stack_pos = cur_top + 1;
     if( stack_pos == 0 ){
	/* Exchange */
	BB* bb = OP_bb( op );
	for( int i = max_top - fxch_pos + 1; i <= max_top; i ++ ){
	    BB_Remove_Op( bb, loadstack[i] );
	}
	
	BB_Insert_Op_After( bb, loadstack[max_top - fxch_pos], loadstack[max_top] );
	for( int i = max_top - fxch_pos + 1; i < max_top; i ++ ){
	    BB_Insert_Op_After( bb, loadstack[max_top - fxch_pos], loadstack[i] );
	}
	
     }
  }else{
     FmtAssert( FALSE, ("Fxch_Quick_Remover_Process: Cannot reach here") );
  }

  return TRUE;
}

/* Reorder the x87 fld instructions for removing the requirement of Fxch instructions 
 * Target Case:
 *     ....
 *     fld 
 *     ..math only..
 ...
 *     fxch <-- 
 *    
 * Assumption: no store or other fxch between the first fld involved and the current fxch
 */

static BOOL Fxch_Quick_Remover( OP* op, int dest_st_idx, BOOL before = FALSE  )
{

  if( CG_opt_level < 3 ) return FALSE;
  if( CG_x87_fxch_remove == FALSE ) return FALSE;
  if( op == NULL ) return FALSE;

  int st_idx = dest_st_idx;

  /* Count potential stack depth */
  UINT32 inst_count = 0;
  if( before == TRUE ) op = OP_prev(op);
  for( OP* prev = op; prev != NULL; prev = OP_prev(prev) ){
    if( !OP_refs_x87(prev) )
      continue;
    if( OP_code(prev) == TOP_fxch )
      break;

    inst_count ++;
  }

  INT32 cur_top = st_idx; 
  INT32 max_top = st_idx; 
  INT32 fxch_pos = st_idx;
  INT32 stack[MAX_SEARCH_DEPTH];
  OP* loadstack[MAX_SEARCH_DEPTH];
  
  /* init stack */
  for( int i = 0; i < inst_count; i ++ ){
    loadstack[i] = NULL;
  }
  for( int i = 0; i <= cur_top; i ++ ){
    stack[i] = i;
  }
  for( int i = cur_top+1 ; i < inst_count; i ++ ) {
    stack[i] = -1;
  }

  /* This step figures out the order of all loads */
  return Fxch_Quick_Remover_Process( op, stack, loadstack, cur_top, max_top, fxch_pos );
}
...
#ifdef ABSOFT_EXTENSIONS
   if( !Fxch_Quick_Remover( BB_last_op(bb), stack->top - from ) ){
#endif
    TN* opnd = Build_RCLASS_TN( ISA_REGISTER_CLASS_x87 );
...
#ifdef ABSOFT_EXTENSIONS
   if( !Fxch_Quick_Remover( BB_last_op(bb), stack->top - to ) ){
#endif
    TN* opnd = Build_RCLASS_TN( ISA_REGISTER_CLASS_x87 );
    Set_TN_register( opnd, stack->reg[stack->top] );
    OP* fxch_op = Mk_OP( TOP_fxch, opnd );
...
/* Put stack->reg[from] to stack->reg[to] in <bb>.
 */
static OP* SwapAt( int to, int from, OP* op, Stack* stack, BOOL insert_before )
{
  BB* bb = OP_bb( op );
  OP* tmp = NULL;

  // if insert_before is true, return the frist op
  // Otherwise return the last op
  OP* result_op = NULL; 
  ...
  if( stack->top != from ){
   ...
   if( !Fxch_Quick_Remover( BB_last_op(bb), stack->top - from ) ){
    ...
    if( insert_before ){
        BB_Insert_Op_Before( bb, op, fxch_op );
    }else{
	BB_Insert_Op_After( bb, op, fxch_op );
        tmp = fxch_op;
    }
   ...
   if( !Fxch_Quick_Remover( BB_last_op(bb), stack->top - to ) ){
	...
	BB_Insert_Op_Before( bb, op, fxch_op );
        ...
        if( tmp == NULL ){
            ...
            BB_Insert_Op_After( bb, tmp, fxch_op );
	}
    }
  ...
  if( result_op == NULL ) return op;
  else return result_op;
}



/* Insert Fxch to make sure src1 is in ST0 & src2 is in ST1 
   Fxch will be inserted into the block right prior to the current block
*/
static void Expand_Fprem( Stack* stack, OP* op )
{
  FmtAssert( OP_code(op) == TOP_fprem, ("Expand_Fprem only takes TOP_fprem operations!"));

  BB* cur_bb = OP_bb( op );
  BB* prev_bb = BB_First_Pred( cur_bb );
  if( prev_bb == cur_bb ) prev_bb = BBLIST_item(BBLIST_next(BB_preds(cur_bb)));

  /* analyze stack information */
  TN* src1 = OP_opnd( op, 0 );
  TN* src2 = OP_opnd( op, 1 );
    
  /* Search for the last x87 operation, 
     because the stack status should be not changed after this op */
  OP* def_op = NULL;
  for( OP* prev = OP_prev(op); prev != NULL; prev = OP_prev(prev) ){
    if( !OP_refs_x87(prev) )
      continue;
  ...
  /* Search the prev block */
  if( def_op == NULL ){   
     for( OP* prev = BB_last_op(prev_bb); prev != NULL; prev = OP_prev(prev) ){
	if( !OP_refs_x87(prev) )
	continue;
  ...
  /* Use the last op in the prev_bb 
   * This usually happens after x87_temp_remover in EBO
   */
  if( def_op == NULL ){
     DevWarn( "Expand_Fprem: the define op is NULL" );
     def_op = BB_last_op(prev_bb);
  }
  
  /* src2 then src1 order is for exchanging */

  // test src2
  int st_idx2 = Get_Stack_Index( stack, TN_register(src2) ); 
  if( st_idx2 != (stack->top - 1 ) ){
      def_op = SwapAt( stack->top - 1, st_idx2, def_op, stack, false );
  }   
  
  // test src1 
  int st_idx1 = Get_Stack_Index( stack, TN_register(src1) );  
  if( st_idx1 != stack->top ){
     def_op = SwapAt( stack->top, st_idx1, def_op, stack, false );
  }
  ...
  *stack->stack_in = *stack;
  ...
  for( OP* op = BB_first_op(bb); op != NULL; op = next ){
...
#ifdef ABSOFT_EXTENSIONS
      if( top == TOP_fprem ) Expand_Fprem( stack, op );
#endif
	...
	Alloc_Stack_Reg( stack, op, -1, st_idx0 );
#ifdef ABSOFT_EXTENSIONS
      } else if( top == TOP_fprem && Is_Opnd_Last_Use( stack, op, opnd1 ) ){
	  // Swap ST0, ST1
          int from = st_idx1;
	  const REGISTER reg = stack->reg[stack->top];
	  stack->reg[stack->top] = stack->reg[from];
	  ...
	  Alloc_Stack_Reg( stack, fxch_op, 0, from );
          // Pop ST0 
	  Pop_Stack( stack, stack->top, fxch_op, false );
#endif
