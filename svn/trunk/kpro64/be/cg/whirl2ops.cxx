 ...
 * $Revision: 2935 $
 * $Date: 2008-11-20 11:30:11 -0500 (Thu, 20 Nov 2008) $
 * $Author: yin $
 * $Source$
 *
 * Description:
...
#include "sections.h"		// for _SEC_INDEX_MAX
#endif
...
#ifdef ABSOFT_EXTENSIONS
static TN *
Handle_Vector_ILOADX (WN *iload, TN* result)
{

  FmtAssert (MTYPE_is_vector(WN_desc(iload)), ("Handle_Vector_ILOADX: Must be vector type"));

  TN *index_tn = Expand_Expr (WN_kid1(iload), iload, NULL);
  TN *base_tn = Expand_Expr (WN_kid0(iload), iload, NULL);
  TN *offset_tn = Gen_Literal_TN (WN_offset(iload), 4);

  TYPE_ID vector_type = WN_desc(iload);

  if (result == NULL) result = Allocate_Result_TN (iload, NULL);

  Expand_Indexed_Load ( vector_type, result, base_tn, offset_tn, index_tn, &New_OPs );
...
static void
Handle_Vector_ISTOREX (WN *istore)
{

  FmtAssert (MTYPE_is_vector(WN_desc(istore)), ("Handle_Vector_ISTOREX: Must be vector type"));

  TN *index_tn = Expand_Expr (WN_kid2(istore), istore, NULL);
  TN *base_tn = Expand_Expr (WN_kid1(istore), istore, NULL);
  TN *value_tn = Expand_Expr (WN_kid0(istore), istore, NULL);
  TN *offset_tn = Gen_Literal_TN (WN_offset(istore), 4);

  TYPE_ID vector_type = WN_desc(istore);

  Expand_Indexed_Store ( vector_type, value_tn, base_tn, offset_tn, index_tn, &New_OPs );
...
#ifdef ABSOFT_EXTENSIONS
  }else if( MTYPE_is_vector(WN_desc(compare) ) ){
        cond = Expand_Expr (compare, select, NULL);
        Exp_Select_Float_Vector_Noncondition( opcode, result, cond, trueop, falseop, &New_OPs);
#endif
  } 
#ifdef TARG_X8664 //bug 11088
...
#ifdef ABSOFT_EXTENSIONS
static TN * 
Handle_DOT (WN *dot, TN *result)
{
  TN	*op1;
  TN	*op2;
  VARIANT variant;

  op1 = Expand_Expr (WN_kid0(dot), dot, NULL);
  op2 = Expand_Expr (WN_kid1(dot), dot, NULL);

  if (result == NULL) result = Allocate_Result_TN (dot, NULL);

  variant = WN_offset(dot);
  Expand_Dot(WN_opcode(dot), result, op1, op2, variant, &New_OPs);

  return result;
}

static TN*
Handle_Round(WN* round, TN* result)
{
  TN	*op1;
  VARIANT variant;

  op1 = Expand_Expr (WN_kid0(round), round, NULL);

  if (result == NULL) result = Allocate_Result_TN (round, NULL);

  variant = WN_offset(round);
  Expand_Round(WN_opcode(round), result, op1, variant, &New_OPs);
...
Handle_ALLOCA (WN *tree, TN *result)
{
  ...
  if (!is_zero) {
...
#ifdef TARG_X64
      Exp_Chkstk( size, NULL, &New_OPs, FALSE  );
#endif
    }
    else {
	...
	tsize = tmp2;
#ifdef TARG_X64
	Exp_Chkstk( 0, tsize, &New_OPs, FALSE  );
#endif
    }
  }
...
#ifdef ABSOFT_EXTENSIONS
/* This section of code is for partitioning WHIRL into x87 & SSE2 register 
 */
static void 
PT_Switch_to_x87_Model()
{
    Target_SSE2 = FALSE;
...
static void
PT_Switch_to_SSE2_Model()
{
    Target_SSE2 = TRUE;
    Init_Mtype_RegClass_Map();
}

static BOOL use_x87 = FALSE;
static BOOL mode_switched = FALSE;

/* If it contains any x87 required operation, like FMOD, or it store to x87 registers
      it needs to be converted to x87 mode.
   if and only if, all loads are directed from memory and there are only math operations
      in the whole tree
*/

static INT32
PT_x87_Conversion_Testing( WN *expr )
{
  OPERATOR opr;
  opr = WN_operator( expr ); 
  TYPE_ID rtype = WN_rtype( expr );
  TYPE_ID dtype = WN_desc( expr );
  BOOL res0 = FALSE;
  BOOL res1 = FALSE;   

  switch( opr ){
  /* Load to a memory address*/
    case OPR_LDID:{

 	if( WN_class(expr) == CLASS_PREG ){
	  PREG_NUM preg_num = WN_load_offset(expr);
          ISA_REGISTER_CLASS rclass;
	  REGISTER reg;
	  CGTARG_Preg_Register_And_Class(preg_num, &rclass, &reg);
	  if( rclass == ISA_REGISTER_CLASS_x87 ){
		use_x87 == TRUE;
	  }else if( rclass != ISA_REGISTER_CLASS_integer ){
	    return FALSE;
	  }
    ...
    case OPR_ILOAD:{
       ...
       if(!( dtype == MTYPE_F8 || dtype == MTYPE_F4 ) ) return FALSE;

       WN *kid0 = WN_kid0(expr); /* Address */
       if( WN_operator_is( kid0, OPR_LDA )){
	 return TRUE;
       }else{
         return PT_x87_Conversion_Testing( kid0 );
       }

    }
  /* Store into x87 reg or memory location*/
    case OPR_STID:{
	
        if(!( dtype == MTYPE_F8 || dtype == MTYPE_F4 ) ) return FALSE;

	res0 = PT_x87_Conversion_Testing( WN_kid0( expr ) );
 	if( res0 == TRUE && WN_class(expr) == CLASS_PREG ){
	  PREG_NUM preg_num = WN_store_offset(expr);
          ISA_REGISTER_CLASS rclass;
	  REGISTER reg;
	  CGTARG_Preg_Register_And_Class(preg_num, &rclass, &reg);
	  if( rclass == ISA_REGISTER_CLASS_x87 ){
  ...
  /* Store to a memory address*/     
    case OPR_ISTORE:{
       ...
       res0 = PT_x87_Conversion_Testing( WN_kid0( expr ) );
       WN *kid1 = WN_kid1(expr); /* Address */
       if( WN_operator_is( kid1, OPR_LDA )){
	 res1 = TRUE;
       }else{
         res1 = PT_x87_Conversion_Testing( kid1 );
       }
       if( res0 == TRUE && res1 == TRUE ) return TRUE;
	  else return FALSE;
  ...
  /* Math */
    ...
    case OPR_MOD:{
        ...
        if(!( rtype == MTYPE_F8 || rtype == MTYPE_F4 ) ) return FALSE;
	if(!( dtype == MTYPE_F8 || dtype == MTYPE_F4 ) ) return FALSE;

	if( opr == OPR_MOD) use_x87 = TRUE;
	res0 = PT_x87_Conversion_Testing( WN_kid0( expr ) );
	res1 = PT_x87_Conversion_Testing( WN_kid1( expr ) );
        if( res0 == TRUE && res1 == TRUE ) return TRUE;
	   else return FALSE;	
    }
    case OPR_CVT:{
        if(!( rtype == MTYPE_F8 || rtype == MTYPE_F4 ) ) return FALSE;
	use_x87 = TRUE;
        return PT_x87_Conversion_Testing( WN_kid0( expr ) );
    }
    case OPR_CONST:{
        if(!( rtype == MTYPE_F8 || rtype == MTYPE_F4 ) ) return FALSE;
	return TRUE;
...
static void
PT_x87_sse2_partition_start( WN* expr )
{
   use_x87 = FALSE;
   mode_switched = FALSE;
   if(!( Is_Target_32bit() && Target_SSE2 == TRUE && CG_x87_sse2_partition == TRUE ) ) return; 
   if( CG_opt_level < 3 ) return ;

   BOOL res = PT_x87_Conversion_Testing( expr );
   if( use_x87 == TRUE && res == TRUE ){
	if( Is_Target_32bit() && Target_SSE2 == TRUE ){
		mode_switched = TRUE;
		PT_Switch_to_x87_Model();
        }
   }
...
static void
PT_x87_sse2_partition_end( )
{
   use_x87 = FALSE;
   if( mode_switched == TRUE ){
	mode_switched = FALSE;
	PT_Switch_to_SSE2_Model();
   }
}
...
Expand_Expr (WN *expr, WN *parent, TN *result)
{
  ...
  case OPR_STID:
#ifdef ABSOFT_EXTENSIONS
    PT_x87_sse2_partition_start( expr );
  ...
  case OPR_ILOADX:
    {
	// For stride simd indexed load
	if( MTYPE_is_vector( WN_desc(expr) ) ){
		return Handle_Vector_ILOADX( expr, result );
	}	
    }
  case OPR_ISTOREX:
    {
	// For stride simd indexed store
	if( MTYPE_is_vector( WN_desc(expr) ) ){
		Handle_Vector_ISTOREX( expr );
		return NULL;
	}
    }
#endif
...
#ifdef ABSOFT_EXTENSIONS
  case OPR_DOT:
    return Handle_DOT(expr, result);

  case OPR_ROUND:
    return Handle_Round(expr, result );
#endif
