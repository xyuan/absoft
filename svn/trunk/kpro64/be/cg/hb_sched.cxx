...
#ifdef ABSOFT_EXTENSIONS
  if( Clock < MIN_Clock ) MIN_Clock = Clock;
#endif
}
...
#ifdef ABSOFT_EXTENSIONS
  if ( !( Run_autopar && LNO_Run_AP > 0) &&  cur_scycle > best_scycle)  {
...
#ifdef ABSOFT_EXTENSIONS
  // Temporary disabling some reordering for 10.1 release because Channel failed 
  // under -apo -O4, I think this is channel's problem.
  // This and the change below will slow down programs a lot
  if( Run_autopar && LNO_Run_AP > 0){

    if( ( OP_store(cur_op) && OP_load( best_op )) ||
        ( OP_load( cur_op ) && OP_load( best_op ) ) ||
	( OP_store( cur_op ) && OP_store( best_op ) ) ){
      TN* cur_base = NULL;
      TN* cur_ofst = NULL;
      TN* best_base = NULL;
...
#ifdef ABSOFT_EXTENSIONS
  if( is_fwd == FALSE ) Clock = MIN_Clock; // For backward scheduling, clock may not be the min value after Add_OP_To_Sched_Vector
#endif
#ifdef KEY
...
List_Based_Fwd::Is_OP_Better (OP *cur_op, OP *best_op)
{
  ...
  if (_hbs_type & HBS_DEPTH_FIRST) {
#ifdef ABSOFT_EXTENSIONS
  // [YIN2006-02-23] [JIRA BE-134]
  /* Moved from the below to here*/
  ...
  */
  ...
  if( ( OP_load( cur_op ) && OP_load( best_op ) ) ||
	...
	return Is_Fwd_Schedule() ? cur_is_better : !cur_is_better;
        ...
        // Try to keep the original memory operation sequence
	INT cur_slack, best_slack;
	cur_slack = OPSCH_lstart(cur_opsch) - OPSCH_estart(cur_opsch);
	best_slack = OPSCH_lstart(best_opsch) - OPSCH_estart(best_opsch);
	if (cur_slack < best_slack) return TRUE;
