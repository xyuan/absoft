 ...
 * Module: cg.cxx
 * $Revision: 3034 $
 * $Date: 2009-05-28 12:57:37 -0400 (Thu, 28 May 2009) $
 * $Author: yin $
...
struct TN_DEFUSE_INFO{
  OP* def_op;
  USE_OP_NODE* use_node;
  USE_OP_NODE* redef_node;
  BOOL redef_split;
  TN* x87_tn;
...
static MEM_POOL defuse_mem_pool;

static TOP
GLOBAL_Quick_x87_Temp_Remover_Get_x87_TOP( OP* op )
{
  TOP org = OP_code(op);
  switch(org){
    case TOP_ldsd: 	return TOP_fldl;
    case TOP_ldsd_n32: 	return TOP_fldl_n32;
    case TOP_ldss: 	return TOP_flds;
    case TOP_ldss_n32: 	return TOP_flds_n32;
    case TOP_addsd:	
    case TOP_addss:	return TOP_fadd;
    case TOP_subsd:	
    case TOP_subss:	return TOP_fsub;
    case TOP_mulsd:	
    case TOP_mulss:	return TOP_fmul;
    case TOP_divsd:	
    case TOP_divss:	return TOP_fdiv;
    case TOP_movsd:	
    case TOP_movss:	
/*    case TOP_cvtsd2ss:
    case TOP_cvtss2sd: */ return TOP_fmov;
    default: 
	FmtAssert( FALSE, ("unsupported x87 code"));
  };
}

static BOOL
GLOBAL_Quick_x87_Temp_Remover_Is_Supported_TOP( OP* op )
{
  TOP org = OP_code(op);
  switch(org){
    case TOP_ldsd: 	
...
/* 
 * Recursively insert a new x87 instruction under the original instruction without testing
 * The redundent original instruction should be removed by dead code elimination
 */ 
static TN*
GLOBAL_Quick_x87_Temp_Remover_Backward_Conversion( BB* bb, OP* cur_op, TN* result_tn, TN_MAP& defuse_map )
{
  const ISA_REGISTER_CLASS cl = TN_register_class(result_tn);
  if( cl == ISA_REGISTER_CLASS_x87 ) return result_tn;
  if( cl != ISA_REGISTER_CLASS_float ) return result_tn;

  // Decide if this op need to be replaced to x87 instructions
  // Check if it is a load
  TOP top = GLOBAL_Quick_x87_Temp_Remover_Get_x87_TOP( cur_op );
  OP* new_op;
  TN* x87_tn;
  TN_DEFUSE_INFO* tninfo;
  TN_DEFUSE_INFO* opnd_tninfo;
  tninfo = (TN_DEFUSE_INFO*) TN_MAP_Get( defuse_map, result_tn );
  if( tninfo != NULL && tninfo->x87_tn != NULL ) return tninfo->x87_tn;  

  new_op =  Dup_OP( cur_op );
  if( OP_load( cur_op ) ){
    // leave node
  }else{
    // computation node
    TN* opnd;
    for( UINT i = 0; i < OP_opnds(cur_op); i ++ ){
      opnd = OP_opnd(cur_op,i);
      opnd_tninfo = (TN_DEFUSE_INFO*) TN_MAP_Get( defuse_map, opnd );
      x87_tn = GLOBAL_Quick_x87_Temp_Remover_Backward_Conversion( OP_bb(opnd_tninfo->def_op) , opnd_tninfo->def_op, OP_opnd(cur_op,i), defuse_map );
      Set_OP_opnd(new_op,i,x87_tn);
    }     
  }

  x87_tn = Build_RCLASS_TN( ISA_REGISTER_CLASS_x87 );
  OP_Change_Opcode( new_op, top );
  Set_OP_result( new_op, 0, x87_tn );
  BB_Insert_Op_After( bb, cur_op, new_op );
  if( tninfo != NULL ) tninfo->x87_tn = x87_tn;

  // Check redef_node
  if( tninfo->redef_node != NULL && tninfo->redef_split == FALSE ){
      // Insert redef op
      USE_OP_NODE* node = tninfo->redef_node;
      while( node != NULL ){
        // create a fmov
        OPS ops = OPS_EMPTY;
        Build_OP( TOP_fmov, x87_tn, node->x87_tn, &ops );
        BB_Insert_Ops_After( OP_bb(node->use_op) , node->use_op, &ops );
	node = node->next;
      }
      tninfo->redef_split = TRUE;
  }
...
/*
 * Return true if tn is x87 based or can be converted into x87 based
 * Clearly, if result_tn can be replaced, the whole tree for generating this tn can be converted to x87
 * This is the critical assumption.
 */
static BOOL
GLOBAL_Quick_x87_Temp_Remover_Backward_Testing( OP* cur_op, TN* result_tn, TN_MAP& defuse_map )
{
  if( cur_op == NULL ) return FALSE;
  if( TN_register_class(result_tn) == ISA_REGISTER_CLASS_x87 ) return TRUE;
  if( TN_register_class(result_tn) != ISA_REGISTER_CLASS_float ) return TRUE;

  // Check result_tn
  if( OP_results(cur_op) != 1 ){
    return FALSE;
  }

  // Check redef 
  TN_DEFUSE_INFO* tninfo = (TN_DEFUSE_INFO*) TN_MAP_Get( defuse_map, result_tn );
  if( tninfo == NULL ) return FALSE;
  if( tninfo->redef_node != NULL ){
     // Check if it is a load/store pair of x87_cvt_temp, we can insert a fmov after this load
     // But all redef points have to be this case
     BOOL convertable = TRUE;
     USE_OP_NODE* node = tninfo->redef_node;
     while( node != NULL && convertable == TRUE ){
	/* check if this a load of x87_cvt_temp*/
	const TOP top = OP_code( node->use_op );
	if( top == TOP_ldsd || top == TOP_ldss ){
	    TN* ofst = OP_opnd( node->use_op, 1 );
	    if( TN_is_symbol(ofst) ){
		ST* symbol = TN_var(ofst);
		if( strstr( ST_name( symbol ), "x87_cvt" ) != NULL ){
			// This is a load, now search for store
			OP* load_op = node->use_op;
			OP* store_op = NULL;
			convertable = FALSE;
			for( OP* cur_op = OP_prev( load_op ); cur_op != NULL; cur_op = OP_prev( cur_op ) ){
			   if( OP_store( cur_op ) && (OP_code(cur_op) == TOP_fstpl || OP_code(cur_op) == TOP_fstps ) ){
				ofst = OP_opnd( cur_op, 2 );
				if( TN_is_symbol( ofst ) ){
				   if( TN_var(ofst) == symbol ){
					// Found
					convertable = TRUE;
					store_op = cur_op;
					break;
			...
			if( store_op == NULL ){ 
			  ...
			  node->x87_tn = OP_opnd( store_op, 0 );
			}
	...
	node = node->next;
     ...
     if( convertable != TRUE ) return FALSE;
  }

  if( GLOBAL_Quick_x87_Temp_Remover_Is_Supported_TOP( cur_op ) == FALSE ) return FALSE;
  ...
  if( OP_load( cur_op ) ){
    ...
    for( UINT i = 0; i < OP_opnds(cur_op); i ++ ){
      opnd = OP_opnd(cur_op,i);
      tninfo = (TN_DEFUSE_INFO*) TN_MAP_Get( defuse_map, opnd );
      if( tninfo == NULL ) return FALSE;
      if( tninfo->def_op == NULL ) return FALSE;
      if( GLOBAL_Quick_x87_Temp_Remover_Backward_Testing( tninfo->def_op, OP_opnd(cur_op, i), defuse_map ) == FALSE ) return FALSE;
    }     
 ...
 */
static BOOL 
GLOBAL_Quick_x87_Temp_Remover_S2XConversion( BB*bb, ST* sym, OP* load, OP* store, BOOL keep_store, TN_MAP& defuse_map )
{
  TN* result_tn = OP_opnd(store,0);
  TN* replace_tn = NULL;
  TN_DEFUSE_INFO* tninfo = (TN_DEFUSE_INFO*) TN_MAP_Get( defuse_map, result_tn );
  if( tninfo->def_op == NULL ) return FALSE;
  if( GLOBAL_Quick_x87_Temp_Remover_Backward_Testing( tninfo->def_op, result_tn, defuse_map ) == TRUE ){
      replace_tn = GLOBAL_Quick_x87_Temp_Remover_Backward_Conversion( OP_bb(tninfo->def_op), tninfo->def_op, result_tn, defuse_map );
      // remove store if needs
      if( !keep_store ){
	BB_Remove_Op( bb, store );
      }
      // change load to copy 
      OPS ops = OPS_EMPTY;
      Build_OP( TOP_fmov, OP_result(load,0) , replace_tn, &ops );
      BB_Insert_Ops_After( bb, load, &ops );
      // Update DU information
      tninfo = (TN_DEFUSE_INFO*) TN_MAP_Get( defuse_map, OP_result(load,0) );
      tninfo->def_op = OPS_first(&ops);
      tninfo->x87_tn = replace_tn;
      BB_Remove_Op( bb, load );

      return TRUE; // returning TRUE will remove the load
  }
  return FALSE;
}

/*
 * check if there is extra define of store TN before load/store
 */
static BOOL 
GLOBAL_Quick_x87_Temp_Remover_HasInvalidDef( OP* load, OP* store )
{
  TN* tn = OP_opnd(store,0);
  BOOL found = FALSE;
  for( OP* op = load; op != store; op = OP_prev(op) )
  {
    if( op == store ) break;
...
static BOOL 
GLOBAL_Quick_x87_Temp_Remover_OP( OP* op, TN_MAP& defuse_map )
{  

    BOOL found;
    BOOL canhandle;
    BOOL keep_last_store;
    BB* bb = OP_bb( op );
    // starting from a x87 load
    if( TOP_is_x87( OP_code(op) ) && OP_load( op ) ){
	// 1. x87_cvt_temp
        // 2. the last return variable 
        const TOP top = OP_code( op );
	OP* load_op = NULL;
	OP* store_op = NULL;
 	found = FALSE;
        canhandle = FALSE;
	keep_last_store = FALSE;
	ST* symbol = NULL;
	
	
	if( top == TOP_flds || top == TOP_fldl ){
	    // step 1: identify load
	    TN* ofst = OP_opnd(op, 1 );
	    if( TN_is_symbol(ofst) ){
		symbol = TN_var(ofst);
	        load_op = op;

		// Search for the last return
		if( op == BB_last_op(bb) && BB_exit( bb ) ){
		  found = TRUE;	  
		  keep_last_store = TRUE;
		}else{
		  if( strstr( ST_name( symbol ), "x87_cvt" ) != NULL ){
		    found = TRUE;   
		  }
	    ...
	    // should be very close to the load & in the same bb
	    if( found == TRUE && load_op != NULL && symbol != NULL ){
		canhandle = FALSE;
                found = FALSE;
		for( OP* cur_op = OP_prev(op); cur_op != NULL; cur_op = OP_prev(cur_op) ){
		  if( OP_store( cur_op ) && (OP_code(cur_op) == TOP_stss || OP_code(cur_op) == TOP_stsd ) ){
                    // Search the pairing store
		    // Only one pair will be handled
		    TN* ofst = OP_opnd(cur_op,2);
		    if( TN_is_symbol(ofst) ){
			if( TN_var(ofst) == symbol ){
                            // Found
			    store_op = cur_op;
		    ...
		    // Check any use of this ST
		    for (UINT i = 0; i < OP_opnds(cur_op); i++){
			TN* opnd = OP_opnd(cur_op,i);
			if( TN_is_symbol(opnd) ){
			  if( TN_var(opnd) == symbol ){
			    found = TRUE;
			    break;
			  }
			}
		...
		if( canhandle == TRUE && GLOBAL_Quick_x87_Temp_Remover_HasInvalidDef( load_op, store_op ) == FALSE ){
		   return GLOBAL_Quick_x87_Temp_Remover_S2XConversion( bb, symbol, load_op, store_op, keep_last_store, defuse_map );
		}
	    }
	}
...
static void
GLOBAL_Quick_x87_Temp_Remover_Create_DU_Info( TN_MAP& defuse_map )
{
  ...
  for( bb = first_bb; bb != NULL; bb = BB_next(bb)){
	...
	for( i = OP_results(op) - 1; i >= 0; i-- ){
	  TN *result_tn = OP_result( op, i );
          info = (TN_DEFUSE_INFO*) TN_MAP_Get( defuse_map, result_tn);
	  if (TN_is_register(result_tn) && !TN_is_const_reg(result_tn)) {
	     if( info == NULL ){
	        info = (TN_DEFUSE_INFO*) MEM_POOL_Alloc( &defuse_mem_pool, sizeof(TN_DEFUSE_INFO) );
	        info->def_op = op;
	     	TN_MAP_Set( defuse_map, result_tn, info );
	     }else{
		node = (USE_OP_NODE*) MEM_POOL_Alloc( &defuse_mem_pool, sizeof(USE_OP_NODE) );
		node->use_op = op;
		node->next  = info->redef_node;
		info->redef_node = node;
	     }
          }
	}
     }
  ...
  for( bb = first_bb; bb != NULL; bb = BB_next(bb)){
	...
	for( i = OP_opnds(op) - 1; i >=0; i-- ){
	  ...
	  if (TN_is_register(opnd_tn) && !TN_is_const_reg(opnd_tn)) {
	     info = (TN_DEFUSE_INFO*) TN_MAP_Get( defuse_map, opnd_tn);
	     node = (USE_OP_NODE*) MEM_POOL_Alloc( &defuse_mem_pool, sizeof(USE_OP_NODE) );
	     node->use_op = op;     
	     if( info == NULL ){
		info = (TN_DEFUSE_INFO*) MEM_POOL_Alloc( &defuse_mem_pool, sizeof(TN_DEFUSE_INFO) );
		TN_MAP_Set( defuse_map, opnd_tn, info );
	     }else{
		node->next = info->use_node;
	     }
	     info->use_node = node;
          }
	}
...
static void
GLOBAL_Quick_x87_Temp_Remover_Process( TN_MAP& defuse_map )
{
  BB *first_bb = REGION_First_BB;
  OP* op;
  OP* next_op = NULL;
  ...
  for( bb = first_bb; bb != NULL; bb = BB_next(bb)){
    for( op = BB_first_op(bb); op != NULL; op = next_op ){
        next_op = OP_next(op);
	GLOBAL_Quick_x87_Temp_Remover_OP( op, defuse_map );
    }
  }
}

/* Only for 32bit with -msse2
 * Scan a single block before calling Split_BB
 * if a x87_cvt temp variable is detected or a x87 return variable is detected
 * recursively replace check the generating instructions to x87 instructions if beneficial
 */
void
GLOBAL_Quick_x87_Temp_Remover( MEM_POOL* _mem_pool )
{
   if( !(Is_Target_32bit() && Is_Target_SSE2()) ) return;
   if( CG_x87_temp_remove != TRUE ) return;
   if( CG_opt_level < 3 ) return;

   MEM_POOL_Initialize( &defuse_mem_pool, "GLOBAL_Quick_x87_Temp_Remover", TRUE );
   MEM_POOL_Push( &defuse_mem_pool );	


   TN_MAP defuse_map = TN_MAP_Create();
   /* Create global def-use map */
   GLOBAL_Quick_x87_Temp_Remover_Create_DU_Info( defuse_map ); 

   /* Analysis */
   GLOBAL_Quick_x87_Temp_Remover_Process( defuse_map );

   TN_MAP_Delete( defuse_map );
   MEM_POOL_Pop( &defuse_mem_pool );
   MEM_POOL_Delete( &defuse_mem_pool );
}
...
#ifdef ABSOFT_EXTENSIONS
  GRA_LIVE_TRIPCOUNT_NEED = TRUE;

  GLOBAL_Quick_x87_Temp_Remover( &MEM_local_region_pool );
#endif
...
//  if (flag_test_coverage)
...
#ifdef ABSOFT_EXTENSIONS
  if (CG_opt_level > 1 && CG_enable_loop_optimizations){
    GRA_LIVE_TRIPCOUNT_NEED = FALSE; // remove trip_count 
    GRA_LIVE_Recalc_Liveness(region ? REGION_get_rid( rwn) : NULL);	
    GRA_LIVE_Rename_TNs();  // rename TNs -- required by LRA
    if (Enable_CG_Peephole) { 
      Set_Error_Phase( "Extended Block Optimizer - Post Loop Optimization");
      Start_Timer( T_EBO_CU );
      EBO_Process_Region (region ? REGION_get_rid(rwn) : NULL);
      PQSCG_reinit(REGION_First_BB);
      Stop_Timer ( T_EBO_CU );
...
#ifdef TARG_X8664
  {
    ...
    /* When a function returns a structure under -m32, the value of SP will be
       increased by 4 bytes. Because gcc does this, do not remove it.
    */
    if( Is_Target_32bit() ){
