 ...
 * Module: tnutil.c
 * $Revision: 2719 $
 * $Date: 2007-06-11 10:44:11 -0400 (Mon, 11 Jun 2007) $
 * $Author: yin $
 * $Source$
 *
 * Description:
...
#ifdef ABSOFT_EXTENSIONS 
LABEL_IDX PU_PIC_Base_Label_Idx;   
#endif
...
Gen_Symbol_TN ( ST *st, INT64 offset, INT32 relocs)
{
  ...
  if( Is_Target_32bit() && Gen_PIC_Shared ){
      if( relocs == TN_RELOC_NONE ){
          ST* base_sym;
	  INT64 offset;
	  Base_Symbol_And_Offset( st, &base_sym, &offset );
          if( base_sym != SP_Sym && base_sym != FP_Sym && base_sym != Local_Spill_Sym 
	      &&!( ST_class(st) == CLASS_FUNC && !ST_is_export_local(st) )){
	      // Change to MACOSX PIC Relocation
              relocs = TN_RELOC_MACOSX32_GOT;
...
#ifdef ABSOFT_EXTENSIONS
          && TN_is_register(OP_opnd(def_op,0))
#endif	  
