...
#ifdef TARG_X64
    // With GTN(%rax), register allocation failed later because TN is set to preallocated here
    // at this time, simply pass the allocation may work
    if( OP_code(op) == TOP_chkstk && reg == REGISTER_UNDEFINED && TN_is_preallocated( tn )){	   
	continue;
    }
#endif    
...
Adjust_X86_Style_For_BB (BB* bb, BOOL* redundant_code, MEM_POOL* pool)
{
  ...
  FOR_ALL_BB_OPs( bb, op ){
    ...
    // [YIN2005-11-10] [JIRA BE-79] [JIRA BE-87] [JIRA BE-93] [JIRA BE-113]
    if( Is_Target_32bit() && Is_Target_SSE2() && !TOP_is_x87(OP_code(op)) ){
        // Check result
        if( OP_results(op) == 1  ){
		TN* result = OP_result( op, 0 );
                if( !TN_is_register(result) ) continue;
		const ISA_REGISTER_CLASS cr = TN_register_class( result );
		if( cr == ISA_REGISTER_CLASS_x87 ){
			OPS ops_copy = OPS_EMPTY;
			TN* tmp = Build_RCLASS_TN( ISA_REGISTER_CLASS_float );
			Set_TN_size( tmp, OP_result_size( op, 0 )/8 );
			Set_OP_result( op, 0, tmp );
			Exp_COPY( result, tmp, &ops_copy );
			BB_Insert_Ops_After( bb, op, &ops_copy );
		}
	}
        // Check operands
	OPS ops_copy_opnd = OPS_EMPTY;
        for( int i = 0; i < OP_opnds(op); i ++ ){
                 TN* opnd = OP_opnd( op, i );
                 if( !TN_is_register(opnd) ) continue;
                 const ISA_REGISTER_CLASS cr = TN_register_class( opnd );
                 if( cr == ISA_REGISTER_CLASS_x87 ){
			TN* tmp = Build_RCLASS_TN( ISA_REGISTER_CLASS_float );
			Set_TN_size( tmp, OP_opnd_size( op, i )/8 );
			Set_OP_opnd( op, i, tmp );
			Exp_COPY( tmp, opnd, &ops_copy_opnd );                      
		 }
	}
        BB_Insert_Ops_Before( bb, op, &ops_copy_opnd );  
    }
#endif  
...
#ifdef ABSOFT_EXTENSIONS
      // [JIRA BE-113]
      // opnd1_lr == NULL if opnd1 is replaced by x87->float copy above
      if( TNs_Are_Equivalent( result, opnd1 ) ||
	  ( opnd1_lr!=NULL && LR_last_use(opnd1_lr) == opnum ) ){
#else
      if( TNs_Are_Equivalent( result, opnd1 ) ||
...
#if Is_True_On
	      // There shouldn't be any definition of tn between op and
	      // the restore.
