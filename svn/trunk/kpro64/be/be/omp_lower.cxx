 ...
 * $Revision: 3001 $
 * $Date: 2009-04-15 10:14:42 -0400 (Wed, 15 Apr 2009) $
 * $Author: yin $
...
//  wn = Lower_Master(wn);
  ...
  if (opcode == OPC_PRAGMA &&
    ...
    if (critical_st.Elements() > 0) {
      ...
      // Do not see this is a problem
      // Parallel_do is just a do worksharing immediate following parallel construct
...
#endif
      if (Inside_Region(wn, WN_PRAGMA_PDO_BEGIN)) {
        ErrMsgSrcpos(EC_MPLOWER_Generic_Error, WN_Get_Linenum(wn),
...
ATOMIC_Lowering_Class WN_ATOMIC_STORE_Lowering_Class(WN *store)
{
  ...
  switch (store_type) {
	...
	case OPR_ADD: 
	  ...
	  if( oper == OPR_SUB ){
		// c = 8 - c 
		if( Same_Location( store, WN_kid1( operation ) ) ) alclass = ALCLASS_CRITICAL;
	  }
#endif
#endif 
          break;
