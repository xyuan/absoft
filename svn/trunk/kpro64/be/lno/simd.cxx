...
static BOOL LWN_Vectorize_Dot_In_Loop = FALSE;
#endif
...
#ifdef ABSOFT_EXTENSIONS
    if( Is_Target_SSE41() && ( rtype == MTYPE_I1 || rtype == MTYPE_I2 ) ) return TRUE;
    else
#endif
      return FALSE;
#if 0 // bug 8885
...
#ifdef ABSOFT_EXTENSIONS
static BOOL Can_Simd_Stride( WN* array )
{
   if( LNO_Simd_Stride == FALSE ) return FALSE;
   ...
   TYPE_ID rtype;
   TYPE_ID desc;
   if( WN_operator(parent) == OPR_ISTORE || WN_operator(parent) == OPR_ILOAD ){
	rtype = WN_rtype( parent );
	desc = WN_desc( parent );
	if( WN_operator(parent) == OPR_ILOAD ){
		if( rtype != desc ) return FALSE;
	}
   }else{
	return FALSE;
   }
   ...
   switch( desc ){
	...
	case MTYPE_I1:
	{
		if( Is_Target_SSE41() ) return TRUE;
			else return FALSE;
	}
   }
...
#ifdef ABSOFT_EXTENSIONS
      bool stride_mode = FALSE;
#endif      
      for (INT i = 0; i < aa->Num_Vec(); i ++) {
        av = aa->Dim(i); //for each diemnsion     
#ifdef ABSOFT_EXTENSIONS
       if( Can_Simd_Stride(wn) ){
		// if A(K,I), K is the induction variable for an outer loop,
		// But K is invariant in the inner loop, if A is the size of (3,1000)
		// I and I + 1 is still one cache line, simd should still good
		if (av->Loop_Coeff(loopno)!=0 && i != aa->Num_Vec()-1){
			if( stride_mode == TRUE ) ok = FALSE;
			else stride_mode = TRUE;
		}
		...
		if (i == aa->Num_Vec()-1 && av->Loop_Coeff(loopno) != 1 &&
			...
			if( stride_mode == TRUE && av->Loop_Coeff(loopno) == 0 ){
				// TODO: check cache line distance
			}else{
				ok = FALSE;
			}
		}
...
#ifdef ABSOFT_EXTENSIONS
      if ( WN_pragma(region_pragma) == WN_PRAGMA_CHUNKSIZE ){
	has_chunksize = TRUE;
      }
#endif
...
#ifdef ABSOFT_EXTENSIONS
    // TODO: when chunksize is multiple of vectorization number, like 8, 16, 32.
    // The loop is still vectorizable, just divide the chunksize by the vectorization number.
    // This feature will leave for the next version to implement 
    if( OPT_Enable_Rigid_Chunksize && has_chunksize ){
     sprintf(verbose_msg, "Schedule chunksize is defined in a DO region.");
     return FALSE;
    }
#endif
  }//end if
...
#ifdef ABSOFT_EXTENSIONS
      // Check if it is a dope vector 
      BOOL is_dopevector = FALSE;
      if(WN_operator(WN_array_base(array0))==OPR_LDID){
	WN *array_base = WN_array_base(array0); //may be different for store
	if( WN_has_sym(array_base) ){
	  ST* st = WN_st(array_base);
	  if( TY_kind(ST_type(st)) == KIND_STRUCT && strncmp(TY_name(ST_type(st)),".dope.",6) == 0) is_dopevector = TRUE;
	}
      }
      if(WN_operator(WN_array_base(array0))==OPR_LDID && is_dopevector == FALSE) // maybe different
#else
...
#ifdef ABSOFT_EXTENSIONS
	else if( is_dopevector == TRUE ){
	   // Upgrade to 16bit alignment for the pointer
	   Simd_Align_Load_Store( load_store, !is_store );
	}
#endif
...
#ifdef ABSOFT_EXTENSIONS
        WN_offset(WN_kid(parent, which_kid)) = vect; /* Reverse */;
#else
...
#ifdef ABSOFT_EXTENSIONS
static void Simd_Add_Shuffle_For_Negative_Coefficient(WN* simd_op, WN *loop, INT vect_type)
#else
...
#endif
{
  ...
  for (INT kid = 0; kid < WN_kid_count(simd_op); kid ++){
    ...
    if (WN_operator(opnd) == OPR_ILOAD && 
            WN_operator(WN_kid0(opnd)) == OPR_ARRAY)
#ifdef ABSOFT_EXTENSIONS
       Simd_Handle_Negative_Coefficient(simd_op, kid, WN_kid0(opnd), loop, FALSE, vect_type);
#else
...
#ifdef ABSOFT_EXTENSIONS
     Simd_Handle_Negative_Coefficient(parent,0,WN_kid1(parent), loop, no_shuffle, vect_type);
#else
...
static void Simd_Update_Copy_Array_Index(WN *copy, WN *orig, 
                                         INT add_to_base, TYPE_ID index_type, INT pos)
#else
static void Simd_Update_Copy_Array_Index(WN *copy, WN *orig, 
...
#endif
{
   ...
   OPCODE add_opc= OPCODE_make_op(OPR_ADD,index_type, MTYPE_V);
#ifdef ABSOFT_EXTENSIONS
   INT kid_for0 = WN_num_dim(copy) + pos; // pos should start from 1
#else
   ...
   // Look at WN structure for OPR_ARRAY       
#ifdef ABSOFT_EXTENSIONS
   for (INT aa_num = 0; aa_num < WN_num_dim(copy); aa_num ++){
      if( aa_num == (pos - 1) ) continue;
...
#ifdef ABSOFT_EXTENSIONS
	   WN* parent = LWN_Get_Parent(iload_copy);
	   if( WN_operator(parent) == OPR_ILOAD && WN_vector_index( parent ) > 0 ){

		Simd_Update_Copy_Array_Index(iload_copy, origA, add_to_base, index_type, WN_vector_index( parent ));
	   }else{
		Simd_Update_Copy_Array_Index(iload_copy, origA, add_to_base, index_type, WN_num_dim(iload_copy) );
	   }
#else
...
#ifdef ABSOFT_EXTENSIONS
      if(  WN_operator(copy) == OPR_ISTORE && WN_vector_index( copy ) > 0 ){
	Simd_Update_Copy_Array_Index(WN_kid1(copy), WN_kid1(istore), add_to_base, index_type, WN_vector_index( copy ) );
      }else{
	Simd_Update_Copy_Array_Index(WN_kid1(copy), WN_kid1(istore), add_to_base, index_type, WN_num_dim(WN_kid1(copy)) );
      }
#else
...
#ifdef ABSOFT_EXTENSIONS
/*
    Convert a loop with less than 3 iteration into a single dot node 
    We change the implement to create a DOT node for the whole loop, insert it after the loop
    the vecterized loop should be removed after the WOPT. So this way should work.
    At the outer level, if the return value is not NULL, the remainder loop will be removed
*/
static WN* Simd_Convert_Loop_to_Dot( WN *red_load, WN *red_store,
                                           WN *simd_op,  WN *innerloop,
                                           TYPE_ID vmtype, INT kid, WN* remainderloop)
{  
   if( !Is_Target_SSE41() ) return NULL;
   if( remainderloop == NULL ) return NULL;

   // Testing for DOT
   if( WN_operator(simd_op) != OPR_ADD ) return NULL;
   WN* wn_load = WN_kid0( simd_op );
   WN* wn_mpy = WN_kid1( simd_op );
   if( WN_operator(wn_mpy) != OPR_MPY) return NULL;
   if( wn_load != red_load ) return NULL;
   if( LWN_Get_Parent(simd_op) != red_store ) return NULL;
   if( WN_st(red_load) != WN_st(red_store) ) return NULL;
   TYPE_ID rtype = WN_rtype(wn_mpy);
   if( !(rtype == MTYPE_V16F4 || rtype == MTYPE_V16F8) ) return NULL;
   TYPE_ID desc;
   if( rtype == MTYPE_V16F4 )  desc = MTYPE_F4; else desc = MTYPE_F8;
   
   // Check loop information, at this time, the innerloop has been transformed into
   // vectorized form. So we need check
   // 1. if the innerloop will be removed later 
   // 2. if the remainderloop will sati
   INT num_iter;
   INT init_val;
   INT stop_val;
   WN* start = WN_start(innerloop);
   WN* end = WN_end(innerloop);
   if( WN_operator(WN_kid0(start)) != OPR_INTCONST ) return NULL;
   init_val = WN_const_val(WN_kid0(start));
   if( WN_operator(end) != OPR_LE ) return NULL;
   if( WN_operator(WN_kid1(end)) != OPR_INTCONST ) return NULL;
   stop_val = WN_const_val(WN_kid1(end));
   if( stop_val >= init_val ) return NULL;    // the innerloop must be within 4 iterations
   
   start = WN_start(remainderloop);
   end = WN_end(remainderloop);
   WN* step = WN_step(remainderloop);
   // Step must be 1
   if( WN_operator(WN_kid0(step)) != OPR_ADD ) return NULL;
   if( WN_operator(WN_kid1(WN_kid0(step))) != OPR_INTCONST ) return NULL;
   INT val = 0;
   val = WN_const_val(WN_kid1(WN_kid0(step)));
   if( val != 1 ) return NULL;
   // Find the interation number 
   if( WN_operator(WN_kid0(start)) != OPR_INTCONST ) return NULL;
   init_val = WN_const_val(WN_kid0(start));
   if( WN_operator(end) != OPR_LE ) return NULL;
   if( WN_operator(WN_kid1(end)) != OPR_INTCONST ) return NULL;
   stop_val = WN_const_val(WN_kid1(end));
   num_iter = stop_val - init_val + 1;
   if( num_iter <= 0 ) return NULL;
   if( desc == MTYPE_F4 && num_iter > 4 ) return NULL;
   if( desc == MTYPE_F8 && num_iter > 2 ) return NULL;

   // Create Dot
   WN* store = LWN_Copy_Tree(red_store, TRUE, LNO_Info_Map);
   LWN_Copy_Def_Use( WN_kid0(red_store), WN_kid0(store), Du_Mgr );
   simd_op = WN_kid0( store );
   wn_load = WN_kid0( simd_op );
   wn_mpy = WN_kid1( simd_op );  

   WN* wn_dot = LWN_CreateExp2( OPCODE_make_op(OPR_DOT, desc, WN_rtype(simd_op)), WN_kid0(wn_mpy), WN_kid1(wn_mpy) );
   WN_offset(wn_dot) = num_iter;
   WN_kid0(store) = wn_dot;

   WN_kid0( wn_mpy ) = NULL;
   WN_kid1( wn_mpy ) = NULL;
   LWN_Delete_Tree(simd_op); 

   LWN_Parentize(store);

   // Create initialized induction variable 
   WN* new_start = LWN_Copy_Tree(WN_start(innerloop), TRUE, LNO_Info_Map);
   LWN_Insert_Block_Before( LWN_Get_Parent(innerloop), innerloop, new_start ); 

   // I think it is better to insert before loop body
   LWN_Insert_Block_Before( LWN_Get_Parent(innerloop), innerloop, store );  

   // Update Dependence
   Du_Mgr->Add_Def_Use(store, store);
   Du_Mgr->Add_Def_Use(new_start, WN_kid0( wn_dot ));        
   Du_Mgr->Add_Def_Use(new_start, WN_kid1( wn_dot ));        
   return store;
}

/* 
    Convert ADD/MPY to DOT 

    Specification of OPR_DOT
    Kid 0 & Kid 1 is the vectorized array ILOAD for computation
    Parent must be a STID node mathcing Kid 0 
 */
static WN* Simd_Vectorize_Dot(WN *red_load, WN *red_store,
                                           WN *simd_op,  WN *innerloop,
                                           TYPE_ID vmtype, INT kid)
{
   ...
   // Create Pre-Loop Init 
   // Create a temp preg variable to temporary value, otherwise, it will add the result back to memory after every computation
   float valuefp = 0.0F; 
   double valuedp = 0.0;
   TCON tcon;
   if (desc == MTYPE_F4){
     tcon = Host_To_Targ_Float (MTYPE_F4, valuefp);
   }else{
     tcon = Host_To_Targ_Float (MTYPE_F8, valuedp);
   }
   ST* st = New_Const_Sym (Enter_tcon (tcon), Be_Type_Tbl(desc));
   WN* inv_node = WN_CreateConst (OPR_CONST, desc, MTYPE_V, st);

   // Create tmp1 to store accumulated values
   WN* tmp1 = Split_Using_Preg( red_store, red_load, adg, FALSE ); 
   WN_OFFSET tmp1_offset = WN_store_offset(tmp1);
   WN* tmp1_load = WN_kid0( simd_op );
   WN_kid0(tmp1) = inv_node; // This is pre-loop init 
   LWN_Parentize(tmp1);
   WN* tmp1_store = LWN_Copy_Tree(tmp1, TRUE, LNO_Info_Map);
   LWN_Extract_From_Block( tmp1 );
   LWN_Insert_Block_Before( LWN_Get_Parent(innerloop), innerloop, tmp1 );    
   
   // Create Dot
   WN* wn_dot = LWN_CreateExp2( OPCODE_make_op(OPR_DOT, desc, WN_rtype(simd_op)), WN_kid0(wn_mpy), WN_kid1(wn_mpy) );
   WN_offset(wn_dot) = 4;
   WN_kid0(red_store) = wn_dot;
   LWN_Parentize(red_store);

   // Split Name
   WN* tmp2 = Split_Using_Preg( red_store, wn_dot, adg, FALSE ); // tmp is the new store 
   LWN_Extract_From_Block( red_store );
      
   // Create Addition to tmp1 
   WN* wn_addtmp = LWN_CreateExp2( OPCODE_make_op(OPR_ADD, desc, MTYPE_V), tmp1_load, WN_kid0(red_store) );
   LWN_Delete_Tree(WN_kid0(tmp1_store)); // duplicated inv_node;
   WN_kid0(tmp1_store) = wn_addtmp;
   LWN_Parentize(tmp1_store);
   LWN_Insert_Block_After(WN_do_body(innerloop), tmp2, tmp1_store );

   // Create Post-Loop Merge 
   tmp1_load = LWN_Copy_Tree(tmp1_load, TRUE, LNO_Info_Map);
   WN* wn_add = LWN_CreateExp2( OPCODE_make_op(OPR_ADD, desc, MTYPE_V), red_load, tmp1_load );
   WN_kid0(red_store) = wn_add;
   LWN_Parentize( red_store );
   LWN_Insert_Block_After(LWN_Get_Parent(innerloop), innerloop, red_store );

   // Update Dependence 
   Du_Mgr->Add_Def_Use(tmp2, WN_kid1(wn_addtmp));
   Du_Mgr->Add_Def_Use(tmp1, WN_kid0(wn_addtmp));
   Du_Mgr->Add_Def_Use(tmp1_store, WN_kid0(wn_addtmp));
   Du_Mgr->Add_Def_Use(tmp1_store, WN_kid1(wn_add));
   Du_Mgr->Add_Def_Use(red_store, WN_kid0(wn_add));
...
#ifdef ABSOFT_EXTENSIONS
  if( Is_Target_SSE41() && LWN_Vectorize_Dot_In_Loop ){
    WN* res = Simd_Vectorize_Dot( red_load, red_store, simd_op, innerloop, vmtype, kid );
...
static void Simd_Vectorize_SimdOp_And_Kids(WN *simd_op, TYPE_ID vmtype, BOOL *invarkid)
{
  ...
  if (WN_operator(simd_op) != OPR_CVT && WN_operator(simd_op) != OPR_TRUNC){
    ...
    switch(WN_rtype(simd_op)) {
	  ...
	  TYPE_ID rtype = WN_rtype(simd_op);
	  TYPE_ID desc = WN_desc(simd_op);
	  if(rtype == MTYPE_I8 && desc == MTYPE_I4){
	     vec_rtype = MTYPE_V16I8;
	     vec_desc = MTYPE_V8I4;
	  }else if(rtype == MTYPE_I8 && desc == MTYPE_I2){
	     vec_rtype = MTYPE_V16I8;
	     vec_desc = MTYPE_V8I2;
	  }else if(rtype == MTYPE_I8 && desc == MTYPE_I1){
	     vec_rtype = MTYPE_V16I8;
	     vec_desc = MTYPE_V8I1;
	  }else if(rtype == MTYPE_I4 && desc == MTYPE_I2){
	     vec_rtype = MTYPE_V16I4;
	     vec_desc = MTYPE_V8I2;
	  }else if(rtype == MTYPE_I4 && desc == MTYPE_I1){
	     vec_rtype = MTYPE_V16I4;
	     vec_desc = MTYPE_V8I1;
	  }else{
		FmtAssert(FALSE, ("NYI"));
...
#ifdef ABSOFT_EXTENSIONS
void Simd_Mark_Vector_Index( WN* wn, WN* loop )
{
    if( LNO_Simd_Stride == FALSE )  return;

    if (WN_opcode(wn) == OPC_BLOCK){
      WN* kid = WN_first (wn);
      while (kid) {
        Simd_Mark_Vector_Index(kid, loop);
        kid = WN_next(kid);
      } // end while
    }// endif

    // Check the induction variable location
    if(WN_operator(wn) == OPR_ARRAY){
      ACCESS_ARRAY* aa = (ACCESS_ARRAY*)WN_MAP_Get(LNO_Info_Map, wn);
      ACCESS_VECTOR* av;
      INT loopno = Do_Loop_Depth(loop);
	...
	if( av->Loop_Coeff(loopno)!=0 && i != (aa->Num_Vec()-1) ){
	  ...
	  if(WN_operator(parent) == OPR_ILOAD || WN_operator(parent) == OPR_ISTORE ){
		WN_vector_index(parent) = (i+1);
		return;
  	  }else{
		FmtAssert( FALSE, ("Simd_Mark_Vector_Index don't support this situation so far") );
	  }
	}
      }
    }
    ...
    for (UINT kidno = 0; kidno < WN_kid_count(wn); kidno ++) {
       Simd_Mark_Vector_Index(WN_kid(wn, kidno), loop);
    }
...
//START: Vectorization Module 
  ...
  for (INT i=vec_simd_ops->Elements()-1; i >= 0; i--){
...
#ifdef ABSOFT_EXTENSIONS
    if( remainderloop != NULL) remainderloop_save = remainderloop;
#endif
...
#ifdef ABSOFT_EXTENSIONS
    INT vect_type = Simd_Unroll_Times_By_SimdKind(simd_kind); //loop unroll time
    Simd_Add_Shuffle_For_Negative_Coefficient(simd_op,innerloop,vect_type);
#else
	...
	if (WN_operator(inv_node) == OPR_REPLICATE) continue;
...
#ifdef ABSOFT_EXTENSIONS
	    invarkid[kid] = invarkid[1-kid]; // should be the same as the one copied
#else
	  ...
	  WN* dotnode = Simd_Convert_Loop_to_Dot( WN_kid(simd_op, kid), stmt,
                                       simd_op,  innerloop, vmtype, kid, remainderloop_save);
	  if( dotnode != NULL ) dot_seen = TRUE;
       }
#endif
...
#ifdef ABSOFT_EXTENSIONS
  Simd_Mark_Vector_Index( LWN_Get_Parent( simd_op ), innerloop ); // from istore
#endif
...
#ifdef ABSOFT_EXTENSIONS
  if( dot_seen && remainderloop_save != NULL ){
    WN* parent = LWN_Get_Parent(remainderloop_save);
    if( parent != NULL ){
	LWN_Extract_From_Block(remainderloop_save);
	LWN_Delete_Tree(remainderloop_save);
    }
  }
#endif
 }
