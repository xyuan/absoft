...
{  
  ...
  if (lego_tile)
    ...
    if (Is_Versioned_Mp_Loop(outer_loop))
      ...
      num_threads = Get_Runtime_Numthreads_Ldid_From_Func(outer_loop); 
#else
...
#ifdef ABSOFT_EXTENSIONS
    ? Get_Runtime_Numthreads_Ldid_From_Func(wn_loop)
#else
...
#ifdef ABSOFT_EXTENSIONS
  // Only auto parallel do the test if it is already in a parallel region
  if (is_auto_parallel) { 
  // Add a condition to determine if we are already in a parallel region. 
