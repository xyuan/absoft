...
static void Change_Du_Loop_Pointer(WN* oldptr, WN* ptr, WN* wn)
{
  OPCODE	opc = WN_opcode(wn);

  if (opc == OPC_BLOCK) {
    for (WN* w = WN_first(wn); w; w = WN_next(w))
      Change_Du_Loop_Pointer(oldptr, ptr, w);
  }
  else {
    ...
    for (INT i = 0; i < WN_kid_count(wn); i++)
      Change_Du_Loop_Pointer(oldptr, ptr, WN_kid(wn,i));
  }
}
#endif
...
#ifdef ABSOFT_EXTENSIONS
  Change_Du_Loop_Pointer(wn_loop, NULL, wn_stid );
#endif
