 ...
 * $Revision: 2911 $
 * $Date: 2008-10-01 11:32:33 -0400 (Wed, 01 Oct 2008) $
 * $Author: yin $
 * $Source$
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source$ $Revision: 2911 $";
#endif /* _KEEP_RCS_ID */
...
#ifdef ABSOFT_EXTENSIONS
//    case INTRN_F4EXPEXPR: return INTRN_F4VEXPEXPR;
//    case INTRN_F8EXPEXPR: return INTRN_F8VEXPEXPR;
#endif
...
#ifdef ABSOFT_EXTENSIONS
  // It is possible that loop_index is NULL because the loop conditions are too complicated.
  // [YIN2005-10-24] [JIRA BE-49]
  DevWarn("The upperbound of a loop doesn't contain the loop induction variable or auxilary induction variable still exists.");
  DevWarn("A loop may run forever. Please check the source code." );
...
static INT Vintrinsic_Fission(WN* innerloop)
{
  ...
  for (i=0; i<intrinsic_ops->Elements(); i++) {
    ...
    UINT kid_no;
#ifdef ABSOFT_EXTENSIONS
    // To support intrinsic functions with more than one parmeters.
    if( WN_kid_count(intrinsic_op) > 1 ){
      for (kid_no=1; kid_no<WN_kid_count(intrinsic_op); kid_no++) {
...
// should be null.
      ...
      UINT kids_num = 4;
      UINT kid_no;
      kids_num += WN_kid_count(intrinsic_op);
      WN* kids[kids_num];
      kids[0]=startx;
      kids[1]=starty;
      kids[2]=count;
      ...
      for( kid_no=1; kid_no < WN_kid_count(intrinsic_op); kid_no++ ){
           kids[ 4 + kid_no ] = LWN_Copy_Tree( WN_kid( intrinsic_op, kid_no ) );
      }
#else
      WN* kids[5];
      kids[0]=startx;
