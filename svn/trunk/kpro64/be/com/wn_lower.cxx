...
#ifdef ABSOFT_EXTENSIONS
#include <map>
// Support for AltEntry
using std::map;
struct ALTENTRY_compare{
  bool operator()(const char* s1, const char* s2) const{
     return strcmp(s1,s2) < 0;
  }
};
std::map< const char*, const WN*, ALTENTRY_compare> ALTENTRY_map;
...
static void WN_Create_MRWE_Timer_CallEx( WN *block, WN* node )
{
  if( Gen_MRWE_Timer ){
      ...
      ST * st = Gen_Intrinsic_Function(ty, "mrwe_timer");
      WN_st_idx(call) = ST_st_idx(st);
      WN_INSERT_BlockAfter(block, node, call);   
  }
}
#endif
				      ...
				      LOWER_ACTIONS actions)
{
  ...
  if (is_non_contig)
  {
     ...
     if(element_size == 2)
       stride_mult = WN_Div(rtype,stride_mult,WN_CreateIntconst(OPC_I8INTCONST,2));
#endif
...
#ifdef ABSOFT_EXTENSIONS
  if( Is_Target_SSE41() ){
     TYPE_ID kid_dst = WN_rtype( expr );
     TYPE_ID kid_src = WN_desc( expr );

     int data_type = -1;
     if( dst == MTYPE_F4 && src == MTYPE_I4 && kid_dst == MTYPE_I4 && kid_src == MTYPE_F4 ) data_type = 1;
     else if( dst == MTYPE_F8 && src == MTYPE_I4 && kid_dst == MTYPE_I4 && kid_src == MTYPE_F8 ) data_type = 2;
     else if( dst == MTYPE_V16F4 && src == MTYPE_V16I4 && kid_dst == MTYPE_V16I4 && kid_src == MTYPE_V16F4 ) data_type = 3;
     else if( dst == MTYPE_V16F8 && src == MTYPE_V16I4 && kid_dst == MTYPE_V16I4 && kid_src == MTYPE_V16F8 ) data_type = 4;
     
     int round_type = -1;
     switch( WN_operator( expr ) ){
	case OPR_FLOOR:  	round_type = 1; break;
	case OPR_TRUNC:  	round_type = 3; break;
	case OPR_CEIL:	 	round_type = 2; break;
	case OPR_CVT:	 	round_type = 0; break;
     }

     if( data_type > 0 && round_type >= 0 ){
	WN* round_wn = WN_Create( OPR_ROUND, dst, dst, 1 );
	WN_kid0(round_wn) = WN_kid0( expr );
	WN_offset(round_wn) = round_type;
...
static WN *lower_return_ldid(WN *block, WN *tree, LOWER_ACTIONS actions)
{
  ...
  switch (mtype) {
...
#ifdef TARG_X64 
    case MTYPE_C4:
      // X64 ABI: MTYPE_C4 is returned in one RAX register;
      WN_set_rtype(tree, MTYPE_I8);
      WN_set_desc(tree, MTYPE_I8);
      WN_set_ty(tree, MTYPE_To_TY(MTYPE_I8));      
      WN_st_idx(tree) = ST_st_idx(Int_Preg);
      WN_load_offset(tree) = First_Int_Preg_Return_Offset;
      return tree;	  
#endif
    ...
    case MTYPE_C4:
      // 32BIT MACOSX ABI: MTYPE_C4 is returned in one RAX register;
      if( Is_Target_32bit() ){
          WN_set_rtype(tree, MTYPE_I8);
          WN_set_desc(tree, MTYPE_I8);
          WN_set_ty(tree, MTYPE_To_TY(MTYPE_I8));      
    ...
    case MTYPE_F8:
    case MTYPE_FQ:
#if !defined(TARG_X64) && !(defined(ABSOFT_EXTENSIONS)&&defined(__APPLE__))
    case MTYPE_C4:
#endif
    case MTYPE_C8:
    case MTYPE_CQ:
...
#ifdef ABSOFT_EXTENSIONS
      if( WN_kid1(tree) ) WN_kid1(tree)= lower_expr(block, WN_kid1(tree), actions);
#endif
      kids_lowered = 	TRUE;
...
#ifdef ABSOFT_EXTENSIONS
      if( MTYPE_is_vector(WN_desc(tree)) && Action(LOWER_ARRAY) ){
	// Non-continuous vectorization, change to iloadx
        WN *addr_kid = WN_kid0(tree);
	if( WN_operator( addr_kid ) == OPR_ARRAY ){
	   TY_IDX iload_tyx = WN_ty( tree );
	   UINT32 index = WN_vector_index( tree ); 
	   WN* array = addr_kid;
	   INT32 num_dim = WN_num_dim( array );
	   // if index is not the innermost index, fortran only?
	   if( index > 0 &&  index != num_dim ){
		// create stride info
		WN* stride = NULL;
		if( WN_element_size(array) < 0 ){
			// extent is stride_multipler			
			// refer cwh_addr_fixup_nseq(cwh_addr.cxx)
			// refer Possible_Contiguous_Dope(simd.cxx)
			// real an ugly solution, we still need to 
			int real_element_size = 1;
			switch( WN_desc(tree) ){
				case MTYPE_V16F8:
				case MTYPE_V16I8:
				case MTYPE_V16I2:// don't know why, but front end give this value
				case MTYPE_V8I2: // don't know why, but front end give this value 
					real_element_size = 2; break;
				case MTYPE_V16I4:
				case MTYPE_V16F4:
				case MTYPE_V8I4:
				case MTYPE_V8F4:
				case MTYPE_V16I1:
				case MTYPE_V8I1:
					real_element_size = 1; break;
				default:
					FmtAssert( FALSE, ("WN_LOWER: Istorex conversion doesn't support this type"));
			}
			
			int shift_num = 0;
			index -- ; 
			stride = lower_copy_tree( WN_array_dim( array, index ), actions );
			shift_num ++;
			while( index < ( num_dim - 1 ) ){
				// times dimension size
				index ++;
				stride = WN_Mpy( WN_rtype( WN_array_dim(array, index) ), stride, lower_copy_tree( WN_array_dim( array, index ), actions ) );
				shift_num ++;
			}
	
			if( real_element_size == 2 ){
			   	// add a shift 
				stride = WN_Ashr( WN_rtype( WN_array_dim(array, index) ), stride, WN_Intconst( MTYPE_I4, shift_num ) );
			}
 		}else{		
			stride = lower_copy_tree( WN_array_dim( array, index ), actions );
			while( index < ( num_dim - 1 ) ){
		...
		stride = lower_expr(block, stride, actions);

		WN* kid0 = lower_expr(block, WN_kid0( tree ), actions);

		// create istorex
		WN* iloadx = WN_CreateIloadx( OPR_ILOADX, WN_rtype( tree ), WN_desc( tree), iload_tyx, WN_load_addr_ty(tree), kid0, stride );
		WN_load_offset( iloadx ) = WN_load_offset(tree);
...
#ifdef ABSOFT_EXTENSIONS
  // Complex*16 is simulated
  // [YIN2005-10-12] [JIRA BE-40]
  if( WN_desc(tree) == MTYPE_CQ || WN_desc(tree) == MTYPE_FQ ){
	return_info = Get_Return_Info(WN_ty(tree), Use_Simulated
	#ifdef TARG_X8664
						, last_call_ff2c_abi
	#endif
						);
...
#ifdef ABSOFT_EXTENSIONS
    if( Is_Target_32bit() ){
        Is_True(WN_rtype(call) == MTYPE_M || MTYPE_is_complex(WN_desc(tree)) || WN_desc(tree) == MTYPE_FQ,
	    ("call preceding MMLDID of Return_Val_Preg not type M or complex or FQ"));
    }else{
#ifdef TARG_X64
        Is_True(WN_rtype(call) == MTYPE_M || MTYPE_is_complex(WN_rtype(call)) || WN_desc(tree) == MTYPE_FQ,
...
#ifdef ABSOFT_EXTENSIONS
    // The func_stidx need to be updated to return void in case of add another fake parm in lower_call with parrallel region
    WN_Set_Call_Return_Lowered( n_call );
#endif
...
#ifdef TARG_X64
      if( mtype == MTYPE_M ){
	      // X64 ABI: Pass Structure by register 
	      switch( TY_size(WN_ty(tree)) ){
			  case 1:
			    mtype = MTYPE_I1;
			    break;
			  case 2:
			  ...
			  case 8:	
			    ...
			    break;
			  default:
			    Fail_FmtAssertion ("Unexpected MTYPE_M Size in lower_return_mstid" );
	      }
      	  desc = mtype;
          WN_set_rtype(WN_kid0(tree), mtype);
	      WN_set_desc(WN_kid0(tree), mtype);
	      WN_set_ty(WN_kid0(tree), MTYPE_To_TY(mtype));
      }
#endif
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
      if( mtype == MTYPE_M && Is_Target_32bit()){
	      // X64 ABI: Pass Structure by register 
	      switch( TY_size(WN_ty(tree)) ){
			  case 1:
			    mtype = MTYPE_I1;
...
static WN *lower_store(WN *block, WN *tree, LOWER_ACTIONS actions)
{
...
#ifdef ABSOFT_EXTENSIONS
      if( MTYPE_is_vector(WN_desc(tree)) && Action(LOWER_ARRAY) ){
	// Non-continuous vectorization, change to istorex
	if( WN_operator( addr_kid ) == OPR_ARRAY ){
	   TY_IDX istore_tyx = WN_ty( tree );
	   UINT32 index = WN_vector_index( tree );
	   WN* array = addr_kid;
	   INT32 num_dim = WN_num_dim( array );
	   // if index is not the innermost index, fortran only?
	   if( index > 0  && index != num_dim ){
		...
		// create istorex
		WN* istorex = WN_Istorex( WN_desc( tree ), istore_tyx, kid0, kid1, stride );
  ...
  case OPR_STID:
...
#ifdef ABSOFT_EXTENSIONS
    // Lower the return of a Complex*16
    // [YIN2005-10-12] [JIRA BE-40]
...
#ifdef TARG_X64
    // X64 ABI: C8 FQ CQ will be returned by the first fake parm
    if (Action(LOWER_RETURN_VAL) && WN_desc(tree) == MTYPE_C8 &&
    	WN_operator(WN_kid0(tree)) == OPR_LDID &&
	WN_st(WN_kid0(tree)) == Return_Val_Preg) {
	return lower_return_mstid(block, tree, actions);
    }
    if (Action(LOWER_RETURN_VAL) && WN_desc(tree) == MTYPE_FQ &&
    	WN_operator(WN_kid0(tree)) == OPR_LDID &&
	WN_st(WN_kid0(tree)) == Return_Val_Preg) {
	return lower_return_mstid(block, tree, actions);
    }
#endif
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
    // 32bit MACOSX ABI: C8 FQ CQ will be returned by the first fake parm
    if (Is_Target_32bit() && Action(LOWER_RETURN_VAL) && WN_desc(tree) == MTYPE_C8 &&
    	WN_operator(WN_kid0(tree)) == OPR_LDID &&
	WN_st(WN_kid0(tree)) == Return_Val_Preg) {
	return lower_return_mstid(block, tree, actions);
    }
    if (Is_Target_32bit() && Action(LOWER_RETURN_VAL) && WN_desc(tree) == MTYPE_FQ &&
    	WN_operator(WN_kid0(tree)) == OPR_LDID &&
	WN_st(WN_kid0(tree)) == Return_Val_Preg) {
	return lower_return_mstid(block, tree, actions);
    }
#endif
    // 32bit, FQ is also a structure
    if (Is_Target_32bit()&&Action(LOWER_RETURN_VAL) && WN_desc(tree) == MTYPE_FQ &&
    	WN_operator(WN_kid0(tree)) == OPR_LDID &&
	WN_st(WN_kid0(tree)) == Return_Val_Preg) {
	return lower_return_mstid(block, tree, actions);
...
#ifdef  TARG_X64
    // x64 ABI: MTYPE_C4 is returned in one RAX register
      if (WN_rtype(WN_kid0(tree)) == MTYPE_I8) {
	// the kid is rax
	...
	return tree;
      ...
      else if (WN_desc(tree) == MTYPE_C4 && WN_st(tree) == Int_Preg && 
	       WN_load_offset(tree) == First_Int_Preg_Return_Offset) {
	// the store target is rax
	ST *c4temp_st = Gen_Temp_Symbol(MTYPE_To_TY(MTYPE_I8), ".c4");
	...
	return tree;
		 ...
		 WN_load_offset(tree) == First_Int_Preg_Return_Offset ){
	/* For C/C++&Fortran under -m32, if the return value is type
	   "__complex__ float", the real part will be stored at %eax, and
	   the imag part will be stored at %edx. 
	 */
...
#ifdef TARG_X64
  if (WN_rtype(wn) == MTYPE_I8) { // store to a temp and return two halves
    ST *c4temp_st = Gen_Temp_Symbol(MTYPE_To_TY(MTYPE_I8), ".c4");
    WN *stid = WN_Stid(MTYPE_I8, 0, c4temp_st, MTYPE_To_TY(MTYPE_I8), wn);
    WN_Set_Linenum (stid, WN_Get_Linenum(tree));
    WN_INSERT_BlockLast(block, stid);
...

  {
    ...
    if (WHIRL_Return_Info_On) {
      ...
      if (RETURN_INFO_count(return_info) <= 2) {
        ...
        if( MAX_NUMBER_OF_REGISTERS_FOR_RETURN > 1 ){
    ...
    switch(type){
      ...
      case MTYPE_CQ:
      {
		// by the first fake param
		TY_IDX   ty_idx = MTYPE_To_TY(type);
		WN       *temp, *actual = WN_kid0(wn);
		...
		wn = WN_Iload(type, 0, ty_idx, temp);
		break;
      }
      case MTYPE_C4:
      {
		    ...
		    const TYPE_ID mid_type = MTYPE_I4;
		     
		    rzN = AssignExpr(block, WN_LdidPreg(mid_type, reg1), mid_type);
		    izN = AssignExpr(block, WN_LdidPreg(mid_type, reg2), mid_type);

		    ST* c4temp_st = Gen_Temp_Symbol(MTYPE_To_TY(type), ".c4");
		
		    // store the real part
		    WN *stid = WN_Stid( mid_type, 0, c4temp_st, MTYPE_To_TY(mid_type), WN_LdidPreg(mid_type, rzN) );
		    WN_Set_Linenum( stid, WN_Get_Linenum(tree) );
		    WN_INSERT_BlockLast( block, stid );
				
		    // store the imag part
		    stid = WN_Stid( mid_type, 4, c4temp_st, MTYPE_To_TY(mid_type), WN_LdidPreg(mid_type, izN) );
		    WN_Set_Linenum (stid, WN_Get_Linenum(tree));
		    WN_INSERT_BlockLast(block, stid);
		
		
		    wn = WN_Ldid( type, 0, c4temp_st, MTYPE_To_TY(type) );
	...
	wn = WN_Complex(type,
      ...
      // fall thru
		...
		// 32 bit, by the first fake param
		TY_IDX   ty_idx = MTYPE_To_TY(type);
		WN       *temp, *actual = WN_kid0(wn);
    ...
    case MTYPE_FQ: 
    {
	if( Action(LOWER_QUAD) && type == MTYPE_FQ && Is_Target_64bit() ){
		// structure type quad
		PREG_NUM high, low;
		TYPE_ID	 real =	MTYPE_F8;
		
		if (!WHIRL_Return_Info_On) 
		Get_Return_Pregs ( ty1, ty2, &reg1, &reg2 );
	
		Is_True((reg1 && reg2),
			("lower_intrinsic_op: expected structured quad return value from intrinsic_op"));
	
	
		low  = AssignExpr(block, WN_LdidPreg(real, reg1), real);
		high = AssignExpr(block, WN_LdidPreg(real, reg2), real);
	
		// Assign to a value with two pseudo-registers
		// But we only return as one register
		// another load should be expended in the later phase
		
		wn = WN_LdidPreg(type, low);
		
		break;
		
	}else if( Action(LOWER_QUAD) && type == MTYPE_FQ ){
		// 32 bit, by the first fake param
		TY_IDX   ty_idx = MTYPE_To_TY(MTYPE_FQ);
		WN       *temp, *actual = WN_kid0(wn);
		...
		wn = WN_Iload(MTYPE_FQ, 0, ty_idx, temp);
		break;
	}
    }
    // fall thru
...
#ifdef ABSOFT_EXTENSIONS
    if (WHIRL_Return_Info_On && OPCODE_is_call(WN_opcode(call)) ) {
       TYPE_ID	type = WN_rtype(tree);
#ifdef TARG_X8664
       if (Is_Target_64bit() && type ==  MTYPE_C4) 
#ifdef  TARG_X64
...
#ifdef  TARG_X64
	regST = MTYPE_To_PREG(MTYPE_I8);
	WN_set_rtype(actual, MTYPE_I8);
	WN_set_desc( actual, MTYPE_I8 );
#else
...
static WN *lower_call(WN *block, WN *tree, LOWER_ACTIONS actions)
{
  ...
  if (WHIRL_Return_Val_On && Action(LOWER_RETURN_VAL) && 
...
#ifdef ABSOFT_EXTENSIONS
      WN_Call_Return_Lowered(tree) == FALSE && 
#endif
	...
	&& (callee_st == NULL ||
#if defined(ABSOFT_EXTENSIONS)
	    (strncmp(ST_name(callee_st),"_TRANSFER",9) != 0 &&
	     strncmp(ST_name(callee_st),"_SPREAD",6) &&
	     strncmp(ST_name(callee_st),"_RESHAPE",8) &&
	     strncmp(ST_name(callee_st),"_PACK",5) &&
	     strncmp(ST_name(callee_st),"_UNPACK",7))
...
#ifdef  TARG_X64
      /* special case for X64 ABI, make a copy of an object and 
       * pass the pointer of this object into integer registers
       * TYPE: FQ, CQ, C8, M > 64bit
       */
      if ( PLOC_reg2(ploc) == 1){
	        // Create a temp variable same as the parmType 
		TYPE_ID	actual_mtype = WN_rtype(actual);
  	        TY_IDX  actual_ty    = MTYPE_To_TY(actual_mtype);
	        ST*     copyactual_st= NULL;
		char sn[31];
		sprintf( sn, "%d", i);
		copyactual_st = Gen_Temp_Symbol(actual_ty, Index_To_Str(Save_Str2(".copyforparam.", sn) ) );
	    
	        WN* awn = WN_Lda( Pointer_type, 0, copyactual_st );
	        
	        // Store the value into the temp variable
	        WN *stid = WN_CreateMstore(0, TY_pointer(actual_ty), actual, awn, WN_CreateIntconst( OPC_U4INTCONST, TY_size(actual_ty)) );
      	        WN_Set_Linenum (stid, WN_Get_Linenum(actual));
		stid = lower_mstore(block, stid, actions);
		WN_copy_linenum(tree, stid);
		WN_INSERT_BlockLast(block, stid);
		
		// Create LDA 
		WN *lda = WN_Lda( Pointer_type, 0, copyactual_st ); 
		
		// Lower the actual
		parmType = Pointer_type;
//              actual = lower_actual(callblock, lda, parmType, PLOC_reg(ploc));
		actual = lda;
        
		// Replace LDA as the actual
                if (WN_operator_is(parm, OPR_PARM)){
		   WN_kid0(parm) = lda;
		   WN_set_rtype(parm, Pointer_type);
		   WN_set_ty(parm, Make_Pointer_Type(actual_ty) );
                }else{
		   WN_actual(tree, i) = lda;		  
                }	    
    ...
    else
    {
      ...
      if ( PLOC_reg2(ploc) == 1){
		...
		copyactual_st = Gen_Temp_Symbol(actual_ty, Index_To_Str(Save_Str2(".copyforparam.", sn) ) );
	        ...
	        // Store the value into the temp variable
	        WN *stid = WN_Stid(actual_mtype, 0, copyactual_st, actual_ty, actual);
      	        WN_Set_Linenum (stid, WN_Get_Linenum(actual));
		stid = lower_store(block, stid, actions);
		WN_copy_linenum(tree, stid);
		WN_INSERT_BlockLast(block, stid);
...
#if defined(ABSOFT_EXTENSIONS) && defined(TARG_64)
    // x64 ABI, pass by an integer register
      rtype = MTYPE_I8;
#else
      rtype = MTYPE_F8;
#endif
...
static WN *lower_return_val(WN *block, WN *tree, LOWER_ACTIONS actions)
{
  ...
  if (return_mtype != MTYPE_M) {
...
#ifdef ABSOFT_EXTENSIONS
      char* func_name = ST_name( WN_st( WN_kid0(tree) ) );
      const WN* altentry_function = ALTENTRY_map[ func_name ];
      WN *first_formal = NULL;
      if( altentry_function != NULL ){
         first_formal = WN_formal(altentry_function, 0);
      }else{
         first_formal = WN_formal(current_function, 0);
      }
#else    
...
#ifdef ABSOFT_EXTENSIONS
#ifdef TARG_X64
    else if( Is_Target_32bit() && RETURN_INFO_count(return_info) == 2){
	// By EAX:EDX
	Is_True( (return_mtype == MTYPE_U8 || return_mtype == MTYPE_I8 || return_mtype == MTYPE_C4 ), ("Only types I8 U8 C4 use EAX:EDX pair for return") );
	
        mtype   = MTYPE_C4;
        preg    = RETURN_INFO_preg (return_info, 0);	
        preg_st = MTYPE_To_PREG(mtype);
    
        wn = WN_CreateStid( OPR_STID, MTYPE_V, Promoted_Mtype[mtype],  preg, preg_st, Be_Type_Tbl(mtype), WN_kid0(tree) ); 
    } 
#else
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
    else if( Is_Target_32bit() && RETURN_INFO_count(return_info) == 2){
    ...
    // Structured Quad returns via two registers
    else if( Is_Target_64bit() && return_mtype == MTYPE_FQ ){   
      Is_True(RETURN_INFO_count(return_info) == 2, ("Structured Quad expected return_info count to be 2"));     
      mtype   = MTYPE_FQ;    
      preg = RETURN_INFO_preg (return_info, 0);
      preg_st = MTYPE_To_PREG(mtype);
      wn = WN_CreateStid( OPR_STID, MTYPE_V, Promoted_Mtype[mtype],  preg, preg_st, Be_Type_Tbl(mtype), WN_kid0(tree) ); 
...
#ifdef TARG_X64
      // x64 ABI, MTYPE_CQ is passed by a single RAX register
      if( mtype == MTYPE_C4 ){ 
	  if( WN_operator( WN_kid0(tree) ) == OPR_CONST ){
	      // Convert to a I8
	      TCON i8 = Zero_I8_Tcon;
	      WN* old_const = WN_kid0(tree);
	      TCON c4 = Const_Val( old_const );
	      TCON_ty(i8) = MTYPE_I8;
	      TCON_I8(i8) = TCON_word1(c4);
	      TCON_I8(i8) = TCON_I8(i8) << 32;
	      TCON_I8(i8) += (INT32) TCON_word0(c4);
	      WN* new_const = Make_Const( i8 );
	      WN_kid0(tree) = new_const;
	      WN_Delete( old_const );  
	      mtype = MTYPE_I8;
	      preg_st = Int_Preg;  
	  }else{
...
#ifdef ABSOFT_EXTENSIONS
      // [YIN2005-11-11] [JIRA BE-86]
      char* func_name = NULL;
      if( WN_has_sym(WN_kid0(tree)) ){ // LDID
          func_name = ST_name( WN_st( WN_kid0(tree) ) );
      }else if( WN_kid0(WN_kid0(tree)) != NULL && WN_has_sym(WN_kid0(WN_kid0(tree))) ){
          func_name = ST_name( WN_st( WN_kid0(WN_kid0(tree)) ) );
      } 
#endif
      // fix rhs
	...
	for (i = 0; i < RETURN_INFO_count(return_info); i++) {
	  mtype = RETURN_INFO_mtype(return_info, i);
#ifdef TARG_X64
	  TYPE_ID return_mtype = mtype;
	  // X64 ABI: Pass Structure by register 
          switch( TY_size(WN_ty(WN_kid0(tree))) ){
           case 1:
            mtype = MTYPE_I1;
            break;
           case 2:
           ...
           case 8:       
            ...
            break;
           default:
            Fail_FmtAssertion ("Unexpected MTYPE_M Size in lower_return_val" );
          }	      
          if( Is_Target_32bit() ){
	      // If passed by two small registers (EAX:EDX)
 	     if( return_mtype < mtype ) mtype = return_mtype; 
          }
...
static WN *lower_stmt(WN *block, WN *tree, LOWER_ACTIONS actions)
{
  ...
  case OPR_GOTO_OUTER_BLOCK:
    if (Action(LOWER_TO_CG)) { 
      ...
      Fail_FmtAssertion ("OPR_GOTO_OUTER_BLOCK is not merged completed" );
#if 0
...
#ifdef ABSOFT_EXTENSIONS
  if( Action(LOWER_INTRINSIC) && Gen_MRWE_Timer ){
    for (node = WN_first(out); node; node = next_node)
    {
      next_node = WN_next(node);

      if (WN_operator_is(node, OPR_LABEL) ){
         /* insert right after a label */
	 WN_Create_MRWE_Timer_CallEx( out, node );
      }
			  ...
			  LOWER_ACTIONS actions)
{
  ...
  if (PLOC_on_stack(ploc))
  {
...
#ifdef TARG_X64
    else if ( PLOC_reg2(ploc) == 1 ){
	/* x64 ABI: pointer to a copy of object */
        ST* base = Copy_ST(sym);
        ...
        Set_ST_sclass( sym, SCLASS_FORMAL_REF );
   
    } 
#endif
  }
#ifdef TARG_X64
  else if ( PLOC_reg2(ploc) == 1 && !MTYPE_is_m(TY_mtype(Ty_Table[formalTY])) ){
	/* x64 ABI: pointer to a copy of object */
	WN* address = WN_LdidPreg( Pointer_type, PLOC_reg(ploc) ); 
	WN* value   = WN_Iload(TY_mtype(Ty_Table[formalTY]), 0, formalTY, address);
	WN* stid    = WN_Stid(TY_mtype(Ty_Table[formalTY]), 0, sym, formalTY, value);
	...
	/* x64 ABI: pointer to a copy of object */
	WN* address = WN_LdidPreg( Pointer_type, PLOC_reg(ploc) ); 
	WN* value   = WN_CreateMload( 0, TY_pointer(formalTY), address, WN_CreateIntconst( OPC_U4INTCONST, TY_size(formalTY)) );  
	WN* awn     = WN_Lda( Pointer_type, 0, sym);
        WN* stid    = WN_CreateMstore( 0, TY_pointer(formalTY), value, awn, WN_CreateIntconst( OPC_U4INTCONST, TY_size(formalTY)) );
...
static WN *lower_entry(WN *tree, LOWER_ACTIONS actions)
{
  ...
  { // create fake first parameter for struct return if needed 
    ...
    // Complex*16 is simulated
    // [YIN2005-10-12] [JIRA BE-40]
    if( TY_mtype( TY_ret_type(prototype) ) == MTYPE_CQ ){
	return_info = Get_Return_Info(TY_ret_type(prototype), Use_Simulated
	#ifdef TARG_X8664
						, PU_ff2c_abi(Pu_Table[puidx])
	#endif
						);
...
#ifdef ABSOFT_EXTENSIONS
  else if (WHIRL_Return_Val_On && Action(LOWER_RETURN_VAL)
    ...
    if (RETURN_INFO_return_via_first_arg(return_info)) {
      // Add this function into ALTENTRY_map. This map will be used 
      // in lower_return_val
      ST *return_st = Gen_Temp_Symbol(
			      Make_Pointer_Type(TY_ret_type(prototype), FALSE),
			      Index_To_Str(Save_Str2(".return.",
      ...
      // create the new altentry node
      WN *n_tree = WN_Create(OPC_ALTENTRY, WN_kid_count(tree) + 1);
      WN_st_idx(n_tree) = func_stidx;
      WN_Set_Linenum(n_tree, WN_Get_Linenum(tree));
      WN_set_map_id(n_tree, WN_map_id(tree));
      WN_kid0(n_tree) = idname;
      for (INT32 i = 0; i < WN_kid_count(tree); i++)
...
#ifdef ABSOFT_EXTENSIONS
      // Record the Altentry name
      ALTENTRY_map[ ST_name( WN_st(tree) ) ] = tree;
#endif
    }    
...
#ifdef ABSOFT_EXTENSIONS
   Lower_IO_Init(); //cilist_st has to reset
#endif
}
