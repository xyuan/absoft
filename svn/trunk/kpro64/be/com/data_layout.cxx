...
INT64 Last_bss_section_offset = 0;
BOOL Reset_bss_section_base = FALSE;
#endif
...
static void
#ifdef ABSOFT_EXTENSIONS
Assign_Offset (ST *blk, ST *base, INT32 lpad, INT32 rpad, INT32 opad=0)
#else
...
#ifdef ABSOFT_EXTENSIONS
  Set_ST_ofst(blk, ROUNDUP(STB_size(base) + lpad, align) + opad );
#else
...
static void
#ifdef ABSOFT_EXTENSIONS
Allocate_Space(ST *base, ST *blk, INT32 lpad, INT32 rpad, INT64 maxsize, INT32 opad=0)
#else
...
#endif
{
  ...
  if (!STB_decrement(base)) {
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
    if( strcmp( ST_name(base), ".bss" ) == 0 ){
    	if( Reset_bss_section_base ){
	    Last_bss_section_offset = old_offset;
	    Reset_bss_section_base = FALSE;
	}
	INT64 blk_size = ROUNDUP(old_offset + lpad - Last_bss_section_offset , align);
        Set_ST_ofst(blk, blk_size );
        Set_STB_size(base, ROUNDUP(blk_size + size + rpad, align) + Last_bss_section_offset );	
    }else{
    ...
    /* align object end */
    Set_ST_ofst(blk, ROUNDUP(old_offset + lpad, align));	
    Set_ST_ofst(blk, -(INT64) ROUNDUP(ST_ofst(blk) + size + rpad, align) - opad ); /* start */
    Set_STB_size(base, -ST_ofst(blk) - opad );
#else
    old_offset = STB_size(base);
    /* align object end */
    Set_ST_ofst(blk, ROUNDUP(old_offset + lpad, align));	
...
ST_Block_Merge (ST *block, ST *sym, INT32 lpad, INT32 rpad, INT64 maxsize)
{
  ...
  base = ST_base(sym);
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
  if( base == FP_Sym && Is_Target_32bit() ) Allocate_Space(base, sym, lpad, rpad, maxsize, 8);
  else Allocate_Space(base, sym, lpad, rpad, maxsize );
#else
...
#else
#ifdef TARG_X64
  // some nodes can becomes function call in the later phase
  // for examples, l2q, So I change to warning
  if( actual_size > Current_PU_Actual_Size ){
	  DevWarn( "size of actual area increased from %d to %d",
        Current_PU_Actual_Size, actual_size );
  }
#else
...
Max_Arg_Area_Bytes(WN *node)
{
  ...
  case OPR_IO:
    if( Is_Target_32bit() ){
      ...
      maxsize = Stack_Parameter_Offset;
    }
#endif
    break;
#endif
...
#ifdef  TARG_X64
  *upformal_size = MAX (maxsize - Formal_Save_Area_Size + Stack_Parameter_Offset, 0);
#else	
...
#ifdef TARG_X64
  if (Stack_Parameter_Offset > 0) {
	ST* upformal_block = SF_Block( SFSEG_UPFORMAL );
	FmtAssert ( upformal_block != NULL, ("Initialize_Stack_Frame: Stack_Parameter_Offset need non-NULL upformal block pointer" ) );
	
	// Add Stack_Parameter_Offset bits
	// Right before upformal area
	Set_STB_size( upformal_block, ST_size( upformal_block ) + 32 );
  }
#endif
...
INT64 Finalize_Stack_Frame (void)
{
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
    // stack aligned as 16-byte as a funciton call
    INT64 Final_Frame_Size = ROUNDUP(Frame_Size, stack_align);
    if( PUSH_FRAME_POINTER_ON_STACK ){
        Final_Frame_Size += MTYPE_byte_size(Pointer_Mtype) ;
    }
    INT64 Diff = ROUNDUP(Final_Frame_Size + MTYPE_byte_size(Pointer_Mtype), 16);
    Final_Frame_Size += Diff - Final_Frame_Size - MTYPE_byte_size(Pointer_Mtype);
    INT32 opad = Final_Frame_Size - Frame_Size - MTYPE_byte_size(Pointer_Mtype);
    if( Is_Target_64bit() ) opad = 0;
    Assign_Offset(SF_Block(SFSEG_UPFORMAL), SP_Sym, 
	(Frame_Has_Calls ? Stack_Offset_Adjustment : 0), 0, opad); 
#else
    Assign_Offset(SF_Block(SFSEG_UPFORMAL), SP_Sym, 
...
#ifdef ABSOFT_EXTENSIONS
      saved_regs_length = push_pop_int_saved_regs * MTYPE_byte_size(Pointer_Mtype);
#else
  ...
  if( Is_Target_32bit() ){
    if (Current_PU_Stack_Model == SMODEL_SMALL && PUSH_FRAME_POINTER_ON_STACK){
        INT64 Diff = ROUNDUP(Frame_Size + MTYPE_byte_size(Pointer_Mtype), 16);
        Frame_Size += Diff - Frame_Size - MTYPE_byte_size(Pointer_Mtype);
    }else if ( PUSH_FRAME_POINTER_ON_STACK ){
        INT64 Diff = ROUNDUP(Frame_Size + MTYPE_byte_size(Pointer_Mtype) + MTYPE_byte_size(Pointer_Mtype) + 8, 16);
        Frame_Size += Diff - Frame_Size - MTYPE_byte_size(Pointer_Mtype) - MTYPE_byte_size(Pointer_Mtype);     
    }else{
	FmtAssert ( TRUE, ("PUSH_FRAME_POINTER_ON_STACK should always be TRUE under MACOSX" ));
    }
  }
#endif

#ifdef ABSOFT_EXTENSIONS
  if( saved_regs_length > 0 ){
	// Adjust for saved regs to 16 bit aligned
	INT64 Diff = ROUNDUP(saved_regs_length, 16);
	Diff = Diff - saved_regs_length;
	Frame_Size += Diff;
        Set_ST_ofst(SF_Block(SFSEG_UPFORMAL), ST_ofst(SF_Block(SFSEG_UPFORMAL)) + Diff);
  }
#endif
...
#ifdef KEY
#ifdef ABSOFT_EXTENSIONS
	// .comm and .MIPS.local cannot be marged, .data
	sec = _SEC_DATA;
#else
...
#if defined(ABSOFT_EXTENSIONS) && defined(__APPLE__)
  // Init the local_initilized_flag
  ST* sym;
  ST* common_sym;
  INT j;
  FOREACH_SYMBOL (GLOBAL_SYMTAB,sym,i) {
	// emit commons here so order is preserved for datapools
    	if (ST_sclass(sym) == SCLASS_DGLOBAL) {
	    
	    if (ST_is_not_used (sym)) continue;
	    FmtAssert( ST_is_initialized (sym), ("An initialized global data is not set to be initialized"));
	    	    	    
	    FOREACH_SYMBOL (GLOBAL_SYMTAB,common_sym,j) {
		if (ST_sclass(common_sym) == SCLASS_COMMON) {
		    
		    if(ST_is_initialized_in_file (common_sym)) continue;

		    // Compare Name
		    if( ST_name(sym) && ST_name(common_sym) ){
		       if( strcmp( ST_name(sym), ST_name(common_sym) ) == 0 ){
		       	   Set_ST_is_initialized_in_file(common_sym); 
		       }
		    }else{
		       FmtAssert(FALSE, ("A common section without a name"));
		    }
      		}
	    }
    	}
