...
/* Generate call to _absoft_dopearray_binopt for Gen_MP_Reduction */
static WN*
Gen_MP_Reduction_Assumed_Array_Opt( ST* dest_st, ST* src_st, int opr )
{
    WN* call_wn = WN_Create(OPCODE_make_op( OPR_CALL, MTYPE_V, MTYPE_V), 3);
    TY_IDX ty = Make_Function_Type( MTYPE_To_TY(MTYPE_V));
    ST * st = Gen_Intrinsic_Function(ty, "_absoft_dopearray_binopt");
    WN_st_idx( call_wn ) = ST_st_idx(st);
    WN_Set_Call_Parm_Mod( call_wn );
    WN_Set_Call_Parm_Ref( call_wn );
    WN_linenum( call_wn ) = line_number;
    
    WN* dope_adr = WN_Lda( Pointer_type, 0, dest_st );
    WN_kid( call_wn, 0 ) = WN_CreateParm(Pointer_type, dope_adr, 
                       WN_ty(dope_adr), WN_PARM_BY_REFERENCE);
  
    dope_adr = WN_Lda( Pointer_type, 0, src_st );  
    WN_kid( call_wn, 1 ) = WN_CreateParm(Pointer_type, dope_adr, 
                       WN_ty(dope_adr), WN_PARM_BY_REFERENCE);

    WN_kid( call_wn, 2 ) = WN_CreateParm(MTYPE_I4, WN_Intconst ( MTYPE_I4, opr ),
                     Be_Type_Tbl(MTYPE_I4), WN_PARM_BY_VALUE); 

    return call_wn;
}

/* Generate call to _absoft_dopearray_init for Gen_MP_Reduction */
static WN*
Gen_MP_Reduction_Assumed_Array_Init( ST* dope_st, WN* init_value )
{
    WN* call_wn = WN_Create(OPCODE_make_op( OPR_CALL, MTYPE_V, MTYPE_V), 2);
    TY_IDX ty = Make_Function_Type( MTYPE_To_TY(MTYPE_V));
    ST * st = Gen_Intrinsic_Function(ty, "_absoft_dopearray_init");
    WN_st_idx( call_wn ) = ST_st_idx(st);
    WN_Set_Call_Parm_Mod( call_wn );
    WN_Set_Call_Parm_Ref( call_wn );
    WN_linenum( call_wn ) = line_number;
    
    WN* dope_adr = WN_Lda( Pointer_type, 0, dope_st );
    WN_kid( call_wn, 0 ) = WN_CreateParm(Pointer_type, dope_adr, 
                       WN_ty(dope_adr), WN_PARM_BY_REFERENCE);
    
    WN_kid( call_wn, 1 ) = WN_CreateParm(MTYPE_I4, init_value,
                     Be_Type_Tbl(MTYPE_I4), WN_PARM_BY_VALUE); 
...
static WN * 
Gen_MP_Dope_Array_Copy ( ST * from_st,  ST * to_st, WN* bytes_wn )
{
  TY_IDX ty;
  TY_IDX pty;
  WN *wn;
  ...
  ty = FLD_type(TY_fld(ST_type(from_st)));


  laddr_wn = WN_RLdid ( Promote_Type(Pointer_type), Pointer_type, 0, from_st, ty );
  saddr_wn = WN_RLdid ( Promote_Type(Pointer_type), Pointer_type, 0, to_st, ty );

  pty = ty;
  wn = WN_CreateMstore ( 0, pty,
			   WN_CreateMload ( 0, pty, laddr_wn,
					    WN_COPY_Tree (bytes_wn) ),
			   saddr_wn, WN_COPY_Tree (bytes_wn) );

  return (wn);
}

static WN*
Gen_Get_Dope_Element_Count( ST* dope_st )
{
    WN* call_wn = WN_Create(OPCODE_make_op( OPR_CALL, MTYPE_I8, MTYPE_I8), 1);
    TY_IDX ty = Make_Function_Type( MTYPE_To_TY(MTYPE_I8));
    ST * st = Gen_Intrinsic_Function(ty, "_absoft_dopearray_memory_element_count");
    WN_st_idx( call_wn ) = ST_st_idx(st);
...
extern class Verify_MP_Lowered *verify_mp_lowered_ptr; 

class OMP_Lowering_Context_switch{

public:

  WN *old_stmt_block;		/* Original statement nodes */
  WN *old_serial_stmt_block;	/* Serial statement nodes */
  WN *old_cont_nodes;		/* Statement nodes after mp code */
  WN *old_do_node;		/* Do loop node for parallel do */
  WN *old_replace_block;	/* Replacement nodes to be returned */
  WN *old_ntrip_calc;		/* Ntrip calculation code */
  WN *old_livein_block;	/* Livein code for mp call */
  WN *old_alloca_block;	/* Alloca code for mp routine */
  WN *old_copyin_block;	/* Copyin code for mp call */
  WN *old_copyout_block;	/* Copyout code for mp call */
  WN *old_firstprivate_block;  /* FIRSTPRIVATE code for mp call */
  WN *old_liveout_block;	/* Liveout code for mp call */
  WN *old_do_prefix;		/* Prefix code for do loop */
  WN *old_do_suffix;   /* Suffix code for do loop */
  WN *old_if_preamble_block;	/* MP if preamble block */
  WN *old_if_postamble_block;	/* MP if postamble block */
  WN *old_do_preamble_block;	/* Do preamble block */
  INT64 old_line_number;	/* Line number of parallel do/region */

  ST *old_parallel_proc;	/* Extracted parallel process */
  ST *old_local_start;   /* Parallel Do local start, obsolete */
  ST *old_local_ntrip;   /* Parallel Do local ntrip, obsolete */
  ST *old_thread_info;   /* Parallel Do thread info, obsolete */
  ST *old_local_upper;   /* Parallel Do local upper bound */
  ST *old_local_lower;   /* Parallel Do local lower bound */
  ST *old_local_stride;  /* Parallel Do local stride for next chunk */
  ST *old_last_iter;      /* Is local execute last iteration? */
  ST *old_local_limit;   /* Parallel Do local limit */
  ST *old_limit_st;      /* Temp var to store do_limit. can be preg. */
  WN_OFFSET old_limit_ofst;
  ST *old_local_gtid;		/* Microtask local gtid */
  ST *old_local_btid;		/* Microtask local btid */
  WN *old_base_node;		  /* Parallel do base */
  WN *old_limit_node;		/* Parallel do limit */
  WN *old_ntrip_node;		/* Parallel do trip count */
  WN *old_stride_node;		/* Parallel do stride */
  WN *old_parallel_func;	/* Parallel do function */
  FEEDBACK *old_parallel_pu_fb;  /* Feedback for parallel function */
  WN *old_reference_block;	/* Parallel funciton reference block */
  INT32 old_func_level;	/* Parallel function stab level */
  ST *old_do_index_st;		/* User do index variable ST */
  TYPE_ID old_do_index_type;	/* User do index variable type */
  BOOL old_fast_doacross;	/* Flag if doacross meets fastpath requirement*/

  INT32 old_copyin_count;	/* Count of copyins */
  INT32 old_local_count;	/* Count of lastlocals, locals, firstprivates
				   & reductions */
  INT32 old_reduction_count;	/* Count of reductions */
  INT32 old_shared_count;	/* Count of shareds, lastlocals & reductions */
  WN *old_affinity_nodes;	/* Points to (optional) affinity nodes */
  WN *old_affinity_d_nodes;	/* Points to (optional) affinity data nodes */
  WN *old_affinity_t_nodes;	/* Points to (optional) affinity thread nodes */
  WN *old_chunk_node;		/* Points to (optional) chunk node */
  WN *old_copyin_nodes;	/* Points to (optional) copyin nodes */
  WN *old_copyin_nodes_end;	/* Points to (optional) copyin nodes end */
  WN *old_if_node;		/* Points to (optional) if node */
  WN *old_lastlocal_nodes;	/* Points to (optional) lastlocal nodes */
  WN *old_lastthread_node;	/* Points to (optional) lastthread node */
  WN *old_local_nodes;		/* Points to (optional) local nodes */
  WN *old_firstprivate_nodes;	/* Points to (optional) firstprivate nodes */
                                // frontend-generated finalization code for
                                // non-POD lastlocals
  WN *old_non_pod_finalization_nodes;
                                // firstprivate and lastprivate appear on same
                                // non-POD ST
  BOOL old_non_pod_first_and_lastprivate;
  WN *old_mpnum_node;		/* Points to (optional) mpnum node */
  WN *old_mpsched_node;	/* Points to (optional) mpsched node */
  WN *old_numthreads_node;	/* Points to (optional) numthreads node */
  WN *old_ordered_node;	/* Points to (optional) ordered node */
  WN *old_do_order_lb;      /* store lb and stride of an ordered parallel */
  WN *old_do_order_stride;  /* loop, passed to omp-pdo-ordered calls */
  WN *old_reduction_nodes;	/* Points to (optional) reduction nodes */
  WN *old_shared_nodes;	/* Points to (optional) shared nodes */

//  INT32 old_num_constructs;	   /* Number of parallel constructs */
  INT32 old_nested_local_count;   /* Nested count of lastlocals, locals, &
				      firstprivates */
  INT32 old_nested_reduction_count; /* Nested count of reductions */
  WN *old_nested_affinity_nodes;    /* Points to (nested) affinity nodes */
  WN *old_nested_affinity_d_nodes;  /* Points to (nested) affin. data nodes */
  WN *old_nested_affinity_t_nodes;	/* Points to (nested) affin. thread nodes */
  WN *old_nested_chunk_node;	/* Points to (nested) chunk node */
  WN *old_nested_lastlocal_nodes; 	/* Points to (nested) lastlocal nodes */
  WN *old_nested_lastthread_node; 	/* Points to (nested) lastthread node */
  WN *old_nested_local_nodes;	/* Points to (nested) local nodes */
  WN *old_nested_firstprivate_nodes; /* Points to (nested) firstprivate
					 nodes */
  WN *old_nested_mpsched_node;	   /* Points to (nested) mpsched node */
  WN *old_nested_nowait_node;	   /* Points to (nested) nowait node */
  WN *old_nested_ordered_node;	   /* Points to (nested) ordered node */
  WN *old_nested_do_order_lb;	 /* store lb and stride of an ordered */
  WN *old_nested_do_order_stride; /* parallel loop */
  WN *old_nested_reduction_nodes; /* Points to (nested) reduction nodes */
  WN *old_nested_shared_nodes;	   /* Points to (nested) shared nodes */

  VAR_TABLE *old_var_table;	    /* Table of variable substitutions */
  VAR_TABLE *old_nested_var_table; /* Table of nested variable substitutions */

  PREG_INFO_TABLE *old_preg_info_table;   /* Table of preg information */

  LABEL_INFO_TABLE *old_label_info_table;    /* Mapping from parent to
                                                 nested PU labels */
  SHARED_TABLE *old_shared_table;  /* Table of shared ST's */
  MPID_TABLE *old_mpid_table;	    /* Table of assigned mpid's */

  INT32 old_mpid_size;	    /* Size of mpid_table */

  // Generic types for nested parallel functions
  TY_IDX old_mpdo32_ty;
  TY_IDX old_mpdo64_ty;
  TY_IDX old_mpregion_ty;
  // Generic type for parallel runtime routines
  TY_IDX old_mpruntime_ty;

  ST_IDX old_last_pu_proc_sym; // last PU we compiled
  
  INT32 old_do_id;			// Unique do number within PU
  INT32 old_region_id;		// Unique region number within PU
  INT32 old_lock_id;		// Unique lock number within PU
  WN *old_pu_chunk_node;        // (optional) PU chunk node
  WN *old_pu_mpsched_node;      // (optional) PU mpsched node

  // ST's for compiler-generated temporaries
  ST *old_mpbase_st;		/* ST for local iteration base */
  ST *old_mptrips_st;		/* ST for local iteration trips */
  ST *old_mpflags_st;		/* ST for local iteration flags */

  BOOL old_pu_has_eh;		/* Parent PU contains EH region */

  SYMTAB_IDX old_psymtab;	/* Parent symbol table */
  SYMTAB_IDX old_csymtab;	/* Child symbol table */
  PU_Info *old_ppuinfo;	/* Parent PU info structure */
  WN_MAP_TAB *old_pmaptab;	/* Parent map table */
  WN_MAP_TAB *old_cmaptab;	/* Child map table */

  BOOL old_pu_has_alloca;	/* PU contains alloca */
  BOOL old_pu_has_region;	/* PU contains region */

  BOOL old_inside_versioning_if; /* root node of tree being lowered is the
                                     MP_IF test for LNO versioning */

  // TRUE if MP region we're currently processing has the compiler-
  // generated flag set on its first pragma (the one that identifies it
  // as a PARALLEL_REGION, PDO, etc.), FALSE otherwise
  BOOL old_comp_gen_construct;
  ...
  // of mpt, set it to something appropriate for the inner construct until
  // we're done processing that construct, then restore the old value of mpt.
  MP_process_type old_mpt;

  // Type and ST for intel openmp runtime library calls
  // csc.
  ST * old_gtid_st;		  /* ST for global thread number */
  TY_IDX old_lock_ty_idx;  /* Type index for lock */
  ST *old_old_gtid_st;

  // To add unnamed_lock_st for unnamed lock and critical_lock_not_init (lg)
  ST *old_unnamed_lock_st;         /* ST for unnamed lock */
  BOOL old_critical_lock_not_init; /* for uninitialized critical lock */

  Verify_MP_Lowered *old_verify_mp_lowered_ptr; 
 
public:
     OMP_Lowering_Context_switch() {}
     ~OMP_Lowering_Context_switch() {}
...
void OMP_Lowering_Context_switch::Pop_Context()
{

   stmt_block = old_stmt_block;		/* Original statement nodes */
   serial_stmt_block = old_serial_stmt_block;	/* Serial statement nodes */
   cont_nodes = old_cont_nodes;		/* Statement nodes after mp code */
   do_node = old_do_node;		/* Do loop node for parallel do */
   replace_block = old_replace_block;	/* Replacement nodes to be returned */
   ntrip_calc = old_ntrip_calc;		/* Ntrip calculation code */
   livein_block = old_livein_block;	/* Livein code for mp call */
   alloca_block = old_alloca_block;	/* Alloca code for mp routine */
   copyin_block = old_copyin_block;	/* Copyin code for mp call */
   copyout_block = old_copyout_block;	/* Copyout code for mp call */
   firstprivate_block = old_firstprivate_block;  /* FIRSTPRIVATE code for mp call */
   liveout_block =old_liveout_block;	/* Liveout code for mp call */
   do_prefix = old_do_prefix;		/* Prefix code for do loop */
   do_suffix = old_do_suffix;   /* Suffix code for do loop */
   if_preamble_block = old_if_preamble_block;	/* MP if preamble block */
   if_postamble_block = old_if_postamble_block;	/* MP if postamble block */
   do_preamble_block =old_do_preamble_block;	/* Do preamble block */
   line_number = old_line_number;	/* Line number of parallel do/region */

   parallel_proc = old_parallel_proc;	/* Extracted parallel process */
   local_start = old_local_start;   /* Parallel Do local start, obsolete */
   local_ntrip = old_local_ntrip;   /* Parallel Do local ntrip, obsolete */
   thread_info = old_thread_info;   /* Parallel Do thread info, obsolete */
   local_upper = old_local_upper;   /* Parallel Do local upper bound */
   local_lower = old_local_lower;   /* Parallel Do local lower bound */
   local_stride = old_local_stride;  /* Parallel Do local stride for next chunk */
   last_iter = old_last_iter;      /* Is local execute last iteration? */
   local_limit = old_local_limit;   /* Parallel Do local limit */
   limit_st = old_limit_st;      /* Temp var to store do_limit. can be preg. */
   limit_ofst = old_limit_ofst;
   local_gtid = old_local_gtid;		/* Microtask local gtid */
   local_btid = old_local_btid;		/* Microtask local btid */
   base_node = old_base_node;		  /* Parallel do base */
   limit_node = old_limit_node;		/* Parallel do limit */
   ntrip_node = old_ntrip_node;		/* Parallel do trip count */
   stride_node = old_stride_node;		/* Parallel do stride */
   parallel_func = old_parallel_func;	/* Parallel do function */
   parallel_pu_fb = old_parallel_pu_fb;  /* Feedback for parallel function */
   reference_block = old_reference_block;	/* Parallel funciton reference block */
   func_level = old_func_level;	/* Parallel function stab level */
   do_index_st = old_do_index_st;		/* User do index variable  */
   do_index_type = old_do_index_type;	/* User do index variable type */
   fast_doacross = old_fast_doacross;	/* Flag if doacross meets fastpath requirement*/

   copyin_count = old_copyin_count;	/* Count of copyins */
   local_count = old_local_count;	/* Count of lastlocals, locals, firstprivates
				   & reductions */
   reduction_count = old_reduction_count;	/* Count of reductions */
   shared_count = old_shared_count;	/* Count of shareds, lastlocals & reductions */
   affinity_nodes = old_affinity_nodes;	/* Points to (optional) affinity nodes */
   affinity_d_nodes = old_affinity_d_nodes;	/* Points to (optional) affinity data nodes */
   affinity_t_nodes = old_affinity_t_nodes;	/* Points to (optional) affinity thread nodes */
   chunk_node =old_chunk_node;		/* Points to (optional) chunk node */
   copyin_nodes = old_copyin_nodes;	/* Points to (optional) copyin nodes */
   copyin_nodes_end = old_copyin_nodes_end;	/* Points to (optional) copyin nodes end */
   if_node = old_if_node;		/* Points to (optional) if node */
   lastlocal_nodes = old_lastlocal_nodes;	/* Points to (optional) lastlocal nodes */
   lastthread_node = old_lastthread_node;	/* Points to (optional) lastthread node */
   local_nodes = old_local_nodes;		/* Points to (optional) local nodes */
   firstprivate_nodes = old_firstprivate_nodes;	/* Points to (optional) firstprivate nodes */
                                // frontend-generated finalization code for
                                // non-POD lastlocals
   non_pod_finalization_nodes = old_non_pod_finalization_nodes;
                                // firstprivate and lastprivate appear on same
                                // non-POD ST
   non_pod_first_and_lastprivate = old_non_pod_first_and_lastprivate;
   mpnum_node = old_mpnum_node;		/* Points to (optional) mpnum node */
   mpsched_node = old_mpsched_node;	/* Points to (optional) mpsched node */
   numthreads_node = old_numthreads_node;	/* Points to (optional) numthreads node */
   ordered_node = old_ordered_node;	/* Points to (optional) ordered node */
   do_order_lb = old_do_order_lb;      /* store lb and stride of an ordered parallel */
   do_order_stride = old_do_order_stride;  /* loop, passed to omp-pdo-ordered calls */
   reduction_nodes = old_reduction_nodes;	/* Points to (optional) reduction nodes */
   shared_nodes = old_shared_nodes;	/* Points to (optional) shared nodes */

//   num_constructs = old_num_constructs;	   /* Number of parallel constructs */
   nested_local_count = old_nested_local_count;   /* Nested count of lastlocals, locals, &
				      firstprivates */
   nested_reduction_count = old_nested_reduction_count; /* Nested count of reductions */
   nested_affinity_nodes = old_nested_affinity_nodes;    /* Points to (nested) affinity nodes */
   nested_affinity_d_nodes = old_nested_affinity_d_nodes;  /* Points to (nested) affin. data nodes */
   nested_affinity_t_nodes = old_nested_affinity_t_nodes;	/* Points to (nested) affin. thread nodes */
   nested_chunk_node = old_nested_chunk_node;	/* Points to (nested) chunk node */
   nested_lastlocal_nodes = old_nested_lastlocal_nodes; 	/* Points to (nested) lastlocal nodes */
   nested_lastthread_node = old_nested_lastthread_node; 	/* Points to (nested) lastthread node */
   nested_local_nodes = old_nested_local_nodes;	/* Points to (nested) local nodes */
   nested_firstprivate_nodes = old_nested_firstprivate_nodes; /* Points to (nested) firstprivate
					 nodes */
   nested_mpsched_node =old_nested_mpsched_node;	   /* Points to (nested) mpsched node */
   nested_nowait_node = old_nested_nowait_node;	   /* Points to (nested) nowait node */
   nested_ordered_node = old_nested_ordered_node;	   /* Points to (nested) ordered node */
   nested_do_order_lb = old_nested_do_order_lb;	 /* store lb and stride of an ordered */
   nested_do_order_stride = old_nested_do_order_stride; /* parallel loop */
   nested_reduction_nodes = old_nested_reduction_nodes; /* Points to (nested) reduction nodes */
   nested_shared_nodes = old_nested_shared_nodes;	   /* Points to (nested) shared nodes */

  var_table = old_var_table;	    /* Table of variable substitutions */
  nested_var_table = old_nested_var_table; /* Table of nested variable substitutions */
...
//  mpid_size = old_mpid_size;	    /* Size of mpid_table */

  // Generic types for nested parallel functions
  mpdo32_ty = old_mpdo32_ty;
  mpdo64_ty = old_mpdo64_ty;
  mpregion_ty = old_mpregion_ty;
  // Generic type for parallel runtime routines
  mpruntime_ty = old_mpruntime_ty;

  last_pu_proc_sym = old_last_pu_proc_sym; // last PU we compiled
  do_id = old_do_id;			// Unique do number within PU
  region_id = old_region_id;		// Unique region number within PU
  lock_id = old_lock_id;		// Unique lock number within PU
  pu_chunk_node = old_pu_chunk_node;        // (optional) PU chunk node
  pu_mpsched_node = old_pu_mpsched_node;      // (optional) PU mpsched node

  // ST's for compiler-generated temporaries
  mpbase_st = old_mpbase_st;		/* ST for local iteration base */
  mptrips_st = old_mptrips_st;		/* ST for local iteration trips */
  mpflags_st = old_mpflags_st;		/* ST for local iteration flags */

  pu_has_eh = old_pu_has_eh;		/* Parent PU contains EH region */

  psymtab = old_psymtab;	/* Parent symbol table */
  csymtab = old_csymtab;	/* Child symbol table */
  ppuinfo = old_ppuinfo;	/* Parent PU info structure */
  pmaptab = old_pmaptab;	/* Parent map table */
  cmaptab = old_cmaptab;	/* Child map table */

  pu_has_alloca = old_pu_has_alloca;	/* PU contains alloca */
  pu_has_region = old_pu_has_region;	/* PU contains region */

  inside_versioning_if = old_inside_versioning_if; /* root node of tree being lowered is the
                                     MP_IF test for LNO versioning */

  // TRUE if MP region we're currently processing has the compiler-
  // generated flag set on its first pragma (the one that identifies it
  // as a PARALLEL_REGION, PDO, etc.), FALSE otherwise
  comp_gen_construct = old_comp_gen_construct;
  ...
  // Type and ST for intel openmp runtime library calls
  // csc.
  gtid_st =  old_gtid_st;		  /* ST for global thread number */
  lock_ty_idx = old_lock_ty_idx;  /* Type index for lock */
  gtid_st = old_old_gtid_st;

  // To add unnamed_lock_st for unnamed lock and critical_lock_not_init (lg)
  unnamed_lock_st = old_unnamed_lock_st;         /* ST for unnamed lock */
  critical_lock_not_init = old_critical_lock_not_init; /* for uninitialized critical lock */

  verify_mp_lowered_ptr = old_verify_mp_lowered_ptr;
}

void OMP_Lowering_Context_switch::Push_Context()
{
   old_stmt_block = stmt_block;		/* Original statement nodes */
   old_serial_stmt_block = serial_stmt_block;	/* Serial statement nodes */
   old_cont_nodes = cont_nodes;		/* Statement nodes after mp code */
   old_do_node = do_node;		/* Do loop node for parallel do */
   old_replace_block = replace_block;	/* Replacement nodes to be returned */
   old_ntrip_calc = ntrip_calc;		/* Ntrip calculation code */
   old_livein_block = livein_block;	/* Livein code for mp call */
   old_alloca_block = alloca_block;	/* Alloca code for mp routine */
   old_copyin_block = copyin_block;	/* Copyin code for mp call */
   old_copyout_block = copyout_block;	/* Copyout code for mp call */
   old_firstprivate_block = firstprivate_block;  /* FIRSTPRIVATE code for mp call */
   old_liveout_block = liveout_block;	/* Liveout code for mp call */
   old_do_prefix = do_prefix;		/* Prefix code for do loop */
   old_do_suffix = do_suffix;   /* Suffix code for do loop */
   old_if_preamble_block = if_preamble_block;	/* MP if preamble block */
   old_if_postamble_block = if_postamble_block;	/* MP if postamble block */
   old_do_preamble_block = do_preamble_block;	/* Do preamble block */
   old_line_number = line_number;	/* Line number of parallel do/region */

   old_parallel_proc = parallel_proc;	/* Extracted parallel process */
   old_local_start = local_start;   /* Parallel Do local start, obsolete */
   old_local_ntrip = local_ntrip;   /* Parallel Do local ntrip, obsolete */
   old_thread_info = thread_info;   /* Parallel Do thread info, obsolete */
   old_local_upper = local_upper;   /* Parallel Do local upper bound */
   old_local_lower = local_lower;   /* Parallel Do local lower bound */
   old_local_stride = local_stride;  /* Parallel Do local stride for next chunk */
   old_last_iter = last_iter;      /* Is local execute last iteration? */
   old_local_limit = local_limit;   /* Parallel Do local limit */
   old_limit_st = limit_st;      /* Temp var to store do_limit. can be preg. */
   old_limit_ofst = limit_ofst;
   old_local_gtid = local_gtid;		/* Microtask local gtid */
   old_local_btid = local_btid;		/* Microtask local btid */
   old_base_node = base_node;		  /* Parallel do base */
   old_limit_node = limit_node;		/* Parallel do limit */
   old_ntrip_node = ntrip_node;		/* Parallel do trip count */
   old_stride_node = stride_node;		/* Parallel do stride */
   old_parallel_func = parallel_func;	/* Parallel do function */
   old_parallel_pu_fb = parallel_pu_fb;  /* Feedback for parallel function */
   old_reference_block = reference_block;	/* Parallel funciton reference block */
   old_func_level = func_level;	/* Parallel function stab level */
   old_do_index_st = do_index_st;		/* User do index variable  */
   old_do_index_type = do_index_type;	/* User do index variable type */
   old_fast_doacross = fast_doacross;	/* Flag if doacross meets fastpath requirement*/

   old_copyin_count = copyin_count;	/* Count of copyins */
   old_local_count = local_count;	/* Count of lastlocals, locals, firstprivates
				   & reductions */
   old_reduction_count = reduction_count;	/* Count of reductions */
   old_shared_count = shared_count;	/* Count of shareds, lastlocals & reductions */
   old_affinity_nodes = affinity_nodes;	/* Points to (optional) affinity nodes */
   old_affinity_d_nodes = affinity_d_nodes;	/* Points to (optional) affinity data nodes */
   old_affinity_t_nodes = affinity_t_nodes;	/* Points to (optional) affinity thread nodes */
   old_chunk_node = chunk_node;		/* Points to (optional) chunk node */
   old_copyin_nodes = copyin_nodes;	/* Points to (optional) copyin nodes */
   old_copyin_nodes_end = copyin_nodes_end;	/* Points to (optional) copyin nodes end */
   old_if_node = if_node;		/* Points to (optional) if node */
   old_lastlocal_nodes = lastlocal_nodes;	/* Points to (optional) lastlocal nodes */
   old_lastthread_node = lastthread_node;	/* Points to (optional) lastthread node */
   old_local_nodes = local_nodes;		/* Points to (optional) local nodes */
   old_firstprivate_nodes = firstprivate_nodes;	/* Points to (optional) firstprivate nodes */
                                // frontend-generated finalization code for
                                // non-POD lastlocals
   old_non_pod_finalization_nodes = non_pod_finalization_nodes;
                                // firstprivate and lastprivate appear on same
                                // non-POD ST
   old_non_pod_first_and_lastprivate = non_pod_first_and_lastprivate;
   old_mpnum_node = mpnum_node;		/* Points to (optional) mpnum node */
   old_mpsched_node = mpsched_node;	/* Points to (optional) mpsched node */
   old_numthreads_node = numthreads_node;	/* Points to (optional) numthreads node */
   old_ordered_node = ordered_node;	/* Points to (optional) ordered node */
   old_do_order_lb = do_order_lb;      /* store lb and stride of an ordered parallel */
   old_do_order_stride = do_order_stride;  /* loop, passed to omp-pdo-ordered calls */
   old_reduction_nodes = reduction_nodes;	/* Points to (optional) reduction nodes */
   old_shared_nodes = shared_nodes;	/* Points to (optional) shared nodes */

//   old_num_constructs = num_constructs;	   /* Number of parallel constructs */
   old_nested_local_count = nested_local_count;   /* Nested count of lastlocals, locals, &
				      firstprivates */
   old_nested_reduction_count = nested_reduction_count; /* Nested count of reductions */
   old_nested_affinity_nodes = nested_affinity_nodes;    /* Points to (nested) affinity nodes */
   old_nested_affinity_d_nodes = nested_affinity_d_nodes;  /* Points to (nested) affin. data nodes */
   old_nested_affinity_t_nodes = nested_affinity_t_nodes;	/* Points to (nested) affin. thread nodes */
   old_nested_chunk_node = nested_chunk_node;	/* Points to (nested) chunk node */
   old_nested_lastlocal_nodes = nested_lastlocal_nodes; 	/* Points to (nested) lastlocal nodes */
   old_nested_lastthread_node = nested_lastthread_node; 	/* Points to (nested) lastthread node */
   old_nested_local_nodes = nested_local_nodes;	/* Points to (nested) local nodes */
   old_nested_firstprivate_nodes = nested_firstprivate_nodes; /* Points to (nested) firstprivate
					 nodes */
   old_nested_mpsched_node = nested_mpsched_node;	   /* Points to (nested) mpsched node */
   old_nested_nowait_node = nested_nowait_node;	   /* Points to (nested) nowait node */
   old_nested_ordered_node = nested_ordered_node;	   /* Points to (nested) ordered node */
   old_nested_do_order_lb = nested_do_order_lb;	 /* store lb and stride of an ordered */
   old_nested_do_order_stride = nested_do_order_stride; /* parallel loop */
   old_nested_reduction_nodes = nested_reduction_nodes; /* Points to (nested) reduction nodes */
   old_nested_shared_nodes = nested_shared_nodes;	   /* Points to (nested) shared nodes */

  old_var_table = var_table;	    /* Table of variable substitutions */
  old_nested_var_table = nested_var_table; /* Table of nested variable substitutions */

  old_preg_info_table = preg_info_table;   /* Table of preg information */

  old_label_info_table = label_info_table;    /* Mapping from parent to
                                                 nested PU labels */
  old_shared_table = shared_table;  /* Table of shared ST's */
//  old_mpid_table = mpid_table;	    /* Table of assigned mpid's */

//  old_mpid_size = mpid_size;	    /* Size of mpid_table */

  // Generic types for nested parallel functions
  old_mpdo32_ty = mpdo32_ty;
  old_mpdo64_ty = mpdo64_ty;
  old_mpregion_ty = mpregion_ty;
  // Generic type for parallel runtime routines
  old_mpruntime_ty = mpruntime_ty;

  old_last_pu_proc_sym = last_pu_proc_sym; // last PU we compiled
  old_do_id = do_id;			// Unique do number within PU
  old_region_id = region_id;		// Unique region number within PU
  old_lock_id = lock_id;		// Unique lock number within PU
  old_pu_chunk_node = pu_chunk_node;        // (optional) PU chunk node
  old_pu_mpsched_node = pu_mpsched_node;      // (optional) PU mpsched node

  // ST's for compiler-generated temporaries
  old_mpbase_st = mpbase_st;		/* ST for local iteration base */
  old_mptrips_st = mptrips_st;		/* ST for local iteration trips */
  old_mpflags_st = mpflags_st;		/* ST for local iteration flags */

  old_pu_has_eh = pu_has_eh;		/* Parent PU contains EH region */

  old_psymtab = psymtab;	/* Parent symbol table */
  old_csymtab = csymtab;	/* Child symbol table */
  old_ppuinfo = ppuinfo;	/* Parent PU info structure */
  old_pmaptab = pmaptab;	/* Parent map table */
  old_cmaptab = cmaptab;	/* Child map table */

  old_pu_has_alloca = pu_has_alloca;	/* PU contains alloca */
  old_pu_has_region = pu_has_region;	/* PU contains region */

  old_inside_versioning_if = inside_versioning_if; /* root node of tree being lowered is the
                                     MP_IF test for LNO versioning */

  // TRUE if MP region we're currently processing has the compiler-
  // generated flag set on its first pragma (the one that identifies it
  // as a PARALLEL_REGION, PDO, etc.), FALSE otherwise
  old_comp_gen_construct = comp_gen_construct;
  ...
  // Type and ST for intel openmp runtime library calls
  // csc.
  old_gtid_st = gtid_st;		  /* ST for global thread number */
  old_lock_ty_idx = lock_ty_idx;  /* Type index for lock */
  old_old_gtid_st = old_gtid_st;

  // To add unnamed_lock_st for unnamed lock and critical_lock_not_init (lg)
  old_unnamed_lock_st = unnamed_lock_st;         /* ST for unnamed lock */
  old_critical_lock_not_init = critical_lock_not_init; /* for uninitialized critical lock */

  old_verify_mp_lowered_ptr = verify_mp_lowered_ptr;
}

void OMP_Lowering_Context_switch::Init_Context()
{
  local_upper = NULL;   /* Parallel Do local upper bound */
  local_lower = NULL;   /* Parallel Do local lower bound */
  local_stride = NULL;  /* Parallel Do local stride for next chunk */
  do_order_lb = NULL; /* store lb and stride of an ordered parallel */
  do_order_stride = NULL; /* loop, passed to omp-pdo-ordered calls */
  nested_do_order_lb = NULL; /* store lb and stride of an ordered */
  nested_do_order_stride = NULL; /* parallel loop */
//  mpid_size = 0;	    /* Size of mpid_table */

  // Generic types for nested parallel functions
  mpdo32_ty =  TY_IDX_ZERO;
  mpdo64_ty =  TY_IDX_ZERO;
  mpregion_ty =  TY_IDX_ZERO;
  
  // Generic type for parallel runtime routines
  mpruntime_ty =  TY_IDX_ZERO;
//  last_pu_proc_sym = ST_IDX_ZERO; // last PU we compiled
  do_id = 0;			// Unique do number within PU
  region_id = 0;		// Unique region number within PU
  lock_id = 0;		// Unique lock number within PU
  pu_chunk_node = NULL;        // (optional) PU chunk node
  pu_mpsched_node = NULL;      // (optional) PU mpsched node
  
  // ST's for compiler-generated temporaries
  mpbase_st = NULL;		/* ST for local iteration base */
  mptrips_st = NULL;		/* ST for local iteration trips */
  mpflags_st = NULL;		/* ST for local iteration flags */

  pu_has_eh = FALSE;		/* Parent PU contains EH region */
  gtid_st = NULL;		  /* ST for global thread number */
  lock_ty_idx =  TY_IDX_ZERO;  /* Type index for lock */
  old_gtid_st = NULL;

  // To add unnamed_lock_st for unnamed lock and critical_lock_not_init (lg)
  unnamed_lock_st = NULL;         /* ST for unnamed lock */
  critical_lock_not_init = TRUE; /* for uninitialized critical lock */
...
#ifdef ABSOFT_EXTENSIONS
    if (WN_pragma(wn) != WN_PRAGMA_CRITICAL_SECTION_BEGIN && 
	WN_pragma(wn) != WN_PRAGMA_CRITICAL_SECTION_END
	) // Ignore CRITICAL_SECTION
#endif
...
#ifdef ABSOFT_EXTENSIONS
static WN* 
Get_First_Stmt_in_Altentry(WN *first)
{
...
#ifdef ABSOFT_EXTENSIONS
    WN *thrprv_test = thrprv_block;
    if( OPT_Enable_Risky_Thdprv ){
       ...
       thrprv_test = WN_CreateIf( test, thrprv_block, WN_CreateBlock());	
    }
#else 
    WN *test = WN_CIOR(WN_EQ(MTYPE_I4,
                             Gen_MP_Load( ST_ptr(WN_pragma_arg2(prags)), return_ofst ),
...
#ifdef ABSOFT_EXTENSIONS
    if (prepend == TRUE) {
	// insert thdprv function for all altentry
	// must insert before inserting into main block
	for (wn = WN_first( block ); wn; wn = WN_next( wn )) {
	   if( WN_operator( wn ) == OPR_ALTENTRY ){
		// move the right position 
		wn = Get_First_Stmt_in_Altentry( wn );
		WN_INSERT_BlockBefore(block, wn, WN_COPY_Tree( thrprv_test ) );
		WN_INSERT_BlockBefore(block, wn, WN_COPY_Tree( thrprv_assign ) );	   
	   }
	}
    }
#endif
...
#ifdef ABSOFT_EXTENSIONS
    if (prepend == TRUE) {
	// insert gdi load  for all altentry
	// must insert before inserting the main block
	WN* old_wn = wn;
	for (wn = WN_first( block ); wn; wn = WN_next( wn )) {
	    if( WN_operator( wn ) == OPR_ALTENTRY ){
		// move the right position 
		wn = Get_First_Stmt_in_Altentry( wn );
		WN_INSERT_BlockBefore(block, wn,  Gen_Get_Thread_Num() );
		WN_INSERT_BlockBefore(block, wn, WN_COPY_Tree( wnx ) );	   
	    }
	}
	wn = old_wn;
    }
...
#ifdef ABSOFT_EXTENSIONS
/*  Calculate element count of a dimension of a dynamic array (in numbers).  */

static WN * 
Calculate_Array_Dim_Count ( ST * st, TY_IDX ty  ,INT32 dimension )
{
  INT32 i;
  WN   *wn = NULL;
  WN   *lower = NULL;
...
#ifdef ABSOFT_EXTENSIONS
  // array of arrays, recursively trace back
  TY_IDX ety = TY_AR_etype(ty); 
  if( TY_size(ety) == 0 && TY_kind(ety) == KIND_ARRAY ){
	wn = Calculate_Array_Size( st, ety );
  }
#endif 
...
// Bug 3795
      ...
      // altry entry return variable need disable this line
      return;
#endif
    }
    break;
  ...
  v->is_dynamic_array = (((vtype == VAR_LOCAL) || (vtype == VAR_LASTLOCAL) ||
			 ...
			 TY_All_Bounds_Defined(TY_pointed(
			                         ST_type(old_st)) ) );
#ifdef ABSOFT_EXTENSIONS
  v->is_assumed_array = (((vtype == VAR_LOCAL) || (vtype == VAR_LASTLOCAL) ||
			  (vtype == VAR_FIRSTPRIVATE) || (vtype == VAR_REDUCTION_ARRAY) 
			   || (vtype == VAR_REDUCTION_ARRAY_OMP ) )&&  // Local
			 ST_Has_Dope_Vector(old_st)  && // dope
                         (ST_keep_name_w2f(old_st) || 
                          PU_src_lang(Get_Current_PU()) == PU_F77_LANG ||
                          PU_src_lang(Get_Current_PU()) == PU_F90_LANG) && // fortran 
			 (ST_sclass(old_st) == SCLASS_FORMAL_REF) );  // dummy arg  
  v->is_automatic_array = (((vtype == VAR_LOCAL) || (vtype == VAR_LASTLOCAL) ||
			  (vtype == VAR_FIRSTPRIVATE) || (vtype == VAR_REDUCTION_ARRAY) 
			   || (vtype == VAR_REDUCTION_ARRAY_OMP ) )&&  // Local
			   (ST_sclass(old_st) == SCLASS_AUTO) && // auto 
			   (TY_kind(ST_type(old_st)) == KIND_ARRAY) && // array 
			   (ST_base(old_st) != NULL && ST_base(old_st) != old_st ) && // has a base
			    (ST_sclass(ST_base(old_st)) == SCLASS_AUTO) && // base is auto 
			    (TY_kind(ST_type(ST_base(old_st))) == KIND_POINTER ) && // base is pointer
                         (ST_keep_name_w2f(old_st) || 
                          PU_src_lang(Get_Current_PU()) == PU_F77_LANG ||
                          PU_src_lang(Get_Current_PU()) == PU_F90_LANG) ); // fortran 
#endif
  ...
  if (prev_def) {
    ...
    if ((TY_kind(ty) == KIND_ARRAY) && (vtype == VAR_REDUCTION_ARRAY))
      ty = TY_etype(ty);
#ifndef ABSOFT_EXTENSIONS
  // As the comments, it does not work
...
#ifdef ABSOFT_EXTENSIONS    
    if ((vtype == VAR_REDUCTION_ARRAY_OMP) 
...
#ifdef ABSOFT_EXTENSIONS
      // automatic array, the base has set to P_symbolname, 
      // no_need for copying again
      !v->is_non_pod && !v->is_automatic_array) {   // fecc generates constructor calls
#else
...
#ifdef ABSOFT_EXTENSIONS
  // code above only clone the dope struct
  // now a new memory is allocated and the data in the old_dope 
  // is copied into the new memory block 
  if (v->is_assumed_array &&
      vtype == VAR_FIRSTPRIVATE && firstprivate_blockp &&
      !v->is_non_pod ){
    if (*firstprivate_blockp == NULL)
      *firstprivate_blockp = WN_CreateBlock ( );

      TYPE_ID el_len_mtype = MTYPE_I8;
      int el_len_offset = 8;
      if( Is_Target_32bit() ){
	 el_len_mtype = MTYPE_I4;
	 el_len_offset = 4;
      }

      // Use _SIZE 
      ST* element_count = NULL;

      Create_Temp( el_len_mtype, "dope_element_count", &element_count );

      WN* call = Gen_Get_Dope_Element_Count( old_st );
      WN_INSERT_BlockLast( *firstprivate_blockp, call );
      PREG_NUM   rreg1, rreg2;
      GET_RETURN_PREGS(rreg1, rreg2, el_len_mtype);
      WN* return_wn = WN_Stid( el_len_mtype, 0, element_count, ST_type( element_count ),
             WN_LdidPreg ( el_len_mtype, rreg1 ));
      WN_linenum(return_wn) = line_number;
      WN_INSERT_BlockLast( *firstprivate_blockp, return_wn );  

      WN* el_len = WN_CreateLdid (OPCODE_make_op(OPR_LDID, el_len_mtype, el_len_mtype),
                               el_len_offset, old_st, MTYPE_To_TY(el_len_mtype) ); // bits

      el_len = WN_Lshr(el_len_mtype, el_len, WN_Intconst (el_len_mtype, 3 ) ); // bytes

      WN* size = WN_CreateLdid (OPCODE_make_op(OPR_LDID, el_len_mtype, el_len_mtype),
                               0, element_count, MTYPE_To_TY(el_len_mtype) );

      size = WN_Mpy ( el_len_mtype, size, el_len );
      // Alloc Memory
      TY_IDX base_ty = FLD_type(TY_fld(v->ty));
      WN_INSERT_BlockLast(*firstprivate_blockp, 
        Gen_Auto_Alloca(v->new_st, 0, base_ty , size ) );
      // Copy Data
      WN_INSERT_BlockLast(*firstprivate_blockp,
         Gen_MP_Dope_Array_Copy(v->orig_st, v->new_st, size ));
  }
#endif
...
#ifdef ABSOFT_EXTENSIONS
    // allocate space for the localized array 
    // cover dynamic array 
    if( (TY_kind(ST_type(old_st)) == KIND_POINTER) &&
                         (ST_keep_name_w2f(old_st) || 
                          PU_src_lang(Get_Current_PU()) == PU_F77_LANG ||
                          PU_src_lang(Get_Current_PU()) == PU_F90_LANG) &&
			 (TY_kind(TY_pointed(ST_type(old_st))) == KIND_ARRAY) &&
      ...
      WN_INSERT_BlockLast(*alloca_blockp,
        Gen_Auto_Alloca(v->new_st, v->new_offset, v->ty,
                        Calculate_Array_Size(old_st, TY_pointed(v->ty))));	
    }
    // cover assumed array 
    // Based on Spec 3.0, cray pointer and assumed array may not be in reduction 
    // Do we need really to support this feature in v11? [YIN]
    if (v->is_assumed_array ){
      if (*alloca_blockp == NULL)
	    *alloca_blockp = WN_CreateBlock ( );

      // clone the dope structure 
      WN_INSERT_BlockLast(*alloca_blockp,
        Gen_MP_Load_Store(v->orig_st, v->orig_offset,
                        v->new_st, v->new_offset,
                        v->is_dynamic_array));
	 ...
	 el_len_offset = 4;
      ...
      // bypass the v->reduction_opc
      v->mtype = el_len_mtype;
      ...
      WN* call = Gen_Get_Dope_Element_Count( old_st );
      WN_INSERT_BlockLast( *alloca_blockp, call );
      PREG_NUM   rreg1, rreg2;
      GET_RETURN_PREGS(rreg1, rreg2, el_len_mtype);
      WN* return_wn = WN_Stid( el_len_mtype, 0, element_count, ST_type( element_count ),
             WN_LdidPreg ( el_len_mtype, rreg1 ));
      WN_linenum(return_wn) = line_number;
      WN_INSERT_BlockLast( *alloca_blockp, return_wn );  
...
//	  ( TY_kind( ST_type( WN_st( l ))) == KIND_ARRAY ))
	...
	// check assumed array 
	if( WN_pragma_omp(l) && WN_st(l) && 
			ST_Has_Dope_Vector(WN_st(l)) &&
			(ST_sclass(WN_st(l)) == SCLASS_FORMAL_REF) ) {
	        reduction_var_type = VAR_REDUCTION_ARRAY_OMP;
        }
#endif
  ...
  INT32 var_count = v - vtab;

  // Process for automatic array 
  for( INT32 i = 0; i < var_count; i ++ ){
    if( vtab[i].vtype == VAR_LASTLOCAL ||
	vtab[i].vtype == VAR_LOCAL ||
	vtab[i].vtype == VAR_FIRSTPRIVATE ){
       
        // there is a localized automatic array 
        if( vtab[i].is_automatic_array ){
	   // Look for its new base, linked them together 
	   ST* base_st = ST_base( vtab[i].orig_st );
	   // check pre_defined, if the predefined list has base_st 
           // set base for new_st to local version, otherwise set to old base_st		
           for( INT32 l = 0; l < var_count; l ++ ){
		if( vtab[l].vtype == VAR_LASTLOCAL ||
		    vtab[l].vtype == VAR_LOCAL ||
		    vtab[l].vtype == VAR_FIRSTPRIVATE ){
		    if( vtab[l].orig_st == base_st ){ 
			base_st = vtab[l].new_st;
			break;
		    }
		}
	   }

	   Set_ST_base( vtab[i].new_st, base_st );
        }
     }
  }

  // Based on the specification, if a list item in both firstprivate and lastprivate
  // the update required for lastprivate occurs after all initializations for firstprivate
  // so I believe, add a barrier right here --[YIN]
  BOOL need_barrier = FALSE;
  if (lastlocals) {
    INT32 i;
    for (i=0; i < var_count; i++) {
	if (vtab[i].vtype == VAR_LASTLOCAL) {
           if (vtab[i].is_last_and_firstprivate ) {
		need_barrier = TRUE;
		break;
           }
	}
  ...
  if( need_barrier ){
    Is_True((*firstprivate_blockp) != NULL,
         ("Firstprivate Block Must be created at this timeD"));     
     WN* br = Gen_Barrier( local_gtid );
     WN_INSERT_BlockLast(*firstprivate_blockp,  br );   
  }
#endif
}
...
Create_Critical_Lock ( void ) 
{
  ...
  // avoid duplicate entry in global_symtab 
  // check first 
  BOOL found = FALSE;
  INT i;
  sprintf ( st_name, "__mplock_%d", (lock_id+1) );
  FOREACH_SYMBOL (GLOBAL_SYMTAB,st,i) {
      if (ST_sclass(st) != SCLASS_COMMON) continue;
      if (strcmp(ST_name(st), st_name) ==0){
      	found = TRUE;
        break;
      }
  }
...
#ifdef ABSOFT_EXTENSIONS
    Walk_and_Replace_Pregs( do_preamble_block );
#endif
  ...
  if (have_pods) {
...
#endif
    {
      ...
      if (WN_opcode(wnx) == OPC_PRAGMA) {
        ST *ppthd, *global;
#ifdef ABSOFT_EXTENSIONS
	INT64 offset = ST_ofst(WN_st(wnx));
#endif
...
#ifdef ABSOFT_EXTENSIONS
	wny = WN_Add( MTYPE_I8, wny, WN_CreateIntconst( OPC_I8INTCONST, offset) );
	wnz = WN_Add( MTYPE_I8, wnz, WN_CreateIntconst( OPC_I8INTCONST, offset) );
#endif
...
#ifdef ABSOFT_EXTENSIONS
  // Process for automatic array for reduction
  // cannot find a better place to put this 
  for( INT32 i = 0; i < num_vars; i ++ ){
    if( var_table[i].vtype == VAR_REDUCTION_ARRAY ||
	var_table[i].vtype == VAR_REDUCTION_ARRAY_OMP ){
       
        // there is a localized automatic array 
        if( var_table[i].is_automatic_array ){
	   // Look for its new base, linked them together 
	   ST* base_st = ST_base( var_table[i].orig_st );
	   // check pre_defined, if the predefined list has base_st 
           // set base for new_st to local version, otherwise set to old base_st		
           for( INT32 l = 0; l < num_vars; l ++ ){
		if( var_table[l].vtype == VAR_REDUCTION_ARRAY ||
		    var_table[l].vtype == VAR_REDUCTION_ARRAY_OMP ){
		    if( var_table[l].orig_st == base_st ){ 
			base_st = var_table[l].new_st;
			break;
		    }
		}
	   }

	   Set_ST_base( var_table[i].new_st, base_st );
        }
     }
  }
#endif
  ...
  for (i = 0, v = var_table; i < num_vars; i++, v++)
    if (v->vtype == VAR_REDUCTION_SCALAR ||
...
#ifdef ABSOFT_EXTENSIONS
      if(v->vtype == VAR_REDUCTION_ARRAY_OMP && v->is_assumed_array )
      {		
		// v is assumed, use function call 

		// initialize Array element.
		// Determine the proper value to store into array.
		WN *wn_init_val = Make_Reduction_Identity( v->reduction_opr, v->mtype );	

		WN *call_wn = Gen_MP_Reduction_Assumed_Array_Init( v->new_st, wn_init_val );	
	        WN_INSERT_BlockLast( *init_block, call_wn );
      }else 
#endif 
      if(v->vtype == VAR_REDUCTION_ARRAY_OMP)
      {
	    ...
	    // Create loop nest.
#ifdef ABSOFT_EXTENSIONS
	    // the address pointer will be processed to do the right job
	    if( v->is_automatic_array ) continue; 
	    
	    // v is dynamic or static 
	    // Now new_st is a pointer or array 
	    BOOL pointer_array = TRUE;
	    if( TY_kind( ST_type( v->new_st ) ) == KIND_ARRAY ) pointer_array = FALSE;
	    
	    ARB_HANDLE arb_base;
	    if( pointer_array ) arb_base = TY_arb( TY_pointed(ST_type( v->new_st )));
		else arb_base = TY_arb( ST_type( v->new_st ));
#else 
...
#ifdef ABSOFT_EXTENSIONS
		     // This algorithm doesn't support array of array
		     WN* dim_size = NULL;
		     if( pointer_array ) dim_size = Calculate_Array_Dim_Count( v->new_st, TY_pointed(ST_type( v->new_st )), j );
			else dim_size = Calculate_Array_Dim_Count( v->new_st, ST_type( v->new_st ), j );
		     sizes[ndim_array-j-1] = dim_size;
		     sizes_bak[j] = WN_COPY_Tree( dim_size );
#else
		...
		WN_array_base( local_array ) = WN_CreateLdid( OPCODE_make_op( OPR_LDID, 
				Pointer_type, Pointer_type ), 0,v->new_st,v->ty);
	    }else{
#endif
...
#ifdef ABSOFT_EXTENSIONS
	    WN *wn_store_val = NULL;
	    if( pointer_array ){
		TY_IDX ety = TY_AR_etype( TY_pointed(v->ty) );
	    	wn_store_val = WN_Istore( v->mtype, 0, 
			    Make_Pointer_Type(ety), local_array, wn_init_val ); 	
	    }else{
		wn_store_val = WN_Istore( v->mtype, 0, 
...
//	    Gen_Flush(v->orig_st, v->orig_offset));
...
#ifdef ABSOFT_EXTENSIONS
    }else if(v->vtype == VAR_REDUCTION_ARRAY_OMP && v->is_assumed_array == TRUE) {
	
	int ropr = -1;
	switch( v->reduction_opr ){
		case OPR_ADD: ropr = 0; break;// +
		case OPR_MPY: ropr = 1; break;// *
		case OPR_SUB: ropr = 2; break;// -
		case OPR_CAND: ropr = 3; break;//.and.
		case OPR_CIOR:  ropr = 4; break;//.or.
		case OPR_EQ:  ropr = 5; break;//.eqv.
		case OPR_NE: ropr = 6; break;//.neqv.
		case OPR_MAX: ropr = 7; break;//max
		case OPR_MIN: ropr = 8; break;//min
		case OPR_BAND: ropr = 9; break;//iand
		case OPR_BIOR: ropr = 10; break;//ior
		case OPR_BXOR: ropr = 11; break;//ieor
		default:
		Is_True( FALSE, ("Unsupported operator in OPENMP reduction for an assumed array")); 
	};
	WN* call_wn = Gen_MP_Reduction_Assumed_Array_Opt( v->orig_st, v->new_st, ropr );
        WN_INSERT_BlockLast( *store_block, call_wn );
#endif
      ...
      // it is a pointer type or array 
      BOOL pointer_array = TRUE;
      if( TY_kind( v->ty ) == KIND_ARRAY ) pointer_array = FALSE;
      
      TY_IDX ety = v->ty;
      ARB_HANDLE arb_base;
      if( pointer_array ){
	ety = TY_AR_etype(TY_pointed( v->ty ) );
	if( TY_size(ety) == 0 && TY_kind(ety) == KIND_ARRAY ){
		FmtAssert( FALSE, ( "Array of a array is not supported in Gen_MP_Reduction" ) );
	}
	arb_base = TY_arb(  TY_pointed(ST_type( v->new_st )) );
      }else{
	arb_base = TY_arb( ST_type( v->new_st ));
      }
#else
      ARB_HANDLE arb_base = TY_arb( ST_type( v->new_st ));
#endif 
	  ...
	  WN* dim_size = NULL;
	  if( pointer_array ){
		dim_size = Calculate_Array_Dim_Count( v->new_st, TY_pointed(ST_type( v->new_st )), j );
	  }else{
		dim_size = Calculate_Array_Dim_Count( v->new_st, ST_type( v->new_st ), j );
	  }
	  sizes[ndim_array-j-1] = dim_size;
	  sizes_bak[j] = WN_COPY_Tree( dim_size );
#else
      ...
      WN *old_array = NULL;
      if( pointer_array ){
	WN_element_size( new_array ) = TY_size( ety );
	WN_array_base( new_array ) = WN_CreateLdid( OPCODE_make_op( OPR_LDID, 
				Pointer_type, Pointer_type ), 0,v->new_st,v->ty);
	
	old_array = WN_Create( op_array, 1+2*ndim_array );
	WN_element_size( old_array ) = TY_size( ety );
	WN_array_base( old_array ) = WN_CreateLdid( OPCODE_make_op( OPR_LDID, 
				Pointer_type, Pointer_type ), 0,v->orig_st,v->ty);
      }else{
	WN_element_size( new_array ) = TY_size( TY_AR_etype( v->ty ));
      ...
      WN *wn_new_val = NULL;
      if( pointer_array ){
        wn_old_val = WN_Iload( v->mtype, 0, Make_Pointer_Type(ety ),
	                         WN_COPY_Tree( old_array ));
        wn_new_val = WN_Iload( v->mtype, 0, Make_Pointer_Type(ety ),
	                         new_array );
      }else{
        wn_old_val = WN_Iload( v->mtype, 0, Make_Pointer_Type( v->ty ),
	                         WN_COPY_Tree( old_array ));
        wn_new_val = WN_Iload( v->mtype, 0, Make_Pointer_Type( v->ty ),
 	                         new_array );
      }
#else 
      ...
      WN *wn_store_val = NULL;
      if( pointer_array ){
         wn_store_val = WN_Istore( v->mtype, 0, Make_Pointer_Type(ety),
                                    old_array, result );
      }else{
	 wn_store_val = WN_Istore( v->mtype, 0, Make_Pointer_Type( v->ty ),
                                    old_array, result );
      }
#else 
...
#ifdef ABSOFT_EXTENSIONS
      // WORKSHARE is temporarily treated as SINGLE 
      // This should be improved in the future release [YIN 2009-FEB-01]
      Strip_Nested_MP( workshare_block, FALSE );
      Transform_Parallel_Block (workshare_block);
      mpt = MPP_SINGLE;      
      ++num_constructs;
      workshare_block = Gen_MP_SingleProcess_Block(workshare_block, FALSE, NULL, TRUE, TRUE);
#endif
...
#ifdef ABSOFT_EXTENSIONS
/*	      WN* nested_block = WN_CreateBlock ( );
	      WN* prev_kid = WN_prev(kid);
	      WN_prev(kid) = NULL;
	      WN_next(kid) = NULL;
	      WN_INSERT_BlockLast( nested_block, kid );
	      
	      // Backup global data 
	      OMP_Lowering_Context_switch cnt;
	      cnt.Push_Context();
	      cnt.Init_Context();

	      nested_block = lower_block( nested_block, LOWER_MP );

	      cnt.Pop_Context();

	      WN_next( nested_block ) = next_kid;
	      WN_prev( nested_block ) = prev_kid;
	      if( prev_kid ){
		WN_next( prev_kid ) = nested_block;
	      }else{
		WN_first( tree ) = nested_block;
	      }
	      next_kid = nested_block;*/
...
//	  wn = Gen_MP_Exit_Gate (gate_construct_num);
	...
	default:
      ...
      mpt = save_mpt;
               ...
               WN_first(WN_region_pragmas(cur_node)) &&
	       WN_pragma(WN_first(WN_region_pragmas(cur_node))) == WN_PRAGMA_PARALLEL_BEGIN) {

      WN_EXTRACT_FromBlock(tree, cur_node);
      WN* nested_block = WN_CreateBlock ( );
      WN_INSERT_BlockLast( nested_block, cur_node );
      ...
      WN_INSERT_BlockAfter(tree, prev_node, nested_block);
 ...
 */
...
#ifdef KEY
  if (LANG_Enable_CXX_Openmp && PU_Info_pu(ppuinfo).unused)
  { // C++: This code has not yet been tested.
    ...
    pu.unused = Process_PU_Exc_Info(PU_Info_pu(ppuinfo).unused, var_table);
  }
...
//  }
  ...
  if (lastlocal_nodes) {
    ...
    // Based on the specification, if a list item in both firstprivate and lastprivate
    // the update required for lastprivate occurs after all initializations for firstprivate
    // so I believe, add a barrier right after firstprivate --[YIN]
