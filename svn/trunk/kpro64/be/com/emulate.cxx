...
const EM_ROUTINES em_routines[]=
{
  ...
  EM_Q_ABS,         "_absoft_fabsl",  PURE_NSE,  COERCE_by_value,	/* quad absolute value */
  EM_Q_SQRT,        "_absoft_sqrtl", PURE_NSE,  COERCE_by_value,	/* quad square root */
  EM_Q_ADD,         "_absoft_qadd",  PURE_NSE,  COERCE_by_value,	/* quad plus */ 
  EM_Q_SUB,         "_absoft_qsub",  PURE_NSE,  COERCE_by_value,	/* quad minus */
  EM_Q_MPY,         "_absoft_qmul",  PURE_NSE,  COERCE_by_value,	/* quad multiply */
  EM_Q_DIV,         "_absoft_qdiv",  PURE_NSE,  COERCE_by_value,	/* quad divide */
  EM_Q_MAX1,        "__q_max1",      PURE_NSE,  COERCE_none,	/* quad max */
  EM_Q_MIN1,        "__q_min1",      PURE_NSE,  COERCE_none,	/* quad min */
  EM_Q_EQ,          "_absoft_qeq",   PURE_NSE,  COERCE_by_value,	/* quad equal */
  EM_Q_NE,          "_absoft_qne",   PURE_NSE,  COERCE_by_value,	/* quad not equal */
  EM_Q_GE,          "_absoft_qge",   PURE_NSE,  COERCE_by_value,	/* quad greater equal */
  EM_Q_GT,          "_absoft_qgt",   PURE_NSE,  COERCE_by_value,	/* quad greater than */
  EM_Q_LE,          "_absoft_qle",   PURE_NSE,  COERCE_by_value,	/* quad less equal */
  EM_Q_LT,          "_absoft_qlt",   PURE_NSE,  COERCE_by_value,	/* quad less than */
  EM_SNGL_Q,        "_absoft_q2f",      PURE_NSE,  COERCE_none,	/* convert quad to single */
  EM_DBLE_Q,        "_absoft_q2d",      PURE_NSE,  COERCE_none,	/* convert quad to double */
  EM_KI_QINT,       "_absoft_q2ll",     PURE_NSE,  COERCE_none,	/* convert quad to 64 bits int */
  EM_JI_QINT,       "_absoft_q2l",     PURE_NSE,  COERCE_none,	/* convert quad to 32 bits int */
  EM_Q_EXT,         "_absoft_f2q",       PURE_NSE,  COERCE_none,	/* convert float to quad */
  EM_Q_EXTD,        "_absoft_d2q",      PURE_NSE,  COERCE_none,	/* convert double to quad */
  EM_Q_FLOTK,       "_absoft_ll2q",  PURE_NSE,  COERCE_by_value,	/* convert to quad from 64 bits int */
  EM_Q_FLOTKU,      "_absoft_ull2q", PURE_NSE,  COERCE_by_value,	/* convert to quad from unsigned 64 bits int */
  EM_Q_FLOTJ,       "_absoft_l2q",     PURE_NSE,  COERCE_none,	/* convert to quad from 32 bits int */
  EM_Q_FLOTJU,      "_absoft_ul2q",    PURE_NSE,  COERCE_none,	/* convert to quad from unsigned 32 bits int */
  EM_KIQNNT,        "__kiqnnt",      PURE_NSE,  COERCE_none,	/* round quad to closest 64 bits int value */
  EM_JIQNNT,        "__jiqnnt",      PURE_NSE,  COERCE_none,	/* round quad to closest 32 bits int value */
#else
...
static WN *em_mod_float(WN *block, WN *x, WN *y)
{
  ...
  WN		*div, *cvt, *mpy, *sub;
#ifdef ABSOFT_EXTENSIONS
  if( Is_Target_32bit() && ((type == MTYPE_F4) || (type == MTYPE_F8)) ){
    /* Check Expand_Mod_Float */   
    sub = WN_Binary( OPR_MOD, type, x, y );
    return sub;
  }else 
#endif
...
/*log10-ABSOFT */
#define M_LOG10Q_hi (double)4601495173785380110LL
#define M_LOG10Q_lo (double)4353102142499388121LL
#endif
...
#if defined(ABSOFT_EXTENSIONS)
	QUAD_TYPE abs_temp;
	abs_temp = Zero_Quad(abs_temp);
	abs_temp.hi = M_LOG10Q_hi;
	abs_temp.low = M_LOG10Q_lo;
     mpy =  WN_Mpy(type,
		   Make_Const(Host_To_Targ_Quad( abs_temp )),
		   log);
#else
     mpy =  WN_Mpy(type, 
...

  {
    ...
    */
	...
	( WN_intrinsic(tree) == INTRN_F8COS ||
	  ...
	  WN_intrinsic(tree) == INTRN_F8LOG10 ||
	  WN_intrinsic(tree) == INTRN_F4LOG10 ||
	  WN_intrinsic(tree) == INTRN_F4VSIN ||
	  WN_intrinsic(tree) == INTRN_F4VCOS ||
	  WN_intrinsic(tree) == INTRN_F8VLOG10 ||
	  WN_intrinsic(tree) == INTRN_F4VLOG10 ||
#endif
      ...
      case INTRN_F8LOG10:  st = Gen_Intrinsic_Function(ty, "fastlog10"); break;
      case INTRN_F4LOG10:  st = Gen_Intrinsic_Function(ty, "fastlog10f"); break;
#endif
...
#ifdef ABSOFT_EXTENSIONS
      case INTRN_F4VSIN: 
	st = Gen_Intrinsic_Function(ty, "vrsa_sinf");	
	vector_call_check_constant_stride = TRUE;
	break;
#endif
      case INTRN_F8VCOS: 
...
#ifdef ABSOFT_EXTENSIONS
      case INTRN_F4VCOS: 
	st = Gen_Intrinsic_Function(ty, "vrsa_cosf");
	vector_call_check_constant_stride = TRUE;
	break;
#endif
      case INTRN_F8VEXP: 
...
#ifdef ABSOFT_EXTENSIONS
      case INTRN_F8VLOG10: 
	st = Gen_Intrinsic_Function(ty, "vrda_log10");
	vector_call_check_constant_stride = TRUE;
	break;
      case INTRN_F4VLOG10: 
	st = Gen_Intrinsic_Function(ty, "vrsa_log10f");
	vector_call_check_constant_stride = TRUE;
	break;
#endif	
      }
...
#ifdef ABSOFT_EXTENSIONS
    } else if (OPT_Fast_Powf && WN_intrinsic(tree) == INTRN_F4EXPEXPR ){
      st = Gen_Intrinsic_Function(ty, "_absoft_fastpowf"); 
#endif
    } else {
      st = Gen_Intrinsic_Function(ty, function);
    }
...
static WN *emulate_intrinsic_op(WN *block, WN *tree)
{
  ...
  */
  ...
  case INTRN_F8LOG10:
#if !defined(ABSOFT_EXTENSIONS)
  /* log10-ABSOFT.removed transformation for quad. This needs to have the correct
     constant in em_alog10 but a quick attemp by me didn't work.  See elem4 test 
     for a check of this.  SR[12/7/05] */
  case INTRN_FQLOG10:
    /*
     *	log(x) * (M_LOG10E = 0.43429448190325182765)
     */
