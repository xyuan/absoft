 ...
 * $Revision: 3054 $
 * $Date: 2009-06-29 15:27:13 -0400 (Mon, 29 Jun 2009) $
 * $Author: yin $
 * $Source$
 *
 * Revision history:
...
static ST *Create_Global_Threadprivate_Symbol(ST *old_st)
{
  ...
  INT i;
  ST* st;
  ST_SCLASS new_sclass = (ST_sclass (old_st) == SCLASS_FSTATIC) ?
                                SCLASS_FSTATIC : SCLASS_COMMON;
  FOREACH_SYMBOL (GLOBAL_SYMTAB,st,i) {
      if (ST_sclass(st) != new_sclass) continue;
      if (strcmp(ST_name(st), new_name) ==0){
        return st;
      }
  }
#endif 
...
Rename_Threadprivate_COMMON(WN* pu, WN* parent, WN *wn, RENAMING_STACK *stack, RENAMING_SCOPE *scope, RENAMING_SCOPE *common_blk_scope)
{
  ...
  if (st) {
	...
	TY_IDX final_ty = ST_type(st);
	// use the ldid type instead for struct
	if( TY_kind(final_ty) == KIND_STRUCT && WN_rtype(wn) != MTYPE_M ){
	  final_ty = MTYPE_To_TY( WN_rtype(wn) );
	}
#endif
...
#ifdef ABSOFT_EXTENSIONS
		   final_ty,
                   Make_Pointer_Type(final_ty,FALSE),
#else
                   ST_type(st),
	...
	TY_IDX final_ty = ST_type(st);
	// use the ldid type instead for struct
	if( TY_kind(final_ty) == KIND_STRUCT && WN_desc(wn) != MTYPE_M ){
	  final_ty = MTYPE_To_TY( WN_desc(wn) );
	}
#endif
