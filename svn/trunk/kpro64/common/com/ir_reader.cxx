...
static void ir_put_wn(WN * wn, INT indent)
{
    ...
    if (OPCODE_operator(opcode) == OPR_ILOAD || OPCODE_operator(opcode) == OPR_ISTORE ){
      if( WN_vector_index(wn) > 0 ) 
        fprintf(ir_ofile, " V<%d>", WN_vector_index(wn) );
    }
#endif
...
static void ir_put_expr(WN * wn, INT indent)
{
  ...
  for (i = 0; i < WN_kid_count(wn); i++) {
...
#ifdef ABSOFT_EXTENSIONS
	{
          if( OPCODE_is_stmt ){
	    ir_put_stmt(wn2, indent+1);
	    fprintf(ir_ofile, "%*sopcode %d not an standard expression\n", indent+1, "", op);
	  }else fprintf(ir_ofile, "%*sopcode %d not an expression\n", indent+1, "", op);
	}
#else
...
#ifdef ABSOFT_EXTENSIONS
extern void dump_tree_to_file(char* filename, WN *wn)
{
   FILE* fp;
