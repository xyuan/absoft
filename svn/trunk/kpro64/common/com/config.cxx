...
BOOL  CG_force_common_16bit_aligned = TRUE; /* output 16bit align for .comm */
BOOL Safe_Math = FALSE;			/* Disable all options that hurt numerial accuracy no matter the individual options are set */
BOOL Safe_Math_Set = FALSE;		/* ... option seen? */
INT32 Speed_Math_Level = 0; 		/* Enable the optimizations that hurt numerical precisions and safe-proof optimizations*/
BOOL Speed_Math_Set = FALSE; 		/* ... option seen? */
INT32 Speed_OpenMP_Level = 0;			/* Enable the optimizations that cause validation test fail but good for some other tests */
BOOL Speed_OpenMP_Set = FALSE;		/* ... option seen? */
INT32 FPU_Trap = 0;        		/* Set FPU Control Register */
BOOL  FPU_Trap_Set = FALSE;             /* ... option seen? */
BOOL Gen_MRWE_Timer = FALSE;            /* Gen call to MRWE_Timer */
#endif
...
#ifdef TARG_X64
/* Pass by -stack:??? from the driver to let backend issue a warning/error for stack overflow */
UINT64 Windows_Stack_Size = 1048576;
/* Emit .section drectv for exporting functions in order to create dll for windows */
BOOL Emit_Dll_Section = FALSE;
#endif
#ifdef TARG_X8664
...
#ifdef TARG_X64
  { OVK_UINT64,	OV_INTERNAL,	FALSE, "stack",		NULL,
    1048576, 4096, INT64_MAX,	&Windows_Stack_Size, NULL,
    "Generate code assuming >32KB stack frame" },
  { OVK_BOOL,	OV_INTERNAL,	FALSE, "emit_dll_section",		NULL,
    0, 0, 0,	&Emit_Dll_Section, NULL,
    "Generate code assuming >32KB stack frame" },
#endif
#ifdef TARG_X8664
...
#ifdef ABSOFT_EXTENSIONS
  /* Set FPU Control Register, not all processor supports this feature */
  { OVK_INT32,   OV_INTERNAL,	FALSE, "fpu_trap",	"fpu_trap",
    0, 0, 47, &FPU_Trap, &FPU_Trap_Set },  

  { OVK_BOOL,	OV_INTERNAL, FALSE,"quad_align_common", "",
    0, 0, 0, &CG_force_common_16bit_aligned, NULL },

  { OVK_BOOL,	OV_INTERNAL, FALSE,"mrwe", "",
    0, 0, 0, &Gen_MRWE_Timer, NULL },
#endif
...
#ifdef ABSOFT_EXTENSIONS
#if Is_True_On
    { OVK_BOOL,	OV_INTERNAL,	FALSE, "devwarn",	NULL,
      0, 0, 0,	&dev_warn_enabled, NULL },
#endif
#endif
#ifdef KEY
...
#ifdef ABSOFT_EXTENSIONS
  if ( CG_force_common_16bit_aligned ){
    Aggregate_Alignment = 16;
  }
#endif
}
  ...
  /**  NOTE: filename CAN BE NULL */
{
  ...
  if( Speed_Math_Set ){
     ...
     switch( Speed_Math_Level ){
	default:
	{
	    if( Speed_Math_Level < 0 ) break;  
	}
        case 11:
	{
            if( !Fast_NINT_Set ){
	      Fast_NINT_Set = TRUE;
              Fast_NINT_Allowed = TRUE;	    
	    }
	    if( !Fast_ANINT_Set ){	    
              Fast_ANINT_Set = TRUE;
              Fast_ANINT_Allowed = TRUE;	// bug 7835
	    }
	    if( !Roundoff_Set ){
	      Roundoff_Level = ROUNDOFF_ANY;
	      Roundoff_Set = TRUE;
	    ...
	    if( !IEEE_Arith_Set ){
	      ...
	      IEEE_Arith_Set = TRUE;
            ...
            if( !Simp_Fold_Unsigned_Relops_Set ){
	      Simp_Fold_Unsigned_Relops_Set = TRUE;
              Simp_Fold_Unsigned_Relops = TRUE;
	    }
	    if( !Fast_Sqrt_Set  ){
	      Fast_Sqrt_Set = TRUE;
              Fast_Sqrt_Allowed = TRUE;
	    }     
#ifdef BACK_END
	    if( Run_autopar && LNO_Run_AP > 0){
	      // 64bit Channel Fails with -apo
	...
	case 10:
	{
	    // This level is for polyhedron only
	    if( !Recip_Set ){
...
#ifdef BACK_END
	    if( LNO_Full_Unrolling_Loop_Size_Limit == 2000 ){ // default value 
		LNO_Full_Unrolling_Loop_Size_Limit = 7000;
	    }

	    if( LNO_Full_Unrolling_Limit == 5 ){ // default value 
		LNO_Full_Unrolling_Limit = 9;
	    }
 
	    if( LNO_Num_Iters == 1000 ){ //default value 
		LNO_Num_Iters = 200;
	    }
#endif	    
	}
	case 9:
	{
	    ...
	    if( !Fast_Complex_Set ) {
	      // capacita failed when this is on with apo. 
	      Fast_Complex_Set = TRUE; 
              Fast_Complex_Allowed = TRUE;
            }
	    if( !DCE_Casgn_Remove_Set ){
	      DCE_Casgn_Remove_Set = TRUE;
	      DCE_Casgn_Remove = 2;
	    }
	}
	case 8:
        ...
        case 6:
	{
		...
		Enable_Cfold_Reassociate = TRUE;
		Cfold_Reassoc_Set = TRUE;
            }
	}
        case 5:
	{   
        ...
        case 4:
	{
	    if( !Fast_trunc_Set ){
	      Fast_trunc_Set = TRUE;
              Fast_trunc_Allowed = TRUE;
	    }
	    if( !OPT_Fast_Powf_Set ){
	      OPT_Fast_Powf  = TRUE;
	      OPT_Fast_Powf_Set = TRUE;
	    }
	}
	case 3:
	{   
	    if( !WOPT_Enable_Iload_Prop_Set ){
	    	WOPT_Enable_Iload_Prop_Set = TRUE;
	    	WOPT_Enable_Iload_Prop = TRUE;
	    }
	}
     	case 2:
	{
	    if( !WOPT_Enable_Store_PRE_Set ){
	        WOPT_Enable_Store_PRE_Set =TRUE;
		WOPT_Enable_Store_PRE = TRUE;
	    }
	    if( !WOPT_Enable_Load_PRE_Set ){
	        WOPT_Enable_Load_PRE_Set =TRUE;
		WOPT_Enable_Load_PRE = TRUE;
	    }
	}
     	case 1:
	{
	    if( !Simp_Unsafe_Relops_Set ){
	      Simp_Unsafe_Relops = TRUE;
	      Simp_Unsafe_Relops_Set = TRUE;
	    }
	}
     	case 0:
	{
	    if( !Allow_wrap_around_opt_Set ){
              // 32bit Linux Capacitia Fails Suddenly-- Yin @ Jun 6, 2006
              // Don't konw why
	      Allow_wrap_around_opt = TRUE;
	      Allow_wrap_around_opt_Set = TRUE;
	    }
	    ...
	    if( !WOPT_Enable_Strong_MP_Barrier_Set ){
	      WOPT_Enable_Strong_MP_Barrier = FALSE; // strong degrade OpenMP peformance if TRUE
	    }
	    
	    // AND in be/cg/hb_sched.cxx
            // List Schedule is set to aggressive DEPTH_FIRST local scheduling
	} 
     };
  }
  ...
  if( Speed_OpenMP_Set ){
     ...
     switch( Speed_OpenMP_Level ){
	default:
	{
	    if( Speed_OpenMP_Level < 0 ) break;  
	}
     	case 5:
	{
 	   if( !OPT_Enable_Risky_Thdprv_Set ){
		OPT_Enable_Risky_Thdprv = TRUE;
	   } 
	} 
     	case 4:
	{
	   if( !OPT_Enable_Rigid_Chunksize_Set ){
		OPT_Enable_Rigid_Chunksize = FALSE;
	   }
	} 
     	case 3:
	{
	   if( !WOPT_Enable_MPPragma_Need_CHI_Set ){
		WOPT_Enable_MPPragma_Need_CHI = FALSE;
	   }
	} 
     	case 2:
	{
	   if( !WOPT_Enable_MPPragma_Need_MU_Set ){
		WOPT_Enable_MPPragma_Need_MU = FALSE;
	   }
	} 
     	case 1:
	{
	   if( !WOPT_Enable_Loose_MP_Equivalent_Set ){
		WOPT_Enable_Loose_MP_Equivalent = TRUE;
	   }
	} 
     	case 0:
	{
	   if( !WOPT_Enable_Strong_MP_Barrier_Set ){
		WOPT_Enable_Strong_MP_Barrier = FALSE;
	   }
	} 
     };
...
#else
  /* disable as default for all levels */
...
#ifdef ABSOFT_EXTENSIONS
   /* Support for Safe Math
      Disable all options that hurt numerical accuracy no matter the indivdual options are set 
      This option is supposed to help testing
      */
   if( Safe_Math_Set == TRUE && Safe_Math == TRUE ){
       ...
       Fast_Complex_Set = FALSE; 
       Fast_Complex_Allowed = FALSE;
       Fast_Exp_Set = FALSE;
       Fast_Exp_Allowed = FALSE;
       Fast_NINT_Set = FALSE;
       Fast_NINT_Allowed = FALSE;
       Fast_Sqrt_Set = FALSE;
       Fast_Sqrt_Allowed = FALSE;
       Fast_trunc_Set = FALSE;
       Fast_trunc_Allowed = FALSE;
       Cfold_Reassoc_Set = FALSE;
       Enable_Cfold_Reassociate = FALSE;
       Simp_Unsafe_Relops_Set = FALSE;
       Simp_Unsafe_Relops = FALSE;
       Simp_Fold_Unsigned_Relops_Set = FALSE;
...
#ifdef TARG_X8664
       Fast_ANINT_Set = FALSE;
       Fast_ANINT_Allowed = FALSE;	// bug 7835
#endif   
   }
