 ...
 * Module: config.h
 * $Revision: 3027 $
 * $Date: 2009-05-21 09:02:54 -0400 (Thu, 21 May 2009) $
 * $Author: yin $
 * $Source$
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *config_rcs_id = "$Source$ $Revision: 3027 $";
#endif /* _KEEP_RCS_ID */
...
extern BOOL  CG_force_common_16bit_aligned; /* output 16bit align for .comm */
extern BOOL Safe_Math;			/* Disable all options that hurt numerial accuracy no matter the individual options are set */
extern BOOL Safe_Math_Set;		/* ... option seen? */
extern INT32 Speed_Math_Level;		/* Enable the optimizations that hurt numerical precisions and safe-proof optimizations*/
extern BOOL Speed_Math_Set; 		/* ... option seen? */
extern INT32 Speed_OpenMP_Level;		/* Enable the optimizations that cause validation test fail but good for some other tests */
extern BOOL Speed_OpenMP_Set;		/* ... option seen? */
extern INT32 FPU_Trap;    	     	/* Set FPU Control Register */
extern BOOL  FPU_Trap_Set;      	/* ... option seen? */
extern BOOL Gen_MRWE_Timer;	 	/* Generate call to mrwe_timer for loops and branches */
#if Is_True_On
extern BOOL dev_warn_enabled;           /* Disable DevWarn */
#endif
#endif
