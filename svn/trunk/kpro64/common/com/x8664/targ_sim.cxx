			  ...
			  enum X86_64_PARM_CLASS classes[MAX_CLASSES])
{
  ...
  if (TY_kind(ty) == KIND_STRUCT ||
    ...
    case MTYPE_CQ:
      ...
      // I believe this should be X86_64_SSE_CLASS but gcc return it by memory,
      // So memory type
  ...
  switch (mtype) {
    ...
    case MTYPE_FQ:
 ...
 #ifdef ABSOFT_EXTENSIONS
      {
		...
		info.preg[1] = PR_last_reg(SIM_INFO.dbl_results);
	 }
      }
#else
	...
	break;
      ...
      if (Is_Target_32bit()){
        ...
        if( TY_size(rtype) <= 8 && TY_size(rtype) > 0 ){
		    if( TY_size(rtype) > 4 ){
	              // Return by %EAX and %EDX
	              info.count = 2;
	              info.mtype[0] = info.mtype[1] = SIM_INFO.int_type;
...
Get_Parameter_Location (TY_IDX ty, BOOL is_output)
{
    ...
    case MTYPE_FQ:
    	{
#ifdef ABSOFT_EXTENSIONS
	// Based on the x86-64 ABI. the 128 floating point should only use
	// a single xmm register
