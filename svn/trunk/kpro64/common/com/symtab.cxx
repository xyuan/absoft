...
ST::Print (FILE *f, BOOL verbose) const
{
	...
	if (ty_idx != 0 && TY_tylist (ty) != 0) {
	    ...
	    if (flags & PU_HAS_SPECIALABI) fprintf (f, " special_abi");
#endif
...
#ifdef ABSOFT_EXTENSIONS
            if (flags_ext & ST_IS_GLOBAL)
                fprintf (f, " global" );
            if (flags_ext & ST_IS_INITIALIZED_IN_FILE)
                fprintf (f, " initialized_in_file" );
	    if (flags_ext & ST_IS_STDCALL)
	        fprintf (f, " stdcall" );
	    if (flags_ext & ST_IS_INTRINSIC)
	        fprintf (f, " intrinsic" );	    	    
#endif
	}
#endif
