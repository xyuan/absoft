...
/* Define a quad-precision floating point type appropriate to host: */
	...
	typedef struct { double hi; double low; } QUAD_TYPE;
	inline QUAD_TYPE Zero_Quad(QUAD_TYPE q) {q.hi = 0.0; q.low = 0.0; return q; }
  #if defined(ABSOFT_EXTENSIONS)
    #define TCON_R16HI(c)	((c).vals.qval.hi)
    #define TCON_IR16HI(c)	((c).cmplxval.qival.hi)
  #endif
#else
...
extern void Targ_Emit_Const ( FILE *fl, TCON tvalue, BOOL add_null, UINT64 rc, INT64 loc );
#ifdef KEY
extern void Targ_Emit_EH_Const ( FILE *fl, TCON tvalue, BOOL add_null, UINT64 rc, INT64 loc, INT format=0 );
#endif // KEY
#else
