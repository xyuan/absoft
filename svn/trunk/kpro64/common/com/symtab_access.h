...
inline BOOL
ST_is_been_localized (const ST* s)	{ return s->flags_ext & ST_IS_BEEN_LOCALIZED;}
inline void
Set_ST_is_been_localized (ST* s)		{ s->flags_ext |= ST_IS_BEEN_LOCALIZED; }
inline void
Clear_ST_is_been_localized (ST* s)	{ s->flags_ext &= ~ST_IS_BEEN_LOCALIZED; }
...
inline BOOL
ST_is_Function_Cri_Pointee (const ST* s)     { return s->flags_ext & ST_IS_FUNCTION_CRI_POINTEE; }
inline void
Set_ST_is_Function_Cri_Pointee (ST *s)       { s->flags_ext |= 0x00000100; }
inline void
Clear_ST_is_Function_Cri_Pointee (ST *s)     { s->flags_ext &= ~ST_IS_FUNCTION_CRI_POINTEE; }


inline BOOL
ST_is_Global (const ST* s)     { return s->flags_ext & ST_IS_GLOBAL; }
inline void
Set_ST_is_Global (ST *s)       { s->flags_ext |= ST_IS_GLOBAL; }
inline void
Clear_ST_is_Global (ST *s)     { s->flags_ext &= ~ST_IS_GLOBAL; }


inline BOOL
ST_is_STDCALL (const ST* s)    { return s->flags_ext & ST_IS_STDCALL; }
inline void
Set_ST_is_STDCALL (ST *s)      { s->flags_ext |= ST_IS_STDCALL; }
inline void
Clear_ST_is_STDCALL (ST *s)     { s->flags_ext &= ~ST_IS_STDCALL; }


inline BOOL
ST_is_INTRINSIC (const ST* s)    { return s->flags_ext & ST_IS_INTRINSIC; }
inline void
Set_ST_is_INTRINSIC (ST *s)      { s->flags_ext |= ST_IS_INTRINSIC; }
inline void
Clear_ST_is_INTRINSIC (ST *s)     { s->flags_ext &= ~ST_IS_INTRINSIC; }


inline BOOL
ST_is_initialized_in_file (const ST* s) { return s->flags_ext & ST_IS_INITIALIZED_IN_FILE; }
inline void
Set_ST_is_initialized_in_file (ST *s)   { s->flags_ext |= ST_IS_INITIALIZED_IN_FILE; }
inline void
Clear_ST_is_initialized_in_file (ST *s) { s->flags_ext &= ~ST_IS_INITIALIZED_IN_FILE; }

inline BOOL
PU_has_SpecialABI (const PU& pu)	{ return (pu.flags & PU_HAS_SPECIALABI) != 0; }
inline void
Set_PU_has_SpecialABI (PU& pu)		{ pu.flags |= PU_HAS_SPECIALABI; }
inline void
Clear_PU_has_SpecialABI (PU& pu)	{ pu.flags &= ~PU_HAS_SPECIALABI; }
#endif
...
#else
inline char *
LABEL_name (const LABEL& lbl)		{ return Str_Table[lbl.name_idx] == '.' ? (&Str_Table[lbl.name_idx])+1 : (&Str_Table[lbl.name_idx]); }
#endif
