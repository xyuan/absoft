...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *config_opt_rcs_id = "$Source$ $Revision: 3044 $";
#endif /* _KEEP_RCS_ID */
...
BOOL Allow_wrap_around_opt = FALSE;
BOOL Allow_wrap_around_opt_Set = FALSE;
UINT32 DCE_Casgn_Remove = FALSE;
BOOL DCE_Casgn_Remove_Set = FALSE;
BOOL OPT_Fast_Powf = FALSE;
BOOL OPT_Fast_Powf_Set = FALSE;
BOOL Recip_Allowed_F4Only = FALSE;
BOOL OPT_Enable_Risky_Thdprv = FALSE;
BOOL OPT_Enable_Risky_Thdprv_Set = FALSE;
BOOL OPT_Enable_Rigid_Chunksize = TRUE;
BOOL OPT_Enable_Rigid_Chunksize_Set = FALSE;
#else
...
#ifdef ABSOFT_EXTENSIONS
  { OVK_BOOL,	OV_VISIBLE,	TRUE, "safe_math",		"safe_math",
    0, 0, 0,	&Safe_Math,	&Safe_Math_Set,
    "Disable all options that hurt numerial accuracy no matter the individual options are set" },
  
  { OVK_INT32,	OV_VISIBLE,	TRUE, "speed_math",		"speed_math",
    0, 0, 11,	&Speed_Math_Level,	&Speed_Math_Set,
    "Enable the optimizations that hurt numerical precisions and not safe-proof optimizations" },

  { OVK_INT32,	OV_VISIBLE,	TRUE, "speed_openmp",		"speed_openmp",
    0, 0, 5,	&Speed_OpenMP_Level,	&Speed_OpenMP_Set,
    "Enable the optimizations that occational fail some programs but work for most programs" },

  { OVK_INT32,	OV_VISIBLE,	TRUE, "dce_casgn",		"dce_casgn",
    0, 0, 2,	&DCE_Casgn_Remove,	&DCE_Casgn_Remove_Set,
    "Apply dead casgn function elimination in DCE" },    

  { OVK_BOOL,	OV_VISIBLE,	TRUE, "fast_powf",		"fast_powf",
    0, 0, 0,	&OPT_Fast_Powf,	&OPT_Fast_Powf_Set,
    "Utilize fast_powf instead of regular powf in libm" },    

  { OVK_BOOL,	OV_VISIBLE,	TRUE, "risky_thdprv",		"risky_thdprv",
    0, 0, 0,	&OPT_Enable_Risky_Thdprv,	&OPT_Enable_Risky_Thdprv_Set,
    "OPENMP: Use a simple version algorithm to handle OpenMP common block" },   

  { OVK_BOOL,	OV_VISIBLE,	TRUE, "rigid_chunksize",		"rigid_chunksize",
    0, 0, 0,	&OPT_Enable_Rigid_Chunksize,	&OPT_Enable_Rigid_Chunksize_Set,
    "OPENMP: Do Simd in schedule with chunksize" },    
