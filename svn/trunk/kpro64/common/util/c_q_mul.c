 ...
 *  Module: c_q_mul.c
 *  $Revision: 2503 $
 *  $Date: 2006-11-30 12:05:35 -0500 (Thu, 30 Nov 2006) $
 *  $Author: yin $
 *  $Source$
 *
 * =======================================================================
...
#define constant 4294967297.000 /* 2.**(64 - 64/2) + 1.0 */
#define MAX_UNSCALED_VAL 9.7453140113999980e+288 /* DBL_MAX / 2**64 */

double ldexp(double,int); /* from math.h */

extern double ldexp(double, int);	/* from <math.h> */

/* note that this routine must be kept in sync with the corresponding libc
 * routine, q_mul.
 */
 
extern	QUAD	c_q_mul(QUAD, QUAD);
...
QUAD c_q_mul(QUAD a, QUAD c) 
	...
	int scale = 0;
	double a1, a2, c1, c2, t, c21, c22;
	
	if(a.hi > MAX_UNSCALED_VAL) {
	    a.hi = ldexp(a.hi, -64);
	    scale = 1;
	} else if(c.hi > MAX_UNSCALED_VAL) {
	    c.hi = ldexp(c.hi, -64);
	    scale = 1;
	}
	...
	c22 = c2 - c21;
	ac.hi = a.hi * c.hi;
	ac.lo = ((((a1 * c1 - ac.hi) + a1 * c2) + c1 * a2) + c21 * a2) + c22 * a2;
