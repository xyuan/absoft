 ...
 *  Module: c_q_div.c
 *  $Revision: 1444 $
 *  $Date: 2006-02-20 16:23:51 -0500 (Mon, 20 Feb 2006) $
 *  $Author: yin $
 *  $Source$
 *
 * =======================================================================
...
QUAD QPROD(double a, double c) /* exact multiplication*/
	{
	QUAD ac;
	int scale = 0;
	double a1, a2, c1, c2, t, c21, c22;
	
	if(a > MAX_UNSCALED_VAL) {
	    a = ldexp(a, -64);
	    scale = 1;
	} else if(c > MAX_UNSCALED_VAL) {
	    c = ldexp(c, -64);
	    scale = 1;
	}
...
QUAD	result;
	...
	z = x.hi / y.hi;
	u = QPROD(y.hi, z);
	zz = ((((x.hi - u.hi) - u.lo) + x.lo) - z*y.lo) / (y.hi + y.lo);
	result.hi = z + zz;
