...
static char *rcs_id = "$Source$ $Revision: 2859 $";
 ...
 * Append_Extension
 *
 * Append the given file name's extension with another extension and
 * return a new filename string.  The given extension should include
 * the period if desired (a period in the original name will be
 * eliminated).
 *
...
char *
Append_Extension ( const char *name, const char *ext )
{
  char *new;
  INT16 len, i;
...
#ifdef TARG_X64
#define is_absolute_file_name(file_name)	((file_name)[0] == '/' || (file_name)[1] == ':')
#else
...
Make_Absolute_Path (char *filename)
{
   ...
   if ( cwd == NULL ) {
	...
	char* name = cwd;
	if( name != NULL && strncmp( name, "/cygdrive/", 10 ) == 0 ){
		if( name[11] == '/' ){
