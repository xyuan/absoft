...
//
//  $Revision: 2937 $
//  $Date: 2008-11-24 14:13:46 -0500 (Mon, 24 Nov 2008) $
//  $Author: yin $
//  $Source$
...
int main()
{
...
#ifdef ABSOFT_EXTENSIONS
  ISA_PRINT_TYPE add_label_offset =  ISA_Print_Type_Create("add_label_offset", "%s $\\\"%s\\\" - \\\"%s\\\" , %s");
  Name();
  Operand(0);  
  Operand(1);
  Result(0); 
  Instruction_Print_Group( add_label_offset,
  			   TOP_addlofst32,
			   TOP_UNDEFINED );
			   

  ISA_PRINT_TYPE leapic =  ISA_Print_Type_Create("leapic", "%s \\\"%s\\\" - \\\"%s\\\"(%s) , %s");
  Name();
  Operand(0);  
  Operand(1);
  Operand(2);
  Result(0); 
  Instruction_Print_Group( leapic,
  			   TOP_leapic32,
			   TOP_leapic64,
			   TOP_UNDEFINED );  
#endif
...
#ifdef ABSOFT_EXTENSIONS
  /* SSE4.1 extractps instruction w/obase with indx and control value */
  ISA_PRINT_TYPE extractps_n32 = ISA_Print_Type_Create("extractps_n32", "%s %s,%s,%s");
  Name();
  Operand(2);
  Operand(0);
  Segment();
  Operand(1);
  Instruction_Print_Group( extractps_n32,
		    	   TOP_extractps_n32,
	            	   TOP_pextrb_n32,	
	      	    	   TOP_pextrw_n32,	
	            	   TOP_pextrd_n32,	
  ...
  /* SSE4.1 extractps instruction with base indx and control value */
  ISA_PRINT_TYPE extractps = ISA_Print_Type_Create("extractps", "%s %s,%s,%s%s(%s)");
  Name();
  Operand(3);
  Operand(0);
  Segment();
  Operand(2);
  Operand(1);
  Instruction_Print_Group( extractps,
		    	   TOP_extractps,
		    	   TOP_pextrb,
		    	   TOP_pextrw2,
		    	   TOP_pextrd,
  ...
  /* SSE4.1 extractps instruction with indx and control value */
  ISA_PRINT_TYPE extractpsx = ISA_Print_Type_Create("extractpsx", "%s %s,%s,%s%s(%s,%s,%s)");
  Name();
  Operand(5);
  Operand(0);
  Segment();
  ...
  Operand(3);
  Operand(4);
  Instruction_Print_Group( extractpsx,
			   TOP_extractpsx,
			   TOP_pextrbx,
			   TOP_pextrwx,
			   TOP_pextrdx,
  ...
  /* SSE4.1 extractps instruction with indx and control value w/o base */
  ISA_PRINT_TYPE extractpsxx = ISA_Print_Type_Create("extractpsxx", "%s %s,%s,%s%s(,%s,%s)");
  Name();
  Operand(4);
  Operand(0);
  Segment();
  ...
  Operand(2);
  Operand(3);
  Instruction_Print_Group( extractpsxx,
			   TOP_extractpsxx,
			   TOP_pextrbxx,
			   TOP_pextrwxx,
			   TOP_pextrdxx,
  ...
  /* SSE4.1 insertps instruction memory w/o base */
  ISA_PRINT_TYPE insertps_n32 = ISA_Print_Type_Create("insertps_n32", "%s %s, %s, %s");
  Name();
  Operand(1);
  Operand(0);
  Result(0);
  Instruction_Print_Group( insertps_n32,
	            	   TOP_insertps_n32,	
	            	   TOP_pinsrb_n32,	
	            	   TOP_pinsrw_n32,	
	            	   TOP_pinsrd_n32,	
  ...
  /* SSE4.1 insertps instruction memory */
  ISA_PRINT_TYPE insertps = ISA_Print_Type_Create("insertps", "%s %s,%s%s(%s), %s");
  Name();
  Operand(2);
  Segment();
  Operand(1);
  Operand(0);
  Result(0);
  Instruction_Print_Group( insertps,
		    	   TOP_insertps,
		    	   TOP_pinsrb,
		    	   TOP_pinsrw2,
		    	   TOP_pinsrd,
  ...
  /* SSE4.1 insertps instruction with indx and control value*/
  ISA_PRINT_TYPE insertpsx = ISA_Print_Type_Create("insertpsx", "%s %s,%s%s(%s,%s,%s), %s");
  Name();
  Operand(4);
  Segment();
  Operand(1);
  ...
  Operand(3);
  Result(0);
  Instruction_Print_Group( insertpsx,
			   TOP_insertpsx,
			   TOP_pinsrbx,
			   TOP_pinsrwx,
			   TOP_pinsrdx,
  ...
  /* SSE4.1 insertps instruction with indx and control value w/o base*/
  ISA_PRINT_TYPE insertpsxx = ISA_Print_Type_Create("insertpsxx", "%s %s,%s%s(,%s,%s), %s");
  Name();
  Operand(3);
  Segment();
  Operand(0);
  ...
  Operand(2);
  Result(0);
  Instruction_Print_Group( insertpsxx,
			   TOP_insertpsxx,
			   TOP_pinsrbxx,
			   TOP_pinsrwxx,
			   TOP_pinsrdxx,
