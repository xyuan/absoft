      ...
      if (check != NULL)
	{
	  if (! bfd_check_format (check, bfd_object))
		  ...
		  && ! ldemul_unrecognized_file (entry))
		{
		  ...
		  // Handle .dylib
		  if( strlen( attempt ) > 6 ){
		  	char *ext = attempt + strlen(attempt) - 5;
			if( strcasecmp( ext, "dylib" ) == 0 ) return TRUE;
		  }
#endif
...
#ifdef __CYGWIN__
	  // ABSOFT_EXTENSIONS
	  found = ldfile_open_file_search (arch->name, entry, "", "");
#else
#ifdef __APPLE__
	  // ABSOFT_EXTENSIONS
	  // Also check the .dylib
	  found = ldfile_open_file_search (arch->name, entry, "lib", ".dylib");	  
	  if (found)
	    break;	  
#endif
