...
#ifdef __CYGWIN__
  // ABSOFT_EXTENSIONS
  // Support MS Linker type searching
  if( file_type == lang_input_file_is_file_enum ){
      ...
      if( strcasecmp( ext, "LIB" ) == 0 ){
	      ...
	      if (working_dir == 0) {
			...
			exit(1);
			    ...
			    for( i = 9; i <= length; i ++ ){
				 working_dir[i-9] = working_dir[i];
			    } 	
			    working_dir[0] = working_dir[1];
			    working_dir[1] = ':';
		    	}
	    	    }
		    ldfile_add_library_path( working_dir, FALSE );
	      }	      
      }
  }
#endif
