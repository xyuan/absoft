...
// ABSOFT_EXTENSIONS
void *(*p_ipa_open_input)(char *, int *) = NULL;
#else
...
void (*p_process_whirl32)(void *, int, void *, int, const char *) = NULL;
#else
...
#ifdef __APPLE__
//ABSOFT_EXTENSIONS
    p_handle = dlopen("lno.so",RTLD_LAZY );
    if (!p_handle) {
...
#if defined(__CYGWIN__) || defined(__APPLE__)
//ABSOFT_EXTENSIONS
	if( p_bfd_link_hash->type == bfd_link_hash_defined ) result = USED_IN_OBJ;
	else result = DEF_IN_OBJ;
...
char*
create_tmp_file (char* filename)
{
    static char* tmppath = "/tmp/";
    static char* tmpsuffix = "XXXXXX";
    char* tmp_file_name;
    char* ret_file_name;
    FILE *file;

    tmp_file_name = (char*)MALLOC(strlen(tmppath) + strlen(filename) + strlen(tmpsuffix) + 1);
