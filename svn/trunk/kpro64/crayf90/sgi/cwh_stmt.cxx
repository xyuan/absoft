...
/*
  ...
  or the like.  Any license provided herein, wheth1043erer implied or 
...
cwh_stmt_call_helper(INT32 num_args, TY_IDX ty, INT32 inline_state, INT64 flags)
{
  ...
  args = (nargs) ? (WN **) malloc(nargs*sizeof(WN *)) : NULL;

#if defined(ABSOFT_EXTENSIONS)
  count = (VF_CHAR_OPTION) ? clen : num_args;
  for (k = count-1 ; k >= 0  ; k --) {
#else
...
#if defined(ABSOFT_EXTENSIONS)
      if(VF_CHAR_OPTION)
        args[k--] = cwh_intrin_wrap_value_parm(wa);
      else
...
#if defined(ABSOFT_EXTENSIONS)
  if (ST_auxst_has_rslt_tmp(st) || (tr && cwh_types_is_character(tr))) {
#else
  if (ST_auxst_has_rslt_tmp(st) || cwh_types_is_character(tr)) {
#endif

    tr = args ? cwh_types_WN_TY(args[0],FALSE) : tr;
...
#if defined(ABSOFT_EXTENSIONS)
     if(ST_auxst_is_auto_or_cpointer(st)) {
       WN_set_ty(wn,TY_pointed(ST_type(ST_base(st))));
       WN_kid(wn,nargs) = cwh_addr_load_ST(st,0,ST_type(ST_base(st)));
     } else {
#endif
  ...
  if ((ST_auxst_is_elemental(st)) && (TY_mtype(ts) != MTYPE_V)
      ...
      && (TY_mtype(ts) != MTYPE_M) && (TY_mtype(ts) != MTYPE_FQ)
#endif
     ) {
...
#if defined(ABSOFT_EXTENSIONS)
  if (ts == 0)
    ts = (wn) ? cwh_types_WN_TY(wn,FALSE) : cwh_types_WN_TY(ln,FALSE);
#else
  if (ts == 0)
...
#ifdef ABSOFT_EXTENSIONS
// f2003 indirect_goto
      assign_id = cwh_auxst_assign_id(CURRENT_SYMTAB, lb);
      if( *assign_id == -1 ){
	*assign_id = cwh_assign_label_id++;
      }
      cwh_stmt_append_truebr(WN_COPY_Tree(expr),(*assign_id),OPR_EQ,lb);
#else
...
#if defined(ABSOFT_EXTENSIONS)
     if(One_Trip_Do_Loops)
        temp = cwh_expr_bincalc(OPR_MAX,temp,
...
#if defined(ABSOFT_EXTENSIONS)
   if (!strcmp(ST_name(routine),"_ACL_DEALLOC")) {
#else
...
#if defined(ABSOFT_EXTENSIONS)
   } else if (!strcmp(ST_name(routine),"_ACL_ALLOCATE")) {
#else
...
cwh_stmt_init_pu(ST * st, INT32 lineno)
{
  ...
  if ((strcmp(ST_name(st), "MAIN__") == 0 || (strcmp(ST_name(st),"_MAIN__") == 0 ) ) &&
#else
  if (strcmp(ST_name(st), "MAIN__") == 0 &&
    ...
    ST_Init(str_st, Save_Str("__absoft_compiler"), CLASS_VAR, SCLASS_DGLOBAL,
