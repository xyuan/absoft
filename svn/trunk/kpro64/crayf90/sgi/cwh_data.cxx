 ...
 *
 * $Revision: 912 $
 * $Date: 2005-12-09 16:33:23 -0500 (Fri, 09 Dec 2005) $
 * $Author: sr $
 * $Source$
 *
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source$ $Revision: 912 $";
#endif /* _KEEP_RCS_ID */
     ...
     
{
   ...
   if((byte_stride > init_size) || (current_pos > 0) ||array_pos > 0) {
     current_data_info->Reserve(dup_count);
     for (i=0; i < dup_count; i++) {
      
current_data_info->Add_Data_Element(current_pos+element_offset,value,bo,init_size,1);
       element_offset += byte_stride;
     }
   } else {
     current_data_info->Reserve(1);
     current_data_info->Add_Data_Element(current_pos+element_offset,value,
                                	 bo,init_size*dup_count,dup_count);
   }
#else
   current_data_info->Reserve(dup_count);
   for (i=0; i < dup_count; i++) {
