 ...
 * Module: cwh_io.c
 * $Revision: 3005 $
 * $Date: 2009-04-22 13:23:00 -0400 (Wed, 22 Apr 2009) $
 * $Author: yin $
 * $Source$
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
  static char *rcs_id = " $Id: cwh_io.cxx 3005 2009-04-22 17:23:00Z yin $ ";
#endif /* _KEEP_RCS_ID */
  ...
  if (cwh_stk_get_class() == FLD_item) {
     ...
     if (ST_sclass(st) == SCLASS_FORMAL) {
        ...
        if(ST_is_value_parm(st) && 
           ...
           TY_kind(TY_pointed(ST_type(st))) == KIND_STRUCT)
...
#ifdef ABSOFT_EXTENSIONS
  open_list = (WN **) malloc(sizeof (WN *) * (int)OPEN_ERR);
#else
...
#ifdef ABSOFT_EXTENSIONS
                  else if (item == OPEN_CARRIAGECONTROL)
		    wn = WN_CreateIoItem2 (IOC_CARRIAGECONTROL, addr, length, NIL);
                  else if (item == OPEN_CONVERT)
		    wn = WN_CreateIoItem2 (IOC_CONVERT, addr, length, NIL);
#if defined (TARG_X64) || defined(__APPLE__)
                  else if (item == OPEN_FILETYPE)
		    wn = WN_CreateIoItem2 (IOC_FILETYPE, addr, length, NIL);
                  else if (item == OPEN_CREATOR)
		    wn = WN_CreateIoItem2 (IOC_CREATOR, addr, length, NIL);
#endif
                  else if (item == OPEN_DISPOSE)
		    wn = WN_CreateIoItem2 (IOC_DISPOSE, addr, length, NIL);
#endif
...
#ifdef ABSOFT_EXTENSIONS
               else if (item == OPEN_BLOCKSIZE) 
                 wn = WN_CreateIoItem1(IOC_BLOCKSIZE, addr, ty);
               else if (item == OPEN_BLOCK) 
                 wn = WN_CreateIoItem1(IOC_BLOCK, addr, ty);
               else if (item == OPEN_BUFFER) 
                 wn = WN_CreateIoItem1(IOC_BUFFER, addr, ty);
               else if (item == OPEN_MAXREC) 
                 wn = WN_CreateIoItem1(IOC_MAXREC, addr, ty);
...
#ifdef ABSOFT_EXTENSIONS
  inq_list = (WN **) malloc(sizeof (WN *) * (int)INQ_ERR);
#else
	...
	case INQ_FILE:
	   switch(cwh_stk_get_class()) {
		  ...
		  if (item == INQ_PAD)
		    ...
		    wn = WN_CreateIoItem2 (IOC_FILE, addr, length, NIL);
#ifdef ABSOFT_EXTENSIONS
                  else if (item == INQ_CARRIAGECONTROL)
		    wn = WN_CreateIoItem2 (IOC_CARRIAGECONTROL, addr, length, NIL);
#endif
	...
	case INQ_UNIT:
	   ...
	   switch(cwh_stk_get_class()) {
		 ...
		 wn = WN_CreateIoItem1(IOC_IOSTAT, addr, ty);
                 ...
                 unit = WN_CreateIoItem1(IOU_EXTERNAL, addr, NIL);
#ifdef ABSOFT_EXTENSIONS
               else if (item == INQ_BLOCKSIZE) 
                 wn = WN_CreateIoItem1(IOC_BLOCKSIZE, addr, ty);
               else if (item == INQ_FLEN) 
                 wn = WN_CreateIoItem1(IOC_FLEN, addr, ty);
               else if (item == INQ_SIZE) 
                 wn = WN_CreateIoItem1(IOC_INQ_SIZE, addr, ty);
#endif
               if (item != INQ_UNIT)
	         inq_list[num_items++] = wn;
	       break;
                          ...
                          WN *char_len, TY_IDX ty, WN *craytype)
{
  ...
  if (indflag == 0) {
      ...
      wn = cwh_dope_from_expression(tree, NULL, char_len, ty, craytype);
#ifdef ABSOFT_EXTENSIONS
      // ad has been used in cwh_dope_initialize
      wn = WN_CreateIoItem2(IOL_DOPE, wn, WN_COPY_Tree(ad), NIL);
#else
      wn = WN_CreateIoItem2(IOL_DOPE, wn, ad, NIL);
#endif
      return wn;
