 ...
 *
 * $Revision: 2645 $
 * $Date: 2007-04-02 13:24:28 -0400 (Mon, 02 Apr 2007) $
 * $Author: sr $
 * $Source$
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source$ $Revision: 2645 $";
#endif /* _KEEP_RCS_ID */
             ...
             INT64         flags )
{
  ...
  ty = cwh_types_mk_procedure_TY(ret_ty,num_dum_args,TRUE,FALSE, test_flag(flags, FEI_PROC_STDCALL) );
#else
...
#ifdef TARG_X64
  if (test_flag(flags, FEI_PROC_STDCALL)) {
    /* the STDCALL flag is set */
    st->flags_ext |= ST_IS_STDCALL;
  }
#endif  
	     ...
	     INT64          flags)
{
  ...
  case PDGCS_Proc_Intern_Ref:
	...
	eclass = EXPORT_LOCAL_INTERNAL;
      ...
      st = cwh_stab_mk_fn_0args(name_string,
				...
				level,
#ifdef ABSOFT_EXTENSIONS
				cast_to_TY(t_TY(result_type)), test_flag(flags, FEI_PROC_STDCALL) );
#else
				cast_to_TY(t_TY(result_type)));
#endif
#if defined(ABSOFT_EXTENSIONS)
      if(test_flag(flags,FEI_PROC_CRI_POINTEE)) {
  ...
  if (sym_class == F90_Module
#if defined(ABSOFT_EXTENSIONS)
      || (sym_class == Unknown_Func && Class == PDGCS_Proc_Imported)
#endif
     )
...
#if defined(ABSOFT_EXTENSIONS)
  if (test_flag(flag_bits,FEI_OBJECT_VOLATILE))
...
/* ignore stmt fn dummy arg - not used */
  ...
  if (in_common) {
    ...
    if (st) {
#if defined(ABSOFT_EXTENSIONS)
      if(ptr_st_idx || sym_class == CRI_Pointee) {
...
st = New_ST(GLOBAL_SYMTAB);
...
#if defined(ABSOFT_EXTENSIONS)
    if(base_st->sym_class != CLASS_FUNC)
#endif
...
#if defined(ABSOFT_EXTENSIONS)
    if (test_flag(flag_bits,FEI_OBJECT_FUNCTION_CRI_POINTEE)) {
      ty = cwh_types_mk_procedure_TY (ty,0,TRUE,hosted,FALSE);

      Set_ST_is_Function_Cri_Pointee(st);
...
#if defined(ABSOFT_EXTENSIONS)
    if (test_flag(flag_bits,FEI_OBJECT_DARG_BY_VALUE)) {
      Set_ST_is_value_parm(st);
    }
#endif
  ...
  if (IS_FORMAL(st)
#if defined(ABSOFT_EXTENSIONS)
      && !test_flag(flag_bits,FEI_OBJECT_CRI_POINTEE)
#endif
      ) {
    if (! hosted )
...
extern ST *
#ifdef ABSOFT_EXTENSIONS
cwh_stab_mk_fn_0args(char *name, ST_EXPORT eclass,SYMTAB_IDX level,TY_IDX rty,BOOL stdcall)
#else
