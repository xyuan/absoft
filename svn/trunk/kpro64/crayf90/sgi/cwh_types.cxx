 ...
 * Module: cwh_types.c
 * $Revision: 2933 $
 * $Date: 2008-11-12 14:18:26 -0500 (Wed, 12 Nov 2008) $
 * $Author: yin $
 * $Source$
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source$ $Revision: 2933 $";
#endif /* _KEEP_RCS_ID */
...
#define ABSOFT_CP_IN_DT 4096
char absoft_dt_member_name_is_automatic[ABSOFT_CP_IN_DT][128];
int absoft_dt_member_name_is_automatic_offset[ABSOFT_CP_IN_DT];
int absoft_dt_member_name_is_automatic_idx = 0;
int absoft_dt_last_is_auto = 0;
int absoft_dt_base_off = 0;
int absoft_dt_prev_base_off = 0;
char absoft_dt_base_name[256];
char absoft_dt_prev_base_name[256];
#endif
    ...
    
{
  ...
  case Func_tion:                  /* external, passed as arg */
    ...
    ty_idx = cwh_types_mk_procedure_TY(Be_Type_Tbl(MTYPE_V), 0,TRUE,FALSE,FALSE);
#else
...
#if defined(ABSOFT_EXTENSIONS)
if(test_flag(flag_bits, FEI_OBJECT_CRI_POINTEE)) {
  Set_FLD_flags(fld, FLD_flags(fld) | (1 << 15));
}
if(test_flag(flag_bits, FEI_OBJECT_CRI_POINTEE_2)) {
  Set_FLD_flags(fld, FLD_flags(fld) | (1 << 14));
}
#endif


#if 0 //defined(ABSOFT_EXTENSIONS)
  if(member_type.automatic) {
    strcpy(absoft_dt_member_name_is_automatic[absoft_dt_member_name_is_automatic_idx],
       TY_name(d.dty));
    if(absoft_dt_member_name_is_automatic_idx > ABSOFT_CP_IN_DT - 1) {
      printf("Internal Error: more than %d cray pointers in a derived type\n", ABSOFT_CP_IN_DT);
      printf("Contact support@absoft.com\n");
    }
    if(strlen(TY_name(d.dty)) > 127) {
      printf("Internal Warning: length of name of cray pointers in a derived type\n");
      printf("Contact support@absoft.com\n");
    }
    absoft_dt_member_name_is_automatic_offset[absoft_dt_member_name_is_automatic_idx++] = off;
  }
#endif
...
extern TY_IDX 
#ifdef ABSOFT_EXTENSIONS
cwh_types_mk_procedure_TY (TY_IDX ret_typ_idx, INT32 nparms, BOOL global, BOOL host, BOOL stdcall)
#else
...
#if defined(ABSOFT_EXTENSIONS)
      || cwh_types_is_character_function(ST_type(dummy))
#endif
     ) {
...
cwh_cray_type_from_TY(TY_IDX ty_idx)
{
   TY_IDX base_ty_idx;
#ifdef ABSOFT_EXTENSIONS
   volatile INT64 rtype;
   volatile f90_type_t  *f90_type_ptr; // For gcc 4.2 -O2 problem
#else
   INT64 rtype;
