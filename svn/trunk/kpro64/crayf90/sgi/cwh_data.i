...
struct data_element_s {
...
#if defined(ABSOFT_EXTENSIONS)
  inline data_element_s(INT64 o, TCON_IDX tc, b_and_o *bo,
                        INT64 isize, UINT32 seqnum, INT64 rep_count) {
#else
  inline data_element_s(INT64 o, TCON_IDX tc, b_and_o *bo, INT64 isize, UINT32 seqnum) {
    ...
    if (bo) {
      ...
      val.tc_val.tc = tc;
#if defined(ABSOFT_EXTENSIONS)
      val.tc_val.repeat_count = rep_count ? rep_count : 1;
#else
...
#if defined(ABSOFT_EXTENSIONS)
  inline void Add_Data_Element(INT64 offset, TCON_IDX val,
                               b_and_o *bo, INT64 isize, INT64 rep_count) {
#else
