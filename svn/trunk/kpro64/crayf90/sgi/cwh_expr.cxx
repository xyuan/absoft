 ...
 * Module: cwh_expr
 * $Revision: 2940 $
 * $Date: 2008-11-25 14:06:11 -0500 (Tue, 25 Nov 2008) $
 * $Author: yin $
 * $Source: 
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source$ $Revision: 2940 $";
#endif /* _KEEP_RCS_ID */
...
cwh_expr_compare_logical(OPCODE opc,TY_IDX ty)
{
  ...
  if( MTYPE_is_float( WN_rtype(rhs) ) || MTYPE_is_complex( WN_rtype(rhs) ) ){
	// Add CVT to OPCODE_rtype 
	 rhs = WN_Cvt( WN_rtype(rhs), OPCODE_rtype(opc), rhs );
  }
  if( MTYPE_is_float( WN_rtype(lhs) ) || MTYPE_is_complex( WN_rtype(lhs) ) ){
	// Add CVT to OPCODE_rtype 
	 lhs = WN_Cvt( WN_rtype(lhs), OPCODE_rtype(opc), lhs );
  }
#endif
  wn  = WN_CreateExp2 ( opc, lhs, rhs) ;
  wn  = cwh_expr_restore_arrayexp(wn,ae);
...
fei_select(TYPE type)
{
   ...
   if (TY_is_character(ty)) {
      ...
      /* this compares the strlens for merge - the following causes issues in
      meisner */
      strlen1 = cwh_expr_operand(NULL);
      addr   = cwh_expr_address(f_NONE);
      t_case = cwh_addr_mload(addr,0,ty,strlen1);

      OPCODE opc = OPCODE_make_op(OPR_MIN,MTYPE_I8,MTYPE_V);
      strlen = WN_CreateExp2 ( opc, strlen, strlen1) ;
#else
      strlen = cwh_expr_operand(NULL);
      addr   = cwh_expr_address(f_NONE);
