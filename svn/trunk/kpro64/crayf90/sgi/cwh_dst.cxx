 ...
 * Module: cwh_dst.c
 * $Revision: 3047 $
 * $Date: 2009-06-16 12:49:40 -0400 (Tue, 16 Jun 2009) $
 * $Author: yin $
 * $Source$
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source$ $Revision: 3047 $";
#endif /* _KEEP_RCS_ID */
...
extern void
#ifdef ABSOFT_EXTENSIONS
cwh_dst_init_file(char *src_path, char *src_fname)
#else
...
#if defined(ABSOFT_EXTENSIONS) && defined(TARG_X8664)
  if(!file)
    file = strrchr(src_path,'/');
#endif
#else
...
#ifdef ABSOFT_EXTENSIONS
  strcpy(comp_info, "Absoft Fortran ");
#else
...
#ifdef TARG_X64
  // Process CYGWIN type path name reverse back
  char* name = current_working_dir;
...
#if defined(ABSOFT_EXTENSIONS) && defined(TARG_X8664)
  if( src_fname != NULL ){
	(void) cwh_dst_enter_path(src_fname);
  }
#else
...
#ifdef ABSOFT_EXTENSIONS
    if( ST_auxst_is_tmp(el) ){
        DST_SET_artificial(DST_INFO_flag(DST_INFO_IDX_TO_PTR(m))); 
    }
#endif
...
#ifdef ABSOFT_EXTENSIONS
  if( TY_kind(ty) == KIND_ARRAY && cwh_dst_is_character_TY(ty) ){
    i = cwh_dst_substring_type(ty);	
  }else
#endif
