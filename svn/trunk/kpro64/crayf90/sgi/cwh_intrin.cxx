 ...
 * Module: cwh_intrin
 * $Revision: 2404 $
 * $Date: 2006-08-16 11:36:38 -0400 (Wed, 16 Aug 2006) $
 * $Author: yin $
 * $Source: 
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source$ $Revision: 2404 $";
#endif /* _KEEP_RCS_ID */
...
fei_matmul(TYPE rtype)
{
   ...
   /* ABSOFT expects an extra argument that is zero.  This is in for
   compatibility with Commercial Beta 2 */
   wn = cwh_expr_operand(NULL);
   wn = NULL;
#endif
   for (i=1; i >= 0; i--) {
...
#ifdef ABSOFT_EXTENSIONS
   ST *param_st = WN_st(wn);
   WN *param_load;
   if( param_st == NULL ){
        param_load = cwh_addr_load_WN( wn, 0, 0 );
   }else{ 
	TY_IDX ty = ST_type( param_st );
	param_load = cwh_addr_load_WN( wn, 0, ty );
   }
   cwh_stk_push(param_load, DEREF_item);
#else
