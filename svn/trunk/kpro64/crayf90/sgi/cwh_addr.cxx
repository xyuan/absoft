...
fei_seq_subscr( TYPE result_type )
{
  ...
  case ST_item:
    ...
    ty = ST_type(st);
#if defined(ABSOFT_EXTENSIONS)
    if(ST_is_Global(ST_base(st))) {
      ad = cwh_addr_address_ST(ST_base(st), ST_ofst(st));
      ad = cwh_addr_load_WN(ad,0, Make_Pointer_Type(ty));
    } else {
#endif
...
cwh_addr_load_ST(ST * st, OFFSET_64 off, TY_IDX dty)
{
  ...
  case SCLASS_FORMAL:
    ...
    if ((BY_VALUE(ts) && !ST_auxst_is_auto_or_cpointer(st))
        || ST_is_Function_Cri_Pointee(st) && ST_is_value_parm(ST_base(st))) {
  ...
  case SCLASS_EXTERN:
    ...
    case KIND_SCALAR :
      ...
      if (ST_class(st)==CLASS_VAR
          && ST_auxst_is_auto_or_cpointer(st)) {
	/* need to load it through its base */
	    wa = cwh_addr_address_ST(st);

#if defined(ABSOFT_EXTENSIONS)
	wn = cwh_addr_load_WN(wa,off,0);
#else
...
#if defined(ABSOFT_EXTENSIONS)
	if(ST_is_Global(ST_base(st))) {
     	  wa = cwh_addr_address_ST(ST_base(st));
	  wa = cwh_addr_load_WN(wa,0, Make_Pointer_Type(ts));

	  wn = cwh_addr_load_WN(wa,ST_ofst(st),ts);
	} else {
...
#if defined(ABSOFT_EXTENSIONS)
	if(ST_is_Global(ST_base(st))) {
     	  wa = cwh_addr_address_ST(ST_base(st), ST_ofst(st)+off);
	  wa = cwh_addr_load_WN(wa,0, Make_Pointer_Type(ts));
...
#if defined(ABSOFT_EXTENSIONS)
	if(p_off.no_cri_pointees) {
	  do {
	    wa=cwh_addr_ldid(st,
                            p_off.offset[p_off.no_cri_pointees],
                            cwh_types_make_pointer_type(p_off.type[p_off.no_cri_pointees],FALSE));

	    if(p_off.is_pointer[p_off.no_cri_pointees])
	      wa = cwh_addr_load_WN(wa,0, Make_Pointer_Type(p_off.type[p_off.no_cri_pointees]));

	  } while(--p_off.no_cri_pointees);
	  wn = cwh_addr_load_WN(wa,off,ts);
           fg |= ACCESSED_ILOAD;
        }else if (ST_class(st)==CLASS_VAR && ST_auxst_is_auto_or_cpointer(st)) {
	    /* need to load it through its base */
	    wa = cwh_addr_address_ST(st);
	    wn = cwh_addr_load_WN(wa,off,ts);
	    fg |= ACCESSED_ILOAD;
	    ...
	    if(ST_is_Global(ST_base(st))) {
	      ...
	      wn = cwh_addr_load_WN(wa,ST_ofst(st)+off,ts);
	    } else {
...
cwh_addr_store_ST(ST * st, OFFSET_64 off, TY_IDX dty,  WN * rhs) 
{
  ...
  case SCLASS_FORMAL_REF:
	...
	if(ST_is_Global(ST_base(st))) {
	  ...
	  cwh_addr_store_WN(wa,ST_ofst(st),ts,rhs);
	} else {
...
//	      wa = cwh_addr_load_WN(wa,0, Make_Pointer_Type(p_off.type[p_off.no_cri_pointees]));
	  ...
	  if (ST_class(st)==CLASS_VAR && ST_auxst_is_auto_or_cpointer(st)) {
	    ...
	    if(ST_is_Global(ST_base(st))) {
	      ...
	      cwh_addr_store_WN(wa,ST_ofst(st)+off,ts,rhs);
	    } else {
#endif
	    wn = cwh_addr_stid(st,off,ts, rhs);
     	    cwh_block_append(wn) ;
...
#if defined(ABSOFT_EXTENSIONS)
            if (Has_Base_Block(ST_base(st)) &&
                ST_auxst_is_auto_or_cpointer(ST_base(st))) {

              TY_IDX p_tp = cwh_types_make_pointer_type(tp,FALSE);
              wn = cwh_addr_ldid(ST_base(ST_base(st)),0,p_tp);
              wn = cwh_addr_load_WN(wn,0, p_tp);
              if(off != 0)
                wn = cwh_expr_bincalc(OPR_ADD,wn,WN_Intconst(Pointer_Mtype,off));
            }/* else {
	       wa = cwh_addr_address_ST(st);
...
#if defined(ABSOFT_EXTENSIONS)
  p_off.no_cri_pointees = 0;
  long long flags;
  OFFSET_64 offset, orig_ofst;
  TY_IDX new_type;
  int first_iteration = 1;
  det.off = 0;

  while (cwh_stk_get_class() == FLD_item) {
    FLD_HANDLE fld2(cwh_stk_pop_FLD());
    flags = FLD_flags(fld2);

    new_type = FLD_type(fld2);
    if(first_iteration) {
      det.type = new_type;
      first_iteration = 0;
    }

    if (test_flag(flags,(15)) || test_flag(flags,(14))) {
      p_off.no_cri_pointees++;
      p_off.offset[p_off.no_cri_pointees] = FLD_ofst(fld2);
      p_off.type[p_off.no_cri_pointees] = new_type;
      p_off.is_pointer[p_off.no_cri_pointees] = test_flag(flags,(14));
      orig_ofst = det.off;
      det.off = 0;
    } else {
      det.off += FLD_ofst(fld2);
    }
    if(p_off.no_cri_pointees)
      det.off = orig_ofst;
