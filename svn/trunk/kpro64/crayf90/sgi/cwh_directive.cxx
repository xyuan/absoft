 ...
 * $Revision: 3022 $
 * $Date: 2009-05-13 16:25:23 -0400 (Wed, 13 May 2009) $
 * $Author: yin $
 *
    ...
    if (ST_is_temp_var(base))
      if (ST_sclass(base) == SCLASS_AUTO)
        if (!ST_is_return_var(base)
           ...
           && ST_auxst_is_auto_or_cpointer(base)
#endif
           )
...
#ifdef ABSOFT_EXTENSIONS
// frontend set all compiler generated temp variables as Context_Omp_Private
// however, the initialized temp in the static block doesn't need to become private
// so just ignore it.
  if(  ST_auxst_is_tmp(st) &&
//	 ST_is_initialized(st) &&  // initialized is not available so far. it is marked later
	 context == Context_Omp_Private &&
...
#ifdef ABSOFT_EXTENSIONS
  // nested = TRUE will trigger LEGO, which contains mp_tiling that is not supported 
  cwh_directive_set_PU_flags(FALSE);
#else
  cwh_directive_set_PU_flags(nested_do_descriptor.depth > 1);
#endif
